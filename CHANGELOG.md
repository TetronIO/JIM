...
- 🔄 Paginated list APIs and the `-All` auto-pagination in `Get-JIMMetaverseObject` / `Get-JIMConnectedSystemObject` now guard against runaway pagination at scale. Requesting a page beyond 1000 (roughly 100,000 objects at the maximum page size) returns a `400` rather than being silently clamped, so a caller learns they have over-paged and the database is never asked to scan such a deep offset. `-All` correspondingly fetches at most 1000 pages and then stops with a warning; a new `-Force` switch overrides the cap to fetch everything, and `-All` warns up front when it is used on a large result set.
