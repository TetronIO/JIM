...
- 🐛 Delta Import on 389 Directory Server and other changelog directories now applies deletions and follows renames: a deletion is identified from the copy of the deleted entry the changelog keeps, and a renamed object is read at its new name. Previously deletions were silently dropped and a renamed object was looked for under its old name; a deletion the changelog kept no copy of is now reported as a warning on the run, naming the object. (#1479)
- 🐛 An import that the directory stops at its search limit now refuses before importing anything from that container and says why: which container and object type, that the account JIM connects as is subject to the directory's search limits (which its rootDN was not), and that the fix is to exempt that account, on OpenLDAP with an `olcLimits` entry for the JIM group, rather than raising the limit for every client. Until now the run failed part-way through the container with the directory's bare "The size limit was exceeded". See [Troubleshooting](https://docs.junctional.io/connectors/jim-ldap-connector/#import-fails-with-the-directory-stopped-the-import-at-its-search-limit).
- 🐛 A Full Synchronisation no longer loses a group's provisioning when the group is processed on an earlier page than its members. The pass that resolves those cross-page member references re-evaluated the group's export with its unsent Create deleted first, so the group's Create became an Update carrying only the members, the export tried to modify a group that had never been created, and the directory refused it. The unsent Create is now kept and rebuilt with the resolved members.
- 🐛 A Delta Import from a 389 Directory Server or generic LDAP directory now returns changes: the changelog search asked the directory for entries with a comparison LDAP does not define, so it had never returned a change since the changelog Delta Import was introduced, and the run completed successfully with nothing. The search now asks correctly, honours the changelog the root DSE advertises, takes its watermark from the advertised last change number where there is one instead of reading the whole changelog, walks a size-limited answer by change number, and refuses when the changelog has been trimmed past the last import's watermark. (#1725)
- 🐛 A Delta Import from a 389 Directory Server or other changelog directory no longer fails with "Duplicate external ID ... found across import pages" when an object was changed more than once since the last import. The changelog records every write, so such an object appeared once per change and was read once per change; it is now read once, on its first record, since that read returns its current state either way. Deletions are never skipped this way, so an object added and then deleted in the same window is still deleted. The accesslog path already behaved like this. (#1479)
- 🐛 A Delta Import from OpenLDAP, 389 Directory Server or a generic LDAP directory no longer completes successfully with no changes when the account JIM connects as cannot read the accesslog or the changelog. It fails before importing anything, naming the source and the remedy; Schema Discovery and the schema refresh preview warn of the same gap, as they already did for Active Directory's Deleted Objects container. (#1725)
- 🐛 A Delta Import from Active Directory now pages its search of the Deleted Objects container, so more deletions since the last import than the directory returns in one answer are all imported rather than none, and are reported plainly on the run if they still cannot be. A multi-page Delta Import now reads the container's tombstones once, rather than once per page. (#1724)
- 🐛 A Delta Import from Active Directory or Samba AD now fails fast when the account JIM connects as is not allowed to list the domain's Deleted Objects container, and completes with a warning when JIM cannot confirm that it can, instead of silently importing no deletions; Schema Discovery and the schema refresh preview warn about the same gap. Granting Read Permissions on the container itself (no OU delegation reaches it) alongside List Contents and Read Property lets JIM give a definite answer; see [Service Account Permissions](https://docs.junctional.io/connectors/jim-ldap-connector/#service-account-permissions). (#1723)
- 🐛 The LDAP Connector's hierarchy import no longer fails when the service account cannot read one of the directory's naming contexts (an OpenLDAP server hosting several suffixes, say); the unreadable partition is skipped with a warning and the rest are imported. Previously a single unreadable partition failed the whole import, which only a rootDN or Domain Admin bind avoided. (#1715)
- 🐛 Correcting a Synchronisation Rule's Initial Password through the REST API or `Set-JIMSyncRuleInitialPassword` now releases the accounts parked against it, exactly as saving in the portal does; previously only a portal save did. (#1697)
- 🐛 The Worker no longer retains every Worker Task and Activity it has ever dequeued in memory. Its main loop's database context lives for the Worker's lifetime and kept each one tracked, so a busy Worker's memory grew with every task it ran; each idle tick now releases them.
- 🐛 The Worker's housekeeping now honours a configuration change made after it first ran. Housekeeping deletes Metaverse Objects whose deletion grace period has expired and deprovisions their target objects per each export Synchronisation Rule's deprovisioning action, but it read that configuration through a database context that lived for the Worker's lifetime and never refreshed what it had already loaded, so it kept acting on the Synchronisation Rules and Metaverse Object Types as they stood the first time it ran: an export rule switched from Delete to Disconnect still had its directory objects deleted until the Worker was restarted. Each housekeeping batch now reads the configuration as it stands.
- 🐛 Provisioning that is withdrawn before it was ever exported is now cancelled cleanly. When a Metaverse Object was deleted, or left an export Synchronisation Rule's scope, before its new target object had been exported, JIM replaced the unsent Create Pending Export with a Delete for an object that did not exist: the export failed with "Delete export has no External ID value", and the Connected System Object was left in the Connector Space with no External ID or Display Name, where no import could ever clear it. Under a **Disconnect** Deprovisioning Action the unsent Create survived instead, so the next export would have created an account for someone no longer in scope. JIM now removes the unsent Create and the Connected System Object together and exports nothing, and reports the cancellation as a **Provisioning cancelled** outcome on the Activity, in the causality tree and Table view, and in Sync Preview. Provisioning that has already been exported is deprovisioned exactly as before.
- 🐛 The causality panel says what kind of object it is naming once, where nothing else does: the Lineage view's column heads wear a **CSO** or **MVO** glyph (three letters, so neither reads as the **CS** glyph a Connected System chip carries), the object chips on the cards beneath them do not repeat it, and the Timeline's chips, which have no column to speak for them, wear the same glyphs. Every object is named in one form, "type: name" with the type lowlighted, on the Lineage heads (a Metaverse Object reads "User: Ava-mae Riley", with "in the Metaverse" beneath, as a record reads "person: Ava-mae Riley" with its Connected System beneath) exactly as on the Timeline's chips. Target objects reached through an outcome (a Provisioned object in another system, an existing object an export was queued for, a deprovisioned one) now carry their type too, rather than the bare name alone. The Timeline view now draws its connecting lines through nested outcomes: each nested outcome is joined to the outcome that caused it, and a parent's line runs on past its branch to the next outcome at its own level, where before only the top-level outcomes were joined and nested ones floated free.
- 🐛 A Connected System Object whose provisioning export was sent but never confirmed by an import, and which was then deprovisioned, no longer remains in the Connector Space forever once its Delete is exported. JIM now recognises this as the only confirmation such an object can ever get and removes it immediately, rather than leaving it waiting for a confirming import that would never come.
- 🐛 An attribute change arriving after a new object's provisioning Create was exported, but before a confirming import ran, no longer sends a second Create to the target. JIM previously re-sent the Create with the latest values, which most Connectors reject for an object that already exists there. JIM now waits for the Create to be confirmed and then sends a single Update carrying the change.
- 🐛 A new object's provisioning Create that a confirming import does not fully confirm is now handled correctly either way. If the import matched the object but one exported attribute came back different, JIM previously re-sent a second Create instead of an Update, which most Connectors reject for an object that already exists there; it now retries the outstanding change as a single Update. If a Full Import does not report the object back at all, JIM previously left the Connected System Object waiting for ever, since nothing else ever revisited an unconfirmed Create; the Create is now retried on the next export, with the usual retry limit and Failed state still applying.
- 🐛 Text and Outlined chip labels now meet WCAG AA contrast in every theme. The label was painted in the theme's raw fill colour, which is chosen to sit behind white text, not to be text; in seven of the twelve themes that fell below the 4.5:1 floor, and the default dark theme's own chip colours were little better at 4.3:1; those are now brighter. Elsewhere the label blends the colour toward the theme's text colour, and the causality panel's outcome pills, lineage operation chips and Table view change chips share the same colour, so a primary chip is one purple wherever it appears.
- 🐛 The **Preview Behaviour Impact** drill-down now names the object on every row and links it, as the other previews do: projection rows link the Connected System Object and provisioning rows the Metaverse Object. Rows were stored without a name, which left the Object column blank with nothing to open.
- 🐛 Previewing a scope change on an export Synchronisation Rule no longer reports a Metaverse Object as leaving or entering **import** scope. An export rule's population is Metaverse Objects, and one with nothing in the target system was described through the import-side transitions, so the preview said an identity would "leave import scope" of a rule that has no import direction. Such an object now reports as **Leaves export scope, nothing to remove**, or, where the rule provisions, as **No longer creates a Connected System Object**, since the object it would have been given is what the exit costs; on the entering side, an object the rule would not create anything for reports as **Enters export scope**. The REST API and PowerShell previews carry the same transitions as `WouldLeaveExportScope` and `WouldEnterExportScope`. See [Previewing a scope change](https://docs.junctional.io/configuration/synchronisation-rules/#previewing-a-scope-change).
- 🐛 In the Attribute Flow editor, the **Connected System Attribute** picker no longer paints the chosen option (with its Suggested chip) inside the field, where it sat on the field's bottom border; the choice now reads as text, matching the Metaverse Attribute picker beside it.
- 🐛 A synchronisation run could fail with a duplicate-key database error when a deletion rule with a grace period scheduled a Metaverse Object for deletion during the run. Scheduling the deletion triggered an extra, out-of-turn database save partway through the page, which inserted that page's execution records early; the page's own save then tried to insert the same records again and the whole run failed. The deletion is now recorded together with the rest of the page's changes, and the run completes normally. A related failure in the same scenario is also fixed: if another Connected System Object rekeyed to the same identity within the same page, the batch save could end up holding two conflicting copies of that identity's pending changes, either failing the run outright or silently losing the scheduled deletion depending on which copy the database happened to keep. The page now consolidates the two into one before saving, keeping the scheduled deletion and the rekeyed identity's changes together.
- 🐛 A synchronisation run could fail with the same duplicate-key database error when a joined Connected System Object fell out of a Synchronisation Rule's scope during the run. Disconnecting it and recalling its contributed values triggered the same extra, out-of-turn database save partway through the page, which inserted that page's execution records early and collided with the page's own save. The recall is now recorded together with the rest of the page's changes, and the run completes normally. (#1610)
- 🐛 The Metaverse Object detail API now returns the object's joined Connected Systems: the `connectedSystemObjects` field always came back empty regardless of the object's actual join state. (#1606)

- 🐛 Clearing a Connector Space no longer leaves values contributed by departed source objects stranded on the Metaverse forever: the next Full Synchronisation of the cleared system now recalls them automatically, handing an attribute to a surviving contributor where one exists, clearing it where none does, and preserving values as last known state where no import source remains, with the outcome reported on the run's Activity. The sweep only runs once a Full Import of the cleared system has completed successfully since the clear; a Full Synchronisation run before then leaves it armed and states so on both the run's Activity and the Connected System page. (#1549)
...
- 🐛 A client IP address recorded from an IPv4 connection no longer carries the `::ffff:` prefix. Kestrel listens on a dual-stack socket, so IPv4 clients arrived as IPv4-mapped IPv6 addresses (`::ffff:203.0.113.7`) and were recorded verbatim on Authentication activities and in logs, while the same client behind a configured reverse proxy was recorded in plain form. Addresses are now normalised to the plain IPv4 form wherever JIM records or partitions by them, which also stops one client occupying two unauthenticated rate limit buckets depending on how it reached JIM. Previously recorded activities keep the form they were written with.
- 🐛 Removing an Attribute Flow mapping through the Synchronisation Rule editor can no longer leave an orphaned, ownerless mapping row behind: a mapping's owning rule is now enforced by the database, and any orphaned rows from earlier removals are cleaned up on upgrade. (#1550)
- 🐛 Applying an Active Directory schema no longer removes the attributes a class inherits. The auxiliary class merge (#492) recognises its own contributions by an attribute carrying another class's name, which is true on RFC 4512 directories but not on Active Directory, where discovery resolves the class hierarchy itself and records each attribute against the class it came from: `cn` and `sn` against `person`, `title` against `organizationalPerson`, `sAMAccountName` against `securityPrincipal`. Any schema apply, including a routine refresh, therefore stripped most of a `user` class's attributes and invalidated the Attribute Flows reading them. The merge now reconciles only object types whose class membership is handed to JIM, which Active Directory's never is. (#492)
- 🐛 Rejoining an existing directory entry no longer fails its next export with "objectClass: value #0 already exists". When export matching joined an Identity to an entry that already existed at the Connected System (i.e. one disconnected earlier and then re-added at the source), the class membership JIM composes on export (#492) was planned as though the entry carried no classes, so the update restated the structural class and the directory refused the duplicate value. Class membership is now planned against the entry actually being written to, so a rejoin stages only what the entry is missing. (#492)
- 🐛 A long attribute value in a Change History card is no longer cut off mid-value. A value that does not fit now ends in an ellipsis, carries the whole value on hover, and uses the card's full width rather than stopping at a fixed 700px.
- 🐛 A confirming import's Causality now reaches the whole story behind it, instead of stopping at the export it confirms. Every confirmation ended one hop short: the export, the synchronisation that staged it and the import that started the story were all recorded and none of them reachable, so the panel said **No earlier causes recorded** with the rest of the chain sitting right there in the database. The link was always present, in the Pending Export the confirmation and the export both name; only the walk was not following it. Nothing new is recorded to make this work, and it applies to history already captured. (#1528)
- 🐛 A confirming import's Lineage now reads left to right in causal order. The record was placed on the source side of the Identity, so a deprovisioning confirmation drew the Identity that had caused the deletion to the *right* of the record it deleted, with the two joined by "imported" though nothing had imported it; where the source record appeared too, it shared a column with the target it had nothing to do with. The item's change type could not tell the two cases apart, because deletion detection and ordinary imports run on every Connected System: the same item means "the source lost this record" on a source and "JIM's own export landed" on a target. The confirmation itself settles it, since JIM only confirms what it exported. Such a story now reads source, Identity, target, in the order the events happened. (#1528)
- 🐛 A chain ending on the Causality panel's Lineage now sits above the events it is about rather than below them. An object's events read downwards from oldest to newest, so **No earlier causes recorded** at the bottom put a statement about the beginning of the story at its end; where a story fitted in a single column it directly contradicted an earlier cause visible above it. (#1528)
- 🐛 A selected filter chip is now a solid fill in its own colour, rather than that colour as text on a tint of itself. The tinted pairing was one nothing measured, and it fell below the accessibility standard: the selected **Normal** chip on the Connector Space came out at 3.98:1 against the 4.5:1 minimum for text at its size. A solid fill uses the colour's paired text colour, which every theme is measured against on each build, so this state can no longer drift below the standard unnoticed. It also makes a selected chip obvious at a glance instead of a slightly deeper tint of an unselected one, everywhere chips filter a list. (#1527)
- 🐛 A deletion is now reported once, not on every import until a synchronisation runs. A Connected System Object stays obsolete until a Synchronisation Run Profile on its own Connected System deletes it, so every import in between found the object missing all over again and recorded another deletion for it, each one claiming the run had deleted something it never touched: the object was obsolete before the run started and obsolete after. On a system imported far more often than it is synchronised the effect is severe; one integration run recorded 81 deletions for four objects in nine minutes. Those items inflated each run's deleted total, and Causality drew every one of them as the start of a fresh chain, because a deletion arriving with no cause is exactly what it looked like. Only the first import to find an object missing now reports it; the ones that follow change nothing and say nothing. Stale Pending Exports are still cleared on every pass, so an export staged against the object by another system's synchronisation is not left to be sent to something that no longer exists. The same applies to a Connector replaying a delete on a Delta Import. (#1527)
- 🐛 Deleting an import Attribute Flow mapping no longer leaves the Metaverse values it contributed in place forever. A Full Synchronisation of the contributing Connected System now recalls a value whose mapping no longer exists, re-elects the next contributor by Attribute Priority in the same run, and where no contributor survives clears the attribute and reports it as an MVO No Contributor outcome. Deleting a mapping also advances the configuration watermark, so the Full Synchronisation actually revisits unchanged objects rather than skipping the very values the deleted mapping left behind. Disabling a mapping (or its whole Synchronisation Rule) is deliberately different: the flow is dormant, not gone, so a surviving contributor still takes the attribute over, but with no survivor the values are retained for the flow's possible re-enablement. Previously deleted mappings' values persisted indefinitely, with provenance pointing at a mapping that no longer existed, until the source data happened to change. (#1533, #1537)
- 🐛 Certificates in the JIM certificate store are now honoured for LDAPS connections on platforms whose LDAP client validates trust with OpenSSL (Ubuntu 26.04 onwards) as well as GnuTLS. Previously, such platforms silently ignored the store and refused connections to Directory Servers whose issuing certificate an administrator had trusted.
- 🐛 Deleting a Synchronisation Rule now removes the configuration it owns, rather than leaving it behind. Its Attribute Flow mappings and the sources they read from, its Scoping Criteria groups (including groups nested inside other groups) and their criteria, and any Object Matching Rules built in Advanced mode all belonged to the rule and all survived it: rows belonging to nothing, absent from every page, from every export of the configuration and from every count, accumulating with each rule anyone deleted. The deletion reported success, because leaving them behind was what the database had been asked to do. Object Matching Rules built in Simple mode are unaffected; they belong to the Connected System Object Type, which is still there. (#1477)
- 🐛 A Connected System configured for Password Synchronisation can now be deleted. The configuration names the Object Type holding the system's accounts, and that reference deliberately refuses to let the Object Type be deleted on its own, so that a configuration cannot be left aimed at nothing. Deleting the whole Connected System removes its Object Types on the way past, which the refusal was never meant to stop; it stopped it anyway, and because the deletion runs as one transaction, nothing at all was removed and the portal reported a save failure. Any system passwords had been pointed at was undeletable. (#1119)
- 🐛 A Connected System that had imported a nested Container hierarchy can now be deleted. Its Containers were removed by a single statement keyed on the Partition, and a Container discovered beneath another one belongs to its parent rather than to a Partition, so every descendant was left pointing at a row that had just gone. The database refused, the whole deletion rolled back, and the portal reported a save failure; any directory with an OU inside an OU, which is most of them, produced a Connected System that could not be deleted. Containers linked directly to the Connected System rather than through a Partition were being missed by the same statement, and are now covered too. (#1477)
- 🐛 `Set-JIMApiKey` now sends an API Key's roles as a JSON array even when there is exactly one, so updating a single-role key (the common case) no longer fails with a validation error, and a key holding no roles is sent as an empty set rather than null. (#1531)
