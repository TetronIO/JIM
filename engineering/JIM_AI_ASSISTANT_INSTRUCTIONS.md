...
JIM's core platform is complete with v0.9.0 released. Core sync (import, sync, export), scheduling, change history, dashboard, and admin UI are all functional. v0.9.0 adds 100K object scale support with bounded memory sync/export pipelines (#451, #437, #438), .NET 10 LTS migration (#174) with non-root Docker containers and read-only filesystem hardening, Service Settings REST API with PowerShell cmdlets, data integrity validation (#465), safe cancellation for sync operations (#339), LDAP export auto-tuning, and performance improvements including CachedDisplayName sort optimisation, composite indexes, and optimised paginated list queries (#482). Earlier releases include OpenLDAP connector support (#72), Worker redesign with ISyncEngine/ISyncRepository (#394), bundled Keycloak IdP (#197), O(1) import matching (#440), COPY binary protocol (#338), and comprehensive UI improvements. The roadmap progresses through v1.0-ILM-COMPLETE, v1.x-CONNECTORS, and v2.0-IGA-FOUNDATION milestones; see GitHub milestones for details.
```
