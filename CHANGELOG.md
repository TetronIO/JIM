...
- ✨ New built-in Metaverse Attributes make SCIM 2.0 systems map cleanly onto JIM's schema: Emails, Account Enabled, Nickname, Preferred Language, Locale, Time Zone, Middle Name, Honorific Prefix and Honorific Suffix. Existing deployments gain them automatically at startup. (#1104)
...
- 🐛 Recording an API key's last-used timestamp no longer surfaces error-level log entries when the database is briefly saturated by a large synchronisation run; the stamp is now a single set-based update, throttled to one write per key per 30 seconds.
...
- ⚡ Full Imports at large scale are dramatically faster: matched objects are no longer hydrated one round trip at a time, confirming a large group no longer degrades quadratically, and bulk attribute writes stream via PostgreSQL binary COPY. 210,000 objects: 40+ minutes to about 8.
...
- ⚡ Full Imports over existing objects gain further from reference lookups no longer running one query per imported object (535,000 round trips at 500,000 users), and from an internal duplicate check that scanned a growing list becoming a constant-time set lookup.
- ⚡ Full Synchronisation at large scale no longer spends most of its time re-verifying large groups for drift. The membership comparison was quadratic in group size (around 36 seconds of processor time per very large group) and is now hash-based, completing in milliseconds.
- ⚡ Synchronisation runs no longer slow down page by page through a large Connected System. Offset pagination re-scanned everything before each page (around 200ms early, 1.5s late); pages are now loaded with a keyset cursor, so every page costs the same.
- ⚡ Deleting Metaverse Objects that groups reference is now dramatically faster: staging the membership-removal Pending Exports no longer reloads every referencing group's full membership on each page. A leaver cohort that took over 9 hours is projected to finish in well under one.
- ⚡ Deleting Metaverse Objects during synchronisation is now set-based rather than object-by-object. Each page of deletions previously cost around 75 sequential round trips per object; a page flush that took around 50 seconds now completes in a few bulk operations.
...
- ⚡ Exports no longer stall between batches at large scale. Batch collection now walks the Pending Export queue in a single keyset pass, instead of rescanning it from the start for every batch.
- ⚡ The tail of a large, reference-heavy export no longer pages through Pending Exports it has already identified as deferred; once a batch turns out to be entirely reference-bearing, the remainder is collected in a single query.
- ⚡ Export runs no longer load every Pending Export to find deferred references; the check is now a single indexed query, saving around 11 minutes per run at 525,000 Pending Exports even when there is nothing to resolve. (#1102)
- ⚡ Exported attribute values are now applied to the Connected System Object as soon as the export succeeds, instead of waiting for the confirming import to re-materialise them (524,997 objects previously re-materialised 9.8 million values). Calls-based connectors only. (#1079)
- ⚡ Watching a Run Profile execute no longer costs the database an aggregation over every execution item on each refresh (measured: around 85 minutes of database time over one 500,000-user run). Stats are now incremental counters, finalised on completion. (#1078)
- ⚡ The worker service no longer floods the database with liveness updates while a task runs. Its main loop spun as fast as its two status queries allowed for the task's whole duration (6.7 million heartbeat updates over one run) and now checks once every two seconds.
- ⚡ Post-export bookkeeping no longer runs change detection once per changed value, which was quadratic; a batch containing a 100,000-member group spent over ten minutes in it. This also fixes a batch failure that could abort after the target write had succeeded.
