...
- 🐛 A multi-source Metaverse attribute contributed by a source that falls out of its Synchronisation Rule's scope is now handed to the next-priority Connected System still contributing it, matching the existing hand-over behaviour for a disconnected source, instead of being cleared. A configured deletion grace period no longer freezes recall wholesale in this case either: a re-elected attribute is still handed over, and only an attribute with no surviving contributor is frozen for the grace window.
