...
- **Never place an edge label by its anchor point; place it by its box.** A `<text>` `x`/`y` sits on the *baseline*, but the box that collides with a line reaches about 11.6 user units above that baseline (and 3.4 below) at the 12.5px label size. On a diagonal edge the line moves sideways as it crosses those 15 units, so a label that clears the line at the baseline is sliced by it at cap height; horizontal-only nudging cannot fix a diagonal. Offset the label perpendicular to the edge until all four box corners are at least 12 units clear, and slide it along the edge if a neighbouring edge is in the way. Rule and the corner formula: `engineering/DESIGN.md` > Diagrams. (This is what shipped touching labels on both architecture-page diagrams; every diagonal label in the set was affected, and none of the vertical ones were.)
- **Check clearance by measuring, not by looking.** A one-unit graze is invisible in a preview and obvious on the rendered page. Inline the SVG into a scratch HTML page alongside `custom.css`, then in the browser take each label's `getBBox()` and compute its distance to every `<line>`; anything under about 12 units needs moving. Do the same after any geometry change, and eyeball the result in both themes.
