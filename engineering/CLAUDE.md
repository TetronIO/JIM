...
It also catches **duplicated entries** within a subsection: identical entries hard-fail, near-identical entries (three quarters or more of their words in common) hard-fail, and two entries that open with the same eight words and then diverge warn. This is the `merge=union` failure mode made mechanical: when an entry is rewritten (typically tightened at release time) while another branch still carries the original, the union driver keeps both, and the pair reads as one feature announced twice. The word-overlap threshold is measured against the file's history: the closest pair of genuinely different entries scores 0.56, every re-add 0.63 or more, and the two that reached `main` 0.97 and 1.0. If either check fires, keep the version you want and delete the other; do not reword one to silence it. (The near-identical check was added after the original #1119 wording came back beside its #1547 rewrite and, opening the same way, only warned.)

And it catches the same failure mode **across a release**, which is the more damaging half: union merging re-adds lines rather than reconciling them, so a branch that merges or rebases across a release brings the entries it contributed back into `[Unreleased]`, where they read as unshipped work and would be announced a second time in the next release notes. An `[Unreleased]` entry identical or near-identical to one in a released section hard-fails, naming the version it shipped in; one that merely opens with the same eight words warns, because a genuine follow-up on shipped work looks the same at the start. Delete the re-add; never "fix" it by rewording. When this check was added it found seven such entries already sitting in `[Unreleased]`, the whole of 0.14.0's Security section bar one, resurrected by merges after that release; two of the seven had been reworded since shipping, and only the near-identical check sees those.

The same `changelog-lint` workflow also runs `scripts/Lint-DocsCoupling.ps1`, which enforces **public docs coupling**: a PR that adds a ✨ or 🔄 entry must also change a file under `docs/`, or opt out with a `Docs: n/a - <reason>` line in the PR body (a reason is required) or the `docs-not-needed` label. Only entries the PR *itself* adds count: the script diffs the `[Unreleased]` user-facing entries against the base branch (`BASE_SHA`), so a feature entry that another PR already merged does not tax every unrelated PR (a Dependabot bump, a CI tweak) opened before the next release. This is the mechanical backstop for the "Keeping Documentation Current" rule above; it covers public docs only, because engineering-doc staleness cannot be inferred from a changelog entry.
