...
<#
.SYNOPSIS
    Configure JIM for Scenario 013: Relative-Date OUTBOUND (Export) Scoping
...
.DESCRIPTION
    ...
    Topology:
      ...
      - File CSV Connected System (outbound target), a header-only CSV the File connector
        appends to (no directory-container flakiness, mirroring Scenario 012's philosophy).
...
.EXAMPLE
    ./Setup-Scenario-013.ps1 -JIMUrl "http://localhost:5200" -ApiKey "jim_abc123..."
#>
...
$hrCsvFilePath = "/connector-files/test-data/scenario-013-hr-users.csv"
$targetCsvFilePath = "/connector-files/test-data/scenario-013-target.csv"

Write-TestSection "Scenario 013 Setup: Relative-Date Outbound (Export) Scoping"
...
Write-TestSection "Scenario 013 Setup Complete"
