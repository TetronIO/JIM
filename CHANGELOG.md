...
- 🐛 A synchronisation expression that fails to evaluate is no longer silently swallowed. Previously, if an expression-based attribute flow mapping threw while evaluating (for example a function received an unexpected value type, or the expression failed to parse), the error was only logged and synchronisation continued as though the expression had produced no result, leaving stale data in the metaverse with no visible error. Now the affected object is errored and recorded against the run with a distinct "expression evaluation error" so administrators can see and fix it; the target attribute is left untouched (a thrown expression is never treated as a deliberate "clear the value"). This applies to both inbound (to metaverse) and outbound (export) expression mappings, and the rest of the run continues so one bad expression does not halt the whole synchronisation.
