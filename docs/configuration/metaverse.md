...
- **Current value**<br /> The value, its source (the Connected System and Synchronisation Rule it came from), the Connected System Object that supplied it, and the Activity that last set it. A source that positively asserts there is no value (see [Null is a value](../concepts/attribute-priority.md#null-is-a-value)) is shown as asserting no value.
