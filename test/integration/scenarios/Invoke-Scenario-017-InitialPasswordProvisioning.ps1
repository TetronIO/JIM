...
<#
.SYNOPSIS
    Test Scenario 017: Initial Password Provisioning
...
.EXAMPLE
    ./Invoke-Scenario-017-InitialPasswordProvisioning.ps1 -ApiKey "jim_..." -Template Micro
#>
...
if ($DirectoryConfig.UserObjectClass -ne "user") {
    throw "Scenario 017 requires Samba AD. 'Must change at next sign-in' has no portable equivalent on " +
...
Write-TestSection "Scenario 017: Initial Password Provisioning"
...
if (-not $config) {
    throw "Failed to set up Scenario 017 configuration"
}
...
try {
    ...
    if ($Step -in @("Provision", "All")) {
        ...
        # (read during Setup-Scenario-001's schema import) and refuses to save one that policy would reject
        ...
        Set-JIMSyncRuleInitialPassword -Id $config.ExportSyncRuleId -Source Static `
            ...
            -ChangeReason "Scenario 017: stage a password the target refuses, to prove parking before release" | Out-Null
        ...
        Set-JIMSyncRuleInitialPassword -Id $config.ExportSyncRuleId -Source Static `
            ...
            -ChangeReason "Scenario 017: correct the rule so the parked accounts release" | Out-Null
        ...
        # persisted baseline for a delta to compare against and the Connector refuses it outright. Scenario 001
...
Write-TestSection "Scenario 017 Summary"
