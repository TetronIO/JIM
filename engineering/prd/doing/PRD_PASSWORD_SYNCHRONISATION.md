...
Every criterion below is met. Each carries where it lives and what proves it, so the claim can be checked without reading the whole diff. "Scenario 020" is `test/integration/scenarios/Invoke-Scenario-020-PasswordSynchronisation.ps1`, run green against a live Samba AD domain controller (23 assertions).
...
- [x] A disabled Connected System accumulates events and drains them automatically on enable. **Evidence:** Requirement 2 and 3. This was **wrong until [#1522](https://github.com/TetronIO/JIM/issues/1522)** and is now guarded on both halves: `PasswordSynchronisationFanOutTests.QueuePasswordChange_ForAConfiguredButDisabledSystem_StillQueues` for accumulate, `ConnectedSystemPasswordSynchronisationDrainTests.PasswordSynchronisationTurnedOn_ReleasesParkedChanges` for drain, and Scenario 020 for both without intervention.
