...
public interface IConnectorImportUsingCalls
{
    ...
    /// <param name="progressCallback">Optional callback for narrating your internal sub-phases (i.e. "Querying root DSE...", "Enumerating containers in Contoso..."). JIM surfaces each message on the Activity, replacing the previous one, so operators can tell a healthy long-running page from a stuck one. JIM already reports page-level progress around this call. The vocabulary is yours; emit on phase transitions rather than per object, and skip building messages entirely when this is null.</param>
