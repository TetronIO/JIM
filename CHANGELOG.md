...
- 🐛 Accounts queued for deletion when an identity is deleted are now reported on the Activity of the run that queued them, instead of appearing only in service logs. A synchronisation run that deletes identities (or a background Metaverse Object Housekeeping batch that deletes them once their grace period expires) queues a delete for every account covered by an export Synchronisation Rule whose Deprovisioning Action is Delete; each of those now appears as its own Pending Export execution item, named after the identity being deprovisioned, linked to the target Connected System Object and Pending Export, and counted in the Activity's Pending Exports total, matching how membership-removal exports are already reported. Deprovisioning is the most consequential thing a run can stage, and it was previously invisible on the Activity. An execution item for a queued deletion also now says so plainly, with the Pending Export's change type shown alongside its status. (#1044)
