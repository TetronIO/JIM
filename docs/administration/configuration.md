...
| `History.InitialPasswordRetentionPeriod` | Initial password record retention period | History | The duration for which an [initial-password record](../concepts/passwords.md) that has reached a terminal state (parked for an administrator, or expired without a password having been set) is kept. Records still being worked are never removed, however old, and the Activity recording what happened to the account outlives this. Format: `d.hh:mm:ss`. | `90.00:00:00` (90 days) |
| `History.PasswordEventRetentionPeriod` | Password Synchronisation retention period | History | The duration for which [Password Synchronisation](../concepts/passwords.md) history is kept: the Activities recording what happened to each password change, and the queue rows that reached a terminal state (parked, expired, or cancelled). Changes still owed to a Connected System are never removed, however old. Shorten this to bound how long JIM holds an encrypted password it can no longer deliver. Format: `d.hh:mm:ss`. | `365.00:00:00` (~1 year) |
