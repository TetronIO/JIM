...
- ✨ The Activity list is now easier to audit: a category quick-filter isolates Configuration changes, Identity data, Sync runs, or System operations in one click, new initiator (user, API key, or system) and created date-range filters narrow the view, and the filter state is reflected in the URL so a filtered view can be bookmarked or shared.
