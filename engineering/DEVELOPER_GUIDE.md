...
**Import mode versus export mode**: an import mapping's assignment is keyed on the Metaverse Object and is exported to every participating Connected System; an export mapping's assignment is keyed on the **Connected System Object** and belongs to that one system alone, never touching the Metaverse. Export-mode resolution happens in `ExportEvaluationServer`/`ResolveExportGeneratedValuesAsync` against the Pending Export's staged change, and adopts from the value the Connected System Object already holds for the attribute (never checked on a `Create`, since a brand-new provisioning object holds nothing yet).

**Adopt before generate and stickiness** (decision 5, FR 30): before generating, the resolver checks whether the object already holds a value a participating target has accepted (`GeneratedValueParticipation.FindAdoptableValueAsync` for import mode; the Connected System Object's own current value for export mode). If so, the assignment is created directly in state `Committed` with `Adopted = true` and nothing is generated; JIM never renames a live account by ordinary generation. Once committed, a value is sticky: `GenerationOutcomeKind.Sticky` re-applies the existing assignment's value unconditionally on every later run, never recomputing it from its inputs.

**Start again** (`UniqueValueGenerationServer.RestartAsync`, `ConnectedSystemServer.RestartGeneratedValuesAsync`): moves the target attribute's counter back to the flow's configured start value and records an audited Activity. It touches **no existing value**: there is no recall, because #1537's recall stages removal exports that would strip the very connector-space values adopt-before-generate is built to keep. Release 1's `RestartAsync` always reports `RetiredValuesForgotten = 0`, since the retired values register does not exist until release 2 (Phase 6).
