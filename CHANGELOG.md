...
- 🖥️ The attribute value tables inside a page no longer page either. A multi-valued attribute on a Metaverse Object or a Connected System Object, the queued changes behind a Pending Export's multi-valued attribute, and the attribute changes on a causality event now scroll continuously like every top-level list, and state how many values they hold beside their search box. A group with half a million members is read by scrolling it, rather than by working through page controls ten values at a time. The causality attribute detail's own filter box has gone with them; its operation chips (All/Set/Add/Remove) now sit in the table's toolbar beside the shared search box, and its count reads "12 of 3,868" against the search rather than against the chips.
- 🖥️ The attribute changes behind a Change History entry now scroll continuously too, and state how many changes they hold. Opening an entry from a Metaverse Object's or Connected System Object's history shows its changes in the same table every other list uses, with its search box in the table's toolbar beside the **Attribute** and **Change type** filters rather than in a row above it; all three narrow the list as before. A change group with tens of thousands of membership changes in it is read by scrolling.
- 🖥️ The tables on an API Key, a Connected System Object and a Pending Export scroll too. An API Key's usage history, a Connected System Object's attributes and the Pending Export changes queued against it, and a Pending Export's Attribute Changes have lost their page controls and their rows-per-page setting; each states how many rows it holds beside its search box, and sorts across the whole table rather than the rows on screen wherever the underlying read can sort. A Connected System Object's attributes can now be searched, by attribute name or by value, which they could not be while they were a plain table. A multi-valued attribute is now one row like any other: it shows its first value and a **+n more** beside it, which opens every value in a dialog that scrolls the same way, rather than stacking up to ten values into the row (a scrolling table places each row without having drawn the rows above it, so a row that is taller than the rest puts the scroll position and everything below it out of step). An empty table says which of the two reasons applies, offering to clear a search that matched nothing, or naming what would put rows in it.
- 🖥️ Every row of a scrolling table is now one line tall, wherever the cell's content could previously grow past it, and nothing is lost to the change. A cell holding a list shows its first item with a **+n more** beside it, opening the whole set in a dialog drawn exactly as the row draws it: an API Key's roles, the Object Types a Metaverse Attribute is bound to, the sources of an Attribute Flow on the Data Flow and Synchronisation Rule views, and an Example Data Template Attribute's generation rules all follow the pattern a multi-valued attribute's values already did. Long free text (Connected System and Connector descriptions, imported schema descriptions, service log messages) is clipped with an ellipsis and readable in full on hover, with the log entry's own panel still holding the complete, selectable message. Secondary text that sat under a value (a Service Setting's description, an attribute's data type and plurality, the pattern behind a previewed change) now reads inline after it, low-lighted. A row taller than the rest put the scroll position, the row a link points at, and everything below it out of step, because a scrolling table places each row without having drawn the rows above it. See [Working with Lists](https://docs.junctional.io/administration/portal-lists/).
- 🖥️ The Operations queue scrolls too, and it is the last table in the portal to lose its page controls. It is now a table per block: each Schedule Execution's steps scroll within their own, bounded in height so a Schedule with twenty steps in it cannot push the rest of the queue off the screen, and everything running outside a Schedule scrolls in a table of its own at the end. Each states how many Worker Tasks it holds. Nothing else about the queue moves: the header above each Schedule still names it, draws its rail, collapses its steps away and carries **View execution** and **Cancel Schedule**, and each step still shows its own progress and can be cancelled on its own. The Schedules tab's rows are one line tall like every other scrolling table, so a Schedule's description now sits beside its name (in full on hover) and its last outcome beside the time it last ran, rather than under them where a fixed row height clipped them. See [Working with Lists](https://docs.junctional.io/administration/portal-lists/).
