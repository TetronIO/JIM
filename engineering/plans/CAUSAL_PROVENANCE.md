...
- The deletion-cascade and recall staging paths stage through `ExportEvaluationServer`, which is deliberately Activity-unaware, so their Pending Exports are persisted before the execution item reporting each one exists. Making export evaluation Activity-aware was rejected (export hot path; synchronisation integrity outranks a diagnostic); instead the stamp runs the other way round, as a set-once fix-up (`SetPendingExportQueueingItemsAsync`) where the reporting item is built: the nested case's deletion item, the standalone fallback's own item, the recall item, and housekeeping's equivalents. This matters doubly for deletes, whose `SourceMetaverseObjectId` is nulled by the deletion's SET NULL cascade: without the stamp a deprovisioned account had no cause at all.
- The read side then walks past any resolved synchronisation-side cause to the import that last changed its Connected System Object, as a derived source-import hop (PRD requirement 4's free per-object join; no edge, no capture, no snapshot). A create's chain ends at "imported into Yellowstone APAC as a new record"; a deprovision's at the source record's deletion. An import whose retention has lapsed simply yields no hop.
