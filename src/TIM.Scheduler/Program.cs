...
// **************************************************************************************
// Tetron Identity Manager - Scheduler Service
//
// Needs to:
...
using Serilog;
InitialiseLogging();
Log.Information("TIM.Scheduler - Hello World!");

try
{
    while (true)
    {
        Log.Information("TIM.Scheduler - Doing some work!");
        Thread.Sleep(2000);
    }
}
catch (Exception ex)
...
static void InitialiseLogging()
{
    ...
    loggerConfiguration.WriteTo.File(Path.Combine(loggingPath, "tim.scheduler..log"), rollingInterval: RollingInterval.Day);
