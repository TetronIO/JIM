...
- **Status:** Doing (Phases 1-6 complete except /Bulk; Phase 7 outstanding)
...
- **Discovery warnings are logged, not yet surfaced in the portal.** Reporting them on the schema-import screen belongs with Phase 7, which owns the setup flow; `ScimDiscoveryResult.Warnings` already carries them.
...
### Phase 6: Export ✅ (except `/Bulk`, deferred to Phase 7)
...
- **`/Bulk` is deferred to Phase 7** (agreed July 2026). It is a large surface for a throughput gain, and per-object requests are already correct: the retry, `Retry-After` and `RateLimit-*` handling from Phase 2 applies to every one of them. Nothing else depends on it, so it can land after enablement without reworking what is here.

Delivered as `ScimConnectorExport` in `JIM.Connectors/SCIM/`, plus `ScimResourceWriter`, `ScimPatchBuilder`, `ScimValueFormatter` and the PATCH message model in `JIM.Scim`. `ScimConnector` implements `IConnectorExportUsingCalls`. Two `ConnectedSystemExportErrorType` values joined: `MissingDependency` and `ConcurrencyConflict`.
...
**Runtime verification (2026-07-30).** The connector was driven end to end against a live HTTP service provider: `test/integration/scim/Start-ScimTestProvider.ps1` serves the discovery documents and a Users and Groups collection over `HttpListener`. JIM is air-gap deployable and carries no third-party service dependency, so its test provider is written here rather than pulled as a container image; the exhaustive misbehaviour coverage stays in the unit suite's `MockScimProvider`, which can be steered per test.
