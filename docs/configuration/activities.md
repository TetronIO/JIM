...
When a reconnection cancels a scheduled deletion (see [Grace period](metaverse.md#grace-period)), the rejoining record's **CSO Joined** event carries an **MVO Deletion Cancelled** child naming the system that rejoined, when the deletion had been due, and the Deletion Rule that permitted the cancellation, so an administrator can see why the Identity survived a deletion the Lineage shows was scheduled.
