...
<#
.SYNOPSIS
    Test Scenario 008: Cross-domain Entitlement Synchronisation
...
.EXAMPLE
    ./Invoke-Scenario-008-CrossDomainEntitlementSync.ps1 -Step All -Template Nano -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-008-CrossDomainEntitlementSync.ps1 -Step InitialSync -Template Small -ApiKey $env:JIM_API_KEY
#>
...
if (-not $isRfcDirectory) {
    # Scenario 008 places groups in OU=Entitlements, not the default OU=Groups
...
Write-TestSection "Scenario 008: Cross-domain Entitlement Synchronisation"
...
try {
    ...
    # Verify Scenario 008 containers are running
    Write-Host "Verifying Scenario 008 infrastructure..." -ForegroundColor Gray
    ...
    if (-not $SkipPopulate) {
        if ($isRfcDirectory) {
            ...
            & "$PSScriptRoot/../Populate-OpenLDAP-Scenario-008.ps1" -Template $Template -Instance Source -DirectoryType $DirectoryConfig.DirectoryType
            Write-Host "Creating OU structure in Target OpenLDAP..." -ForegroundColor Gray
            & "$PSScriptRoot/../Populate-OpenLDAP-Scenario-008.ps1" -Template $Template -Instance Target -DirectoryType $DirectoryConfig.DirectoryType
        }
        else {
    ...
    # Run Setup-Scenario-008 to configure JIM
    Write-Host "Running Scenario 008 setup..." -ForegroundColor Gray
    & "$PSScriptRoot/../Setup-Scenario-008.ps1" -JIMUrl $JIMUrl -ApiKey $ApiKey -Template $Template -ExportConcurrency $ExportConcurrency -MaxExportParallelism $MaxExportParallelism -DirectoryConfig $DirectoryConfig

    Write-Host "JIM configured for Scenario 008" -ForegroundColor Green
    ...
    if (-not $sourceSystem -or -not $targetSystem) {
        throw "Connected systems not found. Ensure Setup-Scenario-008.ps1 completed successfully."
    }
    ...
    if ($Step -eq "NewGroup" -or $Step -eq "All") {
        ...
        $newGroupDescription = "Test group created by Scenario 008 NewGroup test"
    ...
    if ($Step -eq "LeaverCohort" -or $Step -eq "All") {
        ...
        if (-not $isRfcDirectory) {
            ...
            # manually and wait for a terminal state (same pattern as Scenarios 012/13).
    ...
    $testResults.Success = $true
    Write-Host ""
    Write-Host "✓ Scenario 008 test execution completed successfully" -ForegroundColor Green
    exit 0

}
catch {
    Write-Host ""
    Write-Failure "Scenario 008 test failed: $($_.Exception.Message)"
    Write-Host ""
