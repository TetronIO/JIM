...
4. **Fidelity mirrors recorded behaviour, not intent.** The preview's outcome tree must match what a real synchronisation actually records, never what the builder's comments say it should. This has already caught one divergence: the real builder attached export outcomes to the RPEI's root instead of to the Attribute Flow child it documented, because its child lookup keyed on `ParentSyncOutcomeId`, which only resolves at bulk-insert flattening (#1428). The preview deliberately mirrored the flattened shape until the builder itself was fixed, at which point the mirror in `SyncPreviewServer` moved with it in the same change. Keep that discipline: mirror first, and where mirroring means encoding something that looks wrong, file it and say so at the mirror site rather than quietly implementing the intent.
