...
2. **Step setting becomes three-way.** Each step's setting is **Follow the Schedule** (default for new steps), **Stop the Schedule** or **Continue the Schedule**.
   ...
   - Clarification (implementation planning): the write mapping above applies to **new** steps only. Because `continueOnFailure` reads as the effective behaviour, a get-modify-put client (including `Add-JIMScheduleStep`, which resends every existing step) would otherwise pin every step of a continuing Schedule to `Continue`. So, when `onFailure` is absent on an **existing** step, a `continueOnFailure` equal to the step's current effective behaviour leaves its setting unchanged; one that differs maps `true` to `Continue` and `false` to `Stop`, because the client asked for the opposite of what the step does now. The current effective behaviour is judged against the Schedule's stored setting from before the update.
...
6. **"Complete With Error" execution status.** A new `ScheduleExecutionStatus` member, `CompleteWithError`, displayed as "Complete With Error" in the Tertiary colour to match an Activity's Complete With Error. It applies to an execution that reached its end with at least one step that failed and was allowed to continue.
   ...
   - One stated exception (implementation planning): the Temporal Scope Reconciliation watermark (the last completed execution it reconciles from) moves forward only on a clean Complete. A run that is Complete With Error does not advance it, so a window that a failed step missed is covered again on the next run.
