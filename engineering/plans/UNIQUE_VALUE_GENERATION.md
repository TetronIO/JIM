...
- `TryRemediateAsync`, `IsAnchoredAsync`, `CommitAsync`, `EnterNeedsDecisionAsync`, `AuthoriseRenameAsync`, `RetryAsync`, `ReleaseNeedsDecisionForMappingAsync`, `ListNeedsDecisionAsync` (release 4); `GetSequenceStateAsync`, `GetAssignmentsForObjectAsync` (release 1). No regenerate action: deferred to #614 with Set Value.
...
4. Review-flag integration: the #892 flag's processing also resolves generation for the object (no derived pass yet); "Generate a new value" is deferred to #614 and would set the same flag.
