...
foreach ($imageName in $imagesToBuild) {
    ...
    # Verify the certificate actually carries the names JIM connects by before committing, so a
    # build that bakes a wrong certificate fails here rather than at scenario time. Samba
    # autogenerates its own FQDN-only certificate before post-provision.sh runs; an existence
    # guard in that script once kept the autogenerated one, and every image built was unusable
    # (Add-SambaCertificateToJimStore rejected it on every run, with a message advising a
    # rebuild that could never help).
    $requiredSans = @("$($config.Hostname).$($config.Domain.ToLower())", $config.ServiceName)
    $sanOutput = (docker exec $containerName openssl x509 -in /usr/local/samba/private/tls/cert.pem -noout -ext subjectAltName 2>&1) -join "`n"
    if ($LASTEXITCODE -ne 0) {
        Write-Host "ERROR: Could not read the TLS certificate's SAN list: $sanOutput" -ForegroundColor Red
        docker rm -f $containerName | Out-Null
        exit 1
    }
    foreach ($requiredSan in $requiredSans) {
        if ($sanOutput -notmatch [regex]::Escape($requiredSan)) {
            Write-Host "ERROR: TLS certificate is missing required SAN '$requiredSan'. SANs found: $sanOutput" -ForegroundColor Red
            docker rm -f $containerName | Out-Null
            exit 1
        }
    }
    Write-Host "  Certificate SANs verified: $($requiredSans -join ', ')" -ForegroundColor Green
