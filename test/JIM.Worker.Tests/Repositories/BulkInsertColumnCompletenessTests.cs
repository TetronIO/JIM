...
public class BulkInsertColumnCompletenessTests
{
    ...
    [Test]
    public void PendingPasswordChangeBulkColumns_MatchMappedColumnsExactly()
    {
        AssertColumnListMatchesModel(typeof(JIM.Models.Transactional.PendingPasswordChange), "PendingPasswordChanges", PendingPasswordChangeBulkColumns.PendingPasswordChanges);
    }

    [Test]
    public void PendingPasswordChangeUpdateColumns_HaveAConsciousHomeForEveryColumn()
    {
        AssertUpdateListsCoverInsertList(
            "PendingPasswordChanges",
            PendingPasswordChangeBulkColumns.PendingPasswordChanges,
            [PendingPasswordChangeBulkColumns.PendingPasswordChangesSupersedeUpdate, PendingPasswordChangeBulkColumns.PendingPasswordChangesAttemptUpdate],
    ...
    {
        AssertColumnListMatchesModel(typeof(JIM.Models.Operations.ServiceHeartbeat), "ServiceHeartbeats", ServiceHeartbeatBulkColumns.ServiceHeartbeats);
    }

    [Test]
    public void ServiceHeartbeatUpdateColumns_HaveAConsciousHomeForEveryColumn()
    {
        AssertUpdateListsCoverInsertList(
            "ServiceHeartbeats",
            ServiceHeartbeatBulkColumns.ServiceHeartbeats,
            [ServiceHeartbeatBulkColumns.ServiceHeartbeatsUpdate],
            ServiceHeartbeatBulkColumns.ServiceHeartbeatsUpdateExclusions);
    }
    ...
    private static bool IsDatabaseAssignedIdentity(IProperty property) =>
    ...
    private void AssertColumnListMatchesModel(Type entityClrType, string tableName, string[] bulkInsertColumns)
    {
        ...
        var mappedColumns = entityType!.GetProperties()
            ...
            // Nor may identity columns (an int Id the database numbers): supplying one would step outside the
            // sequence and collide with a later default insert. Only a genuine database strategy is skipped; a
            // Guid key is also ValueGenerated.OnAdd but is generated by EF on the client, so a raw insert must
            // still supply it and it stays in the required set.
            .Where(p => !IsDatabaseAssignedIdentity(p))
