...
- ✨ Password Synchronisation history is now kept under its own retention period, `History.PasswordEventRetentionPeriod`, defaulting to a year. It governs two things together: the Activities recording what happened to each password change, and the queued changes that finished (parked, expired or cancelled). Its own class rather than the general history period because the question these answer, "was this person's password ever set in that system, and if not, why?", is asked long after the synchronisation history around them stops being interesting. It is also what bounds how long JIM holds a password it can no longer deliver: a parked or cancelled change still carries its encrypted password, so shortening the period is how you shorten that. Changes still owed to a Connected System are never removed, however old. See [How long any of it is kept](https://docs.junctional.io/concepts/passwords/#-how-long-any-of-it-is-kept). (#1119)
...
- 🔒 The PowerShell module no longer writes a password to its own debug output either. `Invoke-JIMApi` logged the outgoing request body verbatim under `-Debug`, so the four cmdlets that set a password printed the value they had deliberately taken as a `SecureString` to keep it out of the session history. Bodies are now rendered with credential-shaped values replaced, by property name and recursively, using a fixed marker so not even the length survives. Connected System setting values go too: a service account's password travels in exactly the shape a hostname does, so nothing in the payload tells them apart, and losing a base DN from a debug line costs less than leaking the credential beside it. The setting identifiers and non-string values still show. (#1119)
