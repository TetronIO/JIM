...
public class SyncRepositoryCsoReadTests
{
    ...
    #region Run-scoped secondary external Id lookup (#1079 regression A)

    [Test]
    public async Task GetSecondaryExternalIdLookupAsync_ReturnsCaseInsensitivePairsAsync()
    {
        var cso = CreateCso(secondaryExternalIdAttributeId: SecondaryAttrId);
        cso.AttributeValues.Add(CreateAv(SecondaryAttrId, stringValue: "CN=Alice,DC=test"));
        _repo.SeedConnectedSystemObject(cso);

        var lookup = await _repo.GetSecondaryExternalIdLookupAsync(CsId);

        Assert.That(lookup, Has.Count.EqualTo(1));
        Assert.That(lookup["cn=alice,dc=test"], Is.EqualTo(cso.Id));
    }

    [Test]
    public async Task GetSecondaryExternalIdLookupAsync_DuplicateValue_KeepsLowestIdCsoAsync()
    {
        // Regression A: at scale this lookup covers the whole Connected System in one pass, so
        // duplicates are reported as a single aggregate warning rather than logged per-value -
        // what matters for correctness here is that exactly one entry survives, deterministically.
        var lowerId = Guid.Parse("00000000-0000-0000-0000-000000000001");
        var higherId = Guid.Parse("00000000-0000-0000-0000-000000000002");
        var csoLow = CreateCso(id: lowerId, secondaryExternalIdAttributeId: SecondaryAttrId);
        csoLow.AttributeValues.Add(CreateAv(SecondaryAttrId, stringValue: "CN=Duplicate,DC=test"));
        var csoHigh = CreateCso(id: higherId, secondaryExternalIdAttributeId: SecondaryAttrId);
        csoHigh.AttributeValues.Add(CreateAv(SecondaryAttrId, stringValue: "CN=Duplicate,DC=test"));
        // Seed in reverse Id order so "keep the first" can only pass if it is Id-ordered, not
        // insertion-ordered.
        _repo.SeedConnectedSystemObject(csoHigh);
        _repo.SeedConnectedSystemObject(csoLow);

        var lookup = await _repo.GetSecondaryExternalIdLookupAsync(CsId);

        Assert.That(lookup, Has.Count.EqualTo(1));
        Assert.That(lookup["CN=Duplicate,DC=test"], Is.EqualTo(lowerId));
    }

    [Test]
    public async Task GetSecondaryExternalIdLookupAsync_NullStringValue_ExcludedAsync()
    {
        var cso = CreateCso(secondaryExternalIdAttributeId: SecondaryAttrId);
        cso.AttributeValues.Add(CreateAv(SecondaryAttrId, stringValue: null));
        _repo.SeedConnectedSystemObject(cso);

        var lookup = await _repo.GetSecondaryExternalIdLookupAsync(CsId);

        Assert.That(lookup.Values, Does.Not.Contain(cso.Id));
    }

    [Test]
    public async Task GetSecondaryExternalIdLookupAsync_OtherConnectedSystem_ExcludedAsync()
    {
        var cso = CreateCso(connectedSystemId: 2, secondaryExternalIdAttributeId: SecondaryAttrId);
        cso.AttributeValues.Add(CreateAv(SecondaryAttrId, stringValue: "CN=Other,DC=test"));
        _repo.SeedConnectedSystemObject(cso);
