...
<#
.SYNOPSIS
    Test Scenario 007: Clear Connected System Objects
...
.EXAMPLE
    ./Invoke-Scenario-007-ClearConnectedSystemObjects.ps1 -Step All -Template Nano -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-007-ClearConnectedSystemObjects.ps1 -Step DeleteHistory -Template Nano -ApiKey $env:JIM_API_KEY
#>
...
# Hard-fail: the long-tail templates are Scenario 008 only.
...
Write-TestSection "Scenario 007: Clear Connected System Objects"
...
try {
    ...
    # Seed CSV test data before Setup-Scenario-001.ps1 runs — its schema-import step
    ...
    Write-Host "  ✓ JIM configured for Scenario 007" -ForegroundColor Green
    ...
    if ($Step -eq "EdgeCases" -or $Step -eq "All") {
        ...
        # Get LDAP system's current state (it has CSOs from Scenario 001 setup if any were provisioned)
    ...
    if ($Step -eq "StrandedSweep" -or $Step -eq "All") {
        ...
        # Setup-Scenario-001, never would - see PRD Scenario 3). Keep its existing 7-day grace period.
        ...
        # setup configured, whatever that is (Setup-Scenario-001 has changed its baseline before, #1614).
        ...
        # size (Scenario 007 runs at the sweep's chosen template; the All-scenarios run gives it Small).
        ...
        # IMPORTS into these Metaverse Objects. Setup-Scenario-001 also creates export
        ...
        # provisioned CSO in both target systems). Scenario 007 never clears those systems, so
    ...
    Write-Host "`n✗ Scenario 007 FAILED" -ForegroundColor Red
