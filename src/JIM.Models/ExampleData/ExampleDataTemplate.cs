...
[Index(nameof(Name))]
public class ExampleDataTemplate : IAuditable
{
    ...
    public List<ExampleDataObjectType> ObjectTypes { get; } = new();

    public void Validate()
    {
        if (string.IsNullOrEmpty(Name))
            throw new ExampleDataTemplateException($"Null or empty {nameof(Name)}");

        if (ObjectTypes == null || ObjectTypes.Count == 0)
            throw new ExampleDataTemplateException("Null or empty ObjectTypes");
