...
<#
.SYNOPSIS
    Setup for Scenario 22: OpenLDAP Password Policy

.DESCRIPTION
    Configures JIM to provision accounts into the Yellowstone OpenLDAP suffix as a NON-ROOT account and
    to set an Initial Password derived from the password policy it discovers there, so that Scenario 22
    can prove the policy is read correctly and that generated passwords satisfy it (#1702).

    The provisioning substrate is Scenario 1's: HR CSV source, OpenLDAP target, an Export
    Synchronisation Rule that provisions Users. This script composes Setup-Scenario1.ps1 rather than
    rebuilding it, the same way Scenario 17 does, with one change to the directory configuration it is
    handed: the Connected System binds as cn=jim-provisioner,dc=yellowstone,dc=local (created by
    Populate-OpenLDAP-Scenario22.ps1), not as the rootdn. OpenLDAP exempts the rootdn from the ppolicy
    ...
    OpenLDAP only. Setup-Scenario17.ps1 refuses OpenLDAP for its own reasons; this script refuses Samba
...
.PARAMETER ProvisionerBindDN
    The non-root account JIM binds as. Must match what Populate-OpenLDAP-Scenario22.ps1 created.

.PARAMETER ProvisionerBindPassword
    That account's password. Must match what Populate-OpenLDAP-Scenario22.ps1 set.

.EXAMPLE
    ./Setup-Scenario22.ps1 -ApiKey "jim_..." -Template Micro
#>
...
Write-TestSection "Scenario 22 Setup: OpenLDAP Password Policy"
...
Write-TestStep "Step 2" "Running Setup-Scenario1.ps1 for the provisioning substrate"
...
try {
    ...
    #
    # Setup-Scenario1 warns and carries on when the hierarchy import or container selection fails, and the
    ...
    # Select the Partition, importing the hierarchy first where nothing was discovered (Setup-Scenario17's
    ...
    if (-not $selectedPartition) {
        ...
        if (-not $partition) {
            throw "The hierarchy import discovered no Partition matching '$($DirectoryConfig.BaseDN)' on " +
                  ...
                  "Populate-OpenLDAP-Scenario22.ps1 granted that account access to the suffix."
        }
    ...
    # enabled state for the Connector to write, and Scenario 1's OpenLDAP flows do not export one either.
...
# Summary
Write-TestSection "Scenario 22 Setup Complete"
...
# Return Scenario 1's configuration plus what the scenario's assertions need.
