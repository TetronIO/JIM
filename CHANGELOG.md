...
- ✨ An **Export** execution item now says what caused it. Previously it showed no **Caused by** at all: an export run holds only a queue of changes to make, and the synchronisation that decided on a change ran in a different Activity, often days earlier, with nothing linking the two. JIM now records the synchronisation that stages each change onto the change itself, and the export run turns that into a cause as it runs, reading "A synchronisation of Project-AgileCore staged this change, and this run exported it" and linking to the run that did so, whose own causes follow above it. This is the one link that could not be recovered after the fact, because the queued change is deleted the moment its export succeeds. Deprovisioning exports and drift corrections are covered the same way, the latter naming the drift correction that staged them. Where a change was staged by a path with no execution item to name (a deletion cascade, or grace-period housekeeping), the cause names the Identity behind the export instead and the chain ends there rather than walking on. Changes already queued before upgrading carry no cause, since it was never captured. (#1223)
