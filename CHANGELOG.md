...
- ✨ Synchronisation Rules can now carry an optional description, so administrators can record what a rule is for. Set it in the admin portal (the field grows to fit the text), with `New-JIMSyncRule`/`Set-JIMSyncRule`, or via the REST API; changes to it appear in the change history.
- ✨ Date/time criteria can now be relative to "now", not just fixed dates, on both Synchronisation Rule scope filters and object searches: pick a count, a unit (Hours, Days, Weeks, Months or Years) and a direction (ago or from now), for example "terminated between 30 and 364 days ago" or "expiring within the next 7 days". Relative criteria are re-evaluated on every run so the scope keeps moving with time; days and coarser units round to midnight UTC while Hours stays exact, and month/year offsets are calendar-correct. Configurable in the Scope and Predefined Search editors, the PowerShell module, and the REST API, which also gains a scoping criterion update endpoint.
- ✨ Relative-date scope filters now keep working for objects whose source data isn't changing: a new built-in Temporal Scope Reconciliation schedule periodically re-evaluates time-driven scope transitions, so a leaver is deprovisioned when their end date passes and a joiner is provisioned downstream when their start date arrives. It runs hourly by default and can be re-timed, enabled or disabled, but not renamed, deleted, or have its steps changed.
- ✨ Predefined Searches can now filter on any attribute type, not just text: add criteria on Number, Long Number, Date/Time, Boolean and GUID attributes with the operators that suit each type (the full set of ordering comparisons for numbers and dates, with calendar wording such as "on or before" for dates), and choose case-sensitive or case-insensitive text matching. Manage criteria from a new editor on the Predefined Search page, the PowerShell module (`New-/Set-/Remove-JIMPredefinedSearchCriterion` and the criteria-group cmdlets), and the REST API.
...
- ✨ An API Key's Name and Description can now be edited directly from its Details tab in the admin portal, rather than needing PowerShell or the REST API.
- ✨ An Activity's Target link for a Service Setting change now takes you straight to that setting on the Service Settings page, with a matching search already applied, instead of just the settings list.

#### Configuration Change History (#14)

- ✨ JIM now tracks a versioned history of changes to your Synchronisation Rules, Connected Systems, and Schedules: who changed what, and when. The admin portal presents each change as an audit-style field history, showing each field's value before and after the change (for Schedules, via a History tab in the Schedule editor), and you can retrieve the same change via `Get-JIMConfigurationChangeHistory` or the REST API; secrets (including a Schedule step's SQL connection string) are flagged as changed but never stored or revealed.
- ✨ You can now record a reason when changing configuration from automation: `-ChangeReason` on the Synchronisation Rule, Connected System, and Schedule write cmdlets, or an optional field on the REST write requests. The reason shows with the change and on its Activity.
- ✨ Deleting a Connected System now records a final snapshot of its configuration in the change history, so a decommissioned system's last-known state and who removed it stay auditable, with an optional reason via the delete dialog, `Remove-JIMConnectedSystem -ChangeReason`, or the REST API.
- 🐛 A deleted configuration object's captured final state is now shown on its delete Activity, rendered as a clearly-marked removal; previously the snapshot was recorded but the Activity page showed no detail. Applies to every configuration type that records a deletion (Connected Systems, Synchronisation Rules, Schedules, and the rest).
- ✨ Saving a Synchronisation Rule, Connected System, or Schedule in the admin portal now offers an optional "Reason for change" prompt, and the reason is recorded with the change in the configuration change history, matching what automation callers could already do. Leaving it blank saves as before; cancelling the prompt abandons the save.
- ✨ Service Settings now carry the same versioned change history: every update or revert records who changed the setting, when, and the value before and after, viewable from a history button on each setting's row and retrievable via `Get-JIMConfigurationChangeHistory` or the REST API. The "Reason for change" prompt appears on setting saves and reverts too, and encrypted setting values are flagged as changed but never stored or revealed. The built-in settings' creation is recorded from first startup, and a later change to an environment-sourced setting (for example repointing the SSO authority or rotating its secret in the deployment's configuration) is recorded on the next startup.
- ✨ Metaverse schema changes now carry the same versioned change history: creating an Object Type or changing its deletion rules, and creating, updating, or deleting an Attribute (including its Object Type associations) all record who changed what and when. View an Object Type's history from a new Changes tab on its detail page (now organised into Details, Attributes, and Changes tabs) and an Attribute's from a history button on its row, or retrieve either via `Get-JIMConfigurationChangeHistory` or the REST API; `-ChangeReason` is available on the Metaverse write cmdlets and the "Reason for change" prompt appears when saving deletion rules. The built-in Object Types and Attributes' creation is recorded from first startup.
- ✨ Trusted Certificates now carry the same versioned change history: adding, editing, enabling, disabling, or deleting a certificate records who changed what and when, with the "Reason for change" prompt on every certificate action in the admin portal and `-ChangeReason` on the certificate cmdlets. Only certificate metadata is recorded; the certificate material itself is never stored in the history.
- ✨ API Keys now carry the same versioned change history: creating, editing, enabling, disabling, or deleting a key records who changed what and when, viewable from a new Changes tab on the key's detail page (now organised into Details, Activity, and Changes tabs), with the "Reason for change" prompt on every key action in the admin portal and `-ChangeReason` on the API key cmdlets. Only key metadata and Role assignments are recorded; the key secret is never captured in the history in any form, not even as a hash.
- ✨ Roles now carry versioned change history covering the definition and every membership change with who, when, and an optional reason, viewable from the admin portal Activity list, the REST API, or `Get-JIMConfigurationChangeHistory -Type Role` and `-ChangeReason` on the `Add-JIMRoleMember`/`Remove-JIMRoleMember` cmdlets. The built-in Administrator Role's creation is recorded from first startup.
- ✨ Predefined Searches now carry the same versioned change history: enabling or disabling a search, and every criteria group or criterion change, records who changed what and when, viewable from a new Changes tab on the search's detail page, with the "Reason for change" prompt on save and `-ChangeReason` on the `Set-JIMPredefinedSearch` and criteria cmdlets, or retrievable via `Get-JIMConfigurationChangeHistory -Type PredefinedSearch`. The built-in searches' creation is recorded from first startup.
- ✨ Connector Definitions now carry the same versioned change history: the built-in connectors' capabilities, setting definitions, and files are recorded from first startup, and any later change (for example new capabilities or settings shipped in an updated connector) is captured with who changed what and when, viewable from a new Changes tab on the connector's detail page or retrievable via `Get-JIMConfigurationChangeHistory -Type ConnectorDefinition` and the REST API. A connector's file contents are fingerprinted by hash, never stored in the history.
- ✨ Example Data Sets and Example Data Templates now carry the same versioned change history: the built-in sets and template are recorded from first startup, and creating, editing, or deleting an Example Data Set records who changed what and when, viewable from a new Changes tab on each detail page, with a reason available via `-ChangeReason` and the REST API, or retrievable with `Get-JIMConfigurationChangeHistory -Type ExampleDataSet` and `-Type ExampleDataTemplate`. A data set's history records only its metadata and value count, never the individual values.
- 🔄 Data-generation runs are now a distinct "Data Generation" activity type, separated from Example Data Template configuration changes, so the Activities Configuration filter shows template edits without being cluttered by generation runs. Existing generation-run activities are reclassified automatically on upgrade.
- ✨ Configuration change history is now retained on its own schedule, separate from the general history retention: a new Configuration change retention period Service Setting (default ~10 years) governs it, and the routine history cleanup no longer removes configuration changes.
...
- ✨ Attribute Priority is now manageable in the admin portal: a Metaverse Object Type's Attributes tab shows each attribute's inbound contributor count, and expanding a multi-contributor attribute lets you drag its Synchronisation Rules into priority order and toggle "Null is a value" per contributor.
...
- 🔄 A factory reset now preserves the configuration change history provenance of the built-in objects it keeps: their version-1 baselines are re-recorded under a fresh System Initialisation Activity, so resetting no longer strips the built-in Metaverse schema, Predefined Searches, Connector Definitions, Example Data, Roles, and Service Settings of their factory origin in the audit trail.
- ✨ Synchronisation Activities now report when an attribute became blank with nothing contributing a replacement, as a distinct "MVO No Contributor" outcome alongside the existing "MVO Null Asserted", so administrators can tell a deliberate clear from one caused by every source falling away.
- ✨ Retrieving a Metaverse Object via the REST API or `Get-JIMMetaverseObject` now shows each attribute value's provenance: the Connected System and the exact Synchronisation Rule that won attribute priority resolution and contributed the value. Asserted nulls are also visible as flagged, value-less rows with provenance, so automation can tell a deliberate blank from an attribute that simply has no contributor. (#931)
...
- 🐛 Adding a Trusted Certificate through the REST API (`POST /certificates/upload` and `/certificates/file`) or `Add-JIMCertificate` no longer returns an error on success. The endpoints generated their "created" response in a way that failed under API versioning, surfacing as a "No route matches the supplied values" error even though the certificate was stored. Listing certificates when the store is empty via `Get-JIMCertificate` also no longer emits the pagination envelope as though it were a certificate.
...
- 🐛 When the winning source for an attribute departs, the next-priority contributor now takes over reliably in large deployments; previously the hand-over could silently fail and clear the object's attributes instead, including its join keys.
- 🐛 Re-keying an identity in a source system (for example a case-only or formatting correction to a matching attribute, so a new record re-matches the same identity while the old one is removed) no longer fails a Full Synchronisation with a database constraint violation. The obsolete connector's join is now released before the replacing connector claims it, and two brand-new source records that both match one identity in the same run now fail cleanly on the second with a join-conflict error instead of aborting the run.
- 🐛 A Full Synchronisation run after a configuration change (attribute priority reordering, "Null is a value", enabling or disabling a Synchronisation Rule, scoping changes) now applies the new configuration to every object, as documented. Previously objects whose source data had not changed were skipped by a performance optimisation, so a pure configuration change never took effect for them.
- 🐛 Editing an API Key or a Trusted Certificate now records who made the change and when in the object's audit details; previously the "last updated" attribution was silently lost on save.
...
- 🐛 A synchronisation run that both created a new Metaverse Object and detected drift on the same object (state enforcement correcting an unauthorised change in the connected system) no longer fails with a database foreign-key violation. Drift is now evaluated after new Metaverse Objects are saved, so the corrective export always references a real object.
- 🐛 A Full or Delta Synchronisation no longer aborts with an unhandled database concurrency error when it updates a Metaverse Object that was created earlier in the same run, a race that surfaced at scale (large, multi-source populations). Metaverse Object updates now persist by identity via the same direct-SQL path as creates, removing the optimistic-concurrency check that could spuriously fail on the synchronisation write path. If a page of changes ever does fail to persist, the run now reports which page and objects were affected instead of a generic unhandled error.
- 🐛 Adding an Attribute Flow to a Synchronisation Rule no longer fails with an internal server error when the target Metaverse attribute already has contributions from other Synchronisation Rules; the automatic priority renumbering across the sibling flows now saves correctly.
- 🐛 Deleting a Metaverse Object (for example a leaver deprovisioned by a deletion rule) now stages membership-removal exports for every object that referenced it, so groups in target systems without referential integrity no longer keep the deleted user as a member forever. Removal values are resolved at staging time, because a deleted object can no longer be resolved when the export executes, and a group losing many members in one run receives a single export carrying all the removals.
- 🐛 Deleting a Connected System Object that other objects still reference (for example a deprovisioned user still listed in a group's member values before the membership removals are confirmed) no longer fails the whole synchronisation run with a database foreign-key violation; the stale references are cleared as part of the deletion, with the raw reference strings preserved so the next confirming import reconciles normally.
- 🐛 A synchronisation run that fails while saving to the database can no longer leave its Activity stuck in progress: the failure is now recorded via a fresh database session, since the failing session cannot save anything further (including the failure record itself).
...
- 🐛 Updating a Metaverse Object Type (for example changing its deletion rules) is now recorded in the immutable audit log (Activities), attributed to whoever made the change. Previously the update left no audit trail at all.
...
- 🐛 Saving a Connected System or Synchronisation Rule without actually changing anything no longer records a duplicate configuration version; a change entry now always represents a real difference. The unchanged-save detection now compares snapshots semantically rather than textually, so database storage normalisation can no longer defeat it, and the Connected System's runtime status is excluded from the snapshot so deletion attempts cannot record phantom configuration changes.
- 🐛 Saving a Schedule without actually changing anything no longer records a duplicate configuration version, matching the unchanged-save behaviour of Connected Systems and Synchronisation Rules; a Schedule change entry now always represents a real difference.
- 🐛 The save button on a Connected System's Details and Settings tabs, and in the Run Profile edit dialog, no longer starts disabled when editing an existing item whose required fields are already filled in; previously it only became enabled after clicking in and out of a field.
- 🐛 Save and create buttons across the admin portal now react as you type instead of waiting for the field to lose focus: typing a name enables the Create Synchronisation Rule button immediately, clearing a Connected System's or Run Profile's name disables its save button immediately, and the same applies to certificate and API key dialogs, connector settings, the Metaverse Object Type grace period, and Attribute Flow expressions.
- 🐛 The Service Setting edit dialog no longer allows saving an unparseable duration into a time-period setting (such as a retention period); the value is validated as you type and the Save button is disabled until the duration is valid.
- 🐛 Simple Mode Object Matching Rule changes (rules attached to a Connected System Object Type rather than a Synchronisation Rule) are now captured in the owning Connected System's configuration history, and object-type matching rules now appear in the system's snapshot; previously these changes recorded no history at all. The rule's Activity now also links to the change and to the owning object's Matching tab.
- 🐛 Reordering Metaverse attribute priority now records a versioned configuration change on every affected Synchronisation Rule (linked to the reorder Activity), and each Attribute Flow's priority and "Null is a value" setting now appear in the captured configuration, so a priority change shows exactly what moved.
- 🐛 A configuration change Activity that carries no snapshot (recorded before change tracking was introduced, while it was disabled, or for a no-change save) no longer renders an unexplained blank; the Activity detail page now says why nothing can be shown.
- 🐛 Activity targets now link to the surface where their subject is actually managed: an Attribute Flow mapping change deep-links to the Synchronisation Rule's Attribute Flow tab, schema and hierarchy imports to the Connected System's Schema and Partitions & Containers tabs, Run Profile activities to the Run Profiles tab, and Schedule, Service Setting, Metaverse Object Type, and Metaverse attribute activities now link to their pages (the attribute link previously pointed at a page that does not exist).
...
- 🐛 Imports no longer record an empty Connected System update Activity every cycle when persisting the connector's watermark (for example an LDAP sync cookie). The watermark is machine-generated runtime state, not a configuration change, and each import is already audited by its Run Profile Execution Activity.
...
- 🐛 A multi-source Metaverse attribute contributed by a source that falls out of its Synchronisation Rule's scope is now handed to the next-priority Connected System still contributing it, matching the existing hand-over behaviour for a disconnected source, instead of being cleared; downstream Connected Systems now receive the handed-over value (or the clear, when no contributor survives) as a Pending Export in the same run, for scope exit and source disconnection alike. A configured deletion grace period no longer freezes recall wholesale at scope exit either: a re-elected attribute is still handed over, and only an attribute with no surviving contributor is frozen for the grace window.
- 🐛 The `-ConnectedSystemAttributeName` parameter on `New-JIMScopingCriterion` and `Set-JIMScopingCriterion` now resolves the attribute correctly. It previously queried an endpoint that does not exist, so any Synchronisation Rule scoping criterion specified by Connected System attribute name failed; the id-based `-ConnectedSystemAttributeId` parameter was unaffected. (#930)
...
[Unreleased]: https://github.com/TetronIO/JIM/compare/v0.12.0...HEAD
