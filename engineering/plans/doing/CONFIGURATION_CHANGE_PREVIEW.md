...
- [ ] **Wave 2:** G4 partition/container deselection ([#1251](https://github.com/TetronIO/JIM/issues/1251)). Scope-change behaviour audited before design; see [Partition and Container scope changes: what happens today](#partition-and-container-scope-changes-what-happens-today) below.
    ...
    - [x] **G4 delivered** across all three surfaces: `ConnectedSystemScopeSelectionPreviewAdapter`, the panel embedded in the Partitions & Containers tab, `POST connected-systems/{id}/scope-selection/preview`, and `New-JIMConfigurationChangePreview -ConnectedSystemId`. Four decisions worth recording:
        ...
        - **Containment is asked of the Connector** through the new `IConnectorContainment`, sharing one predicate with the export scope guard, so a preview counts exactly what export would refuse to write. The predicate was upgraded to compare Distinguished Names component by component in the process, fixing both directions of the character-level match it replaced (`OU=UsersArchive` read as inside `OU=Users`; `CN=a, OU=Users, DC=x` read as outside it).
