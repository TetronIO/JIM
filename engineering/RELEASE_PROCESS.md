...
| Version | Date | Notes |
|---------|------|-------|
| `0.10.1` | 2026-04-27 | Interactive PowerShell SSO support for identity providers that require a separate public client registration (Keycloak), correct static member counts on role and metaverse-object-role API endpoints and PowerShell cmdlets, correct connected system object count on the connected-system detail endpoint, sidebar UX polish (user menu pinned to drawer bottom, dedicated pin/collapse chevron in the header, refined active/hover pill styling), and a Microsoft.AspNetCore.DataProtection security patch for CVE-2026-40372 (high-severity authentication cookie forgery). |
