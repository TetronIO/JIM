...
1. `RetiredGeneratedValue` table and migration (`LOWER` unique index), `RetiredGate`, retirement on every assignment deletion path (cascade paths write the retirement in the same transaction; the flush reconciliation writes it directly), `GeneratedValueRetired` history event on the identity's timeline, "Never reuse a value" honoured (release 1 persists the switch and forces it for sequences; this phase gives it effect for the other tokens).
