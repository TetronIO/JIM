...
<#
.SYNOPSIS
    Test Scenario 006: Scheduler Service End-to-End Testing
...
.DESCRIPTION
    ...
    IMPORTANT: This scenario requires Setup-Scenario-001.ps1 to be run first, which
...
.EXAMPLE
    ./Invoke-Scenario-006-SchedulerService.ps1 -Step All -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-006-SchedulerService.ps1 -Step Parallel -ApiKey $env:JIM_API_KEY
#>
...
Write-TestSection "Scenario 006: Scheduler Service"
...
if ($connectedSystems.Count -eq 0) {
    Write-Host "  No connected systems found - running Setup-Scenario-001..." -ForegroundColor Yellow
    ...
    if (-not $config) {
        throw "Failed to run Setup-Scenario-001"
    }

    # Re-fetch connected systems after setup
    $connectedSystems = @(Get-JIMConnectedSystem)
    if ($connectedSystems.Count -eq 0) {
        throw "No connected systems found after running Setup-Scenario-001"
    }
}

# Prefer "HR CSV Source" as it's always set up correctly by Setup-Scenario-001
...
if ($Step -eq "ManualTrigger" -or $Step -eq "All") {
    ...
    while ($elapsed -lt $maxWaitTime) {
        ...
        # Status: Queued, InProgress, Complete, CompleteWithError, Failed, Cancelled
        $statusValue = $finalExecution.status
        $isTerminal = $statusValue -in @("Complete", "CompleteWithError", "Failed", "Cancelled")
...
# ============================================================================
# Test: Parallel Steps (requires 4 connected systems from Scenario-001 setup)
# ============================================================================

if ($Step -eq "Parallel" -or $Step -eq "All") {
    Write-TestSection "Test 6: Parallel Step Execution (Complex Multi-System Schedule)"

    # Check if we have all 4 connected systems from Scenario-001 setup
    # The extended Scenario-001 setup creates:
    # - HR CSV Source
    ...
    if ($missingCount -gt 0) {
        Write-Host "  ✗ FAILED: Parallel step testing requires 4 connected systems from extended Scenario-001 setup" -ForegroundColor Red
        Write-Host "  Found $($connectedSystems.Count) systems, missing $missingCount required systems" -ForegroundColor DarkGray
        Write-Host "  Run: ./Setup-Scenario-001.ps1 first to create all required systems" -ForegroundColor DarkGray
