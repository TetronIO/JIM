...
**That capability was exposed as a first-class UI criterion**, not merely in the underlying filter language. The criteria-based set-membership builder rendered relative date conditions directly from dropdowns, for example `Employee End Date prior to 30 days ago` and `Employee End Date after 364 days ago`: a relation (`prior to` / `after`, mapping to the comparison operators) plus a relative value expressed as `N days ago` (or days from now). The engine compiled this to `now ± duration` in its filter language under the hood, but the administrator built it from controls, never by hand-authoring a filter. Our proposal adopts the same model (operator + relative offset of count + unit + direction) and **extends** it: units beyond days (hours, weeks, months, years), explicit whole-day rounding, on-demand evaluation, and applying relative dates to object/predefined searches as well as sync rule scoping.

A UI detail worth carrying across: that prior art labelled the date relations in friendly terms (`prior to` / `after`) rather than `less than` / `greater than`. For `DateTime` attributes, JIM's criteria editor should consider the same friendlier wording (for example "before" / "on or before" / "after" / "on or after") rather than the raw comparison-operator names, while still mapping to the existing `SearchComparisonType` values.
