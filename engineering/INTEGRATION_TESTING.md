...
./test/integration/Run-IntegrationTests.ps1 -Scenario "Scenario22-OpenLdapPasswordPolicy" -Step Discovery  # Scenario 22 (cumulative): Discovery, Provision, Override, Preflight (OpenLDAP only)
...
**Status**: authored against [#1702](https://github.com/TetronIO/JIM/issues/1702) Phase 6; verification follows Phases 1 to 4. OpenLDAP only.
...
**Why no TLS is acceptable here.** `LdapConnector.OpenPasswordConnection` only *warns* on an unencrypted channel; it does not refuse, and Initial Password delivery proceeds. The Bitnami image runs with `LDAP_ENABLE_TLS=no`, and test 2 establishes that it takes an RFC 3062 operation in the clear. What the scenario proves is policy discovery and enforcement, neither of which depends on the transport. Scenario 20's write-up says the OpenLDAP container "serves no TLS, so the RFC 3062 Password Modify path JIM would use there cannot be exercised at all"; test 2 here is the direct check of that claim, and a green test 2 means that sentence should be corrected. If the container ever refuses a cleartext extended operation ("Confidentiality required (13)" or "unwilling to perform (53)"), the fallback described at the top of the Invoke script applies: `LDAP_ENABLE_TLS=yes` with a generated certificate trusted through `Add-JIMCertificate` (Scenario 15's pattern), and a directory configuration switched to `ldaps://`.
...
| Scenario 22 | 📝 Authored | OpenLDAP password policy discovery: enforcement negative control, discovered values, non-root provisioning with nothing parked, override signal, preflight (OpenLDAP only) (#1702); verification runs after the reader lands |
