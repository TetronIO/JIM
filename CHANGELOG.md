...
- ✨ Relative-date scope filters now keep working for objects whose source data isn't changing: a new built-in Temporal Scope Reconciliation schedule periodically re-evaluates time-driven scope transitions, so a leaver is deprovisioned when their end date passes and a joiner is provisioned downstream when their start date arrives. It runs hourly by default and can be re-timed, enabled or disabled, but not renamed or deleted.
