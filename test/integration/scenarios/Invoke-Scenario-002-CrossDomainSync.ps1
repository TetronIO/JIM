...
<#
.SYNOPSIS
    Test Scenario 002: Person Entity - Cross-domain Synchronisation
...
.EXAMPLE
    ./Invoke-Scenario-002-CrossDomainSync.ps1 -Step All -Template Micro -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-002-CrossDomainSync.ps1 -Step Provision -Template Small -ApiKey $env:JIM_API_KEY
#>
...
Write-TestSection "Scenario 002: Directory to Directory Synchronisation ($directoryType)"
...
# Test user details - use unique names for Scenario 002 to avoid conflicts with Scenario 001
...
try {
    ...
    Write-Host "Verifying Scenario 002 infrastructure..." -ForegroundColor Gray
    ...
    # Run Setup-Scenario-002 to configure JIM
    Write-Host "Running Scenario 002 setup..." -ForegroundColor Gray
    $setupParams = @{
    ...
    & "$PSScriptRoot/../Setup-Scenario-002.ps1" @setupParams

    Write-Host "✓ JIM configured for Scenario 002" -ForegroundColor Green
    ...
    if (-not $sourceSystem -or -not $targetSystem) {
        throw "Connected systems not found. Ensure Setup-Scenario-002.ps1 completed successfully."
    }
...
if ($testResults.Success) {
    Write-Host ""
    Write-Host "✓ All Scenario 002 tests passed!" -ForegroundColor Green
    exit 0
}
else {
    Write-Host ""
