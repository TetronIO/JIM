...
- **The status chip must stay labelled.** The Activity page shows two chips: the Summary panel's `ActivityStatus` ("Complete") and this panel's `ScheduleExecutionStatus` ("Completed"). They describe different objects, and unlabelled they read as a contradiction; the "Schedule Execution:" label is the only thing telling a reader that this one is the whole run's outcome, not this Activity's. Do not resolve the clash by renaming the enum member: the REST API serialises enums by name (`JsonStringEnumConverter(namingPolicy: null, allowIntegerValues: false)`, `ApiJsonConfiguration.cs`), so a rename is a breaking API change.
