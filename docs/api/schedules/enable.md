...
Enables a schedule so it will run on its configured trigger. Only applies to cron-triggered schedules; manual schedules can be enabled but will only run when triggered via the [Run](run.md) endpoint.
