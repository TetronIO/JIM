...
<#
.SYNOPSIS
    Configure JIM for the Scenario 016 JIM SQL Connector matrix against one database provider
...
Write-TestSection "Scenario 016 Setup: $($config.DisplayName)"
...
$system = New-JIMConnectedSystem -Name $Name `
    -Description "Scenario 016: JIM SQL Connector matrix against $($config.DisplayName). $Purpose" `
...
$importRule = New-JIMSyncRule `
    ...
    -Description "Scenario 016 inbound rule: projects EMPLOYEES rows into the Metaverse so the driver-shape rows can read the values JIM derived." `
    -ConnectedSystemId $system.id `
...
$appUserRule = New-JIMSyncRule `
    ...
    -Description "Scenario 016 outbound rule: provisions into APP_USERS, whose primary key the database generates." `
...
$naturalRule = New-JIMSyncRule `
    ...
    -Description "Scenario 016 outbound rule: provisions into a table whose primary key JIM authors." `
...
if ($Provider -eq "Oracle") {
    $guidRule = New-JIMSyncRule `
        ...
        -Description "Scenario 016 outbound rule: provisions into a table keyed RAW(16) DEFAULT SYS_GUID()." `
        -ConnectedSystemId $exportSystem.System.id `
...
Write-TestSection "Scenario 016 Setup Complete: $($config.DisplayName)"
