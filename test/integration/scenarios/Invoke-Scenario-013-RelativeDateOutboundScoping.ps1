...
.DESCRIPTION
    ...
    container, so the test stays fast and free of directory flakiness, mirroring Scenario 012. The
...
Write-TestSection "Scenario 013: Relative-Date Outbound (Export) Scoping"
...
try {
    ...
    Write-Host "Running Scenario 013 setup..." -ForegroundColor Gray
    & "$PSScriptRoot/../Setup-Scenario-013.ps1" -JIMUrl $JIMUrl -ApiKey $ApiKey -Template $Template
...
if ($testResults.Success) {
    Write-Host ""
    Write-Host "OK All Scenario 013 tests passed!" -ForegroundColor Green
    exit 0
}
else {
    Write-Host ""
