...
public interface IConnectorImportUsingCalls
{
    ...
    /// <param name="progress">Narrates what you are doing, moves between the phases you declared through <see cref="IConnectorPhases"/> (i.e. "Querying root DSE...", "Fetching User objects from Employees (page 3)..."), and carries how many objects there are in total and how many you have read so far. Never null. JIM counts each page as it arrives, so reporting objects read matters most where a single call drains more than one of your own pages. The vocabulary is yours; emit on phase and page boundaries rather than per object.</param>
