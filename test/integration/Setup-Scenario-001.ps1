...
<#
.SYNOPSIS
    Configure JIM for Scenario 001: HR to Enterprise Directory
...
.EXAMPLE
    ./Setup-Scenario-001.ps1 -JIMUrl "http://localhost:5200" -ApiKey "jim_abc123..."
...
Write-TestSection "Scenario 001 Setup: HR to Enterprise Directory ($($DirectoryConfig.ConnectedSystemName))"
...
# 1. Running Setup-Scenario-001.ps1 manually during development
...
try {
    ...
    if ($usernameSetting) {
        ...
        $ldapSettings[$usernameSetting.id] = @{ stringValue = $DirectoryConfig.JimBindDN }
    }
    if ($passwordSetting) {
        $ldapSettings[$passwordSetting.id] = @{ stringValue = $DirectoryConfig.JimBindPassword }
    }
    if ($useSSLSetting) {
    ...
    if ($ExportConcurrency -gt 1) {
        ...
        if ($exportConcurrencySetting) {
            ...
            Set-JIMConnectedSystem -Id $ldapSystem.id -SettingValues $exportSettings | Out-Null
            Write-Host "  ✓ Configured LDAP Export Concurrency: $ExportConcurrency" -ForegroundColor Green
        }
        else {
    ...
    if ($MaxExportParallelism -gt 1) {
        ...
        Write-Host "  ✓ Configured LDAP Max Export Parallelism: $MaxExportParallelism" -ForegroundColor Green
    }
}
catch {
...
if ($trainingObjectTypes -and $trainingObjectTypes.Count -gt 0) {
    ...
    try {
        ...
        Write-Host "    The scenario must seed training-records.csv before running Setup-Scenario-001." -ForegroundColor Yellow
        throw
    }
}
...
if ($crossDomainObjectTypes -and $crossDomainObjectTypes.Count -gt 0) {
    ...
    try {
        ...
        Write-Host "    The scenario must seed cross-domain-users.csv before running Setup-Scenario-001." -ForegroundColor Yellow
        throw
    }
}
...
try {
    ...
    elseif (-not $mvUserType) {
        ...
        if (-not $exportRule) {
            ...
            # their Metaverse Object is deleted (issue #655; asserted by Scenario 004)
...
Write-Host ""
Write-Host "✓ Scenario 001 setup complete (4 connected systems)" -ForegroundColor Green
Write-Host ""
Write-Host "Next steps:" -ForegroundColor Yellow
Write-Host "  1. Run: ./scenarios/Invoke-Scenario-001-HRToIdentityDirectory.ps1 -Template $Template" -ForegroundColor Gray
