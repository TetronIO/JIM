...
- 🐛 The first hierarchy retrieval against a newly added Connected System now keeps the Containers it discovered. Partitions were saved and every Container beneath them silently discarded, so the Container tree came back empty with nothing to select, and the retrieval still reported them as found. Retrieving a second time populated them, because by then the Partitions existed and a different code path ran. Any Connected System affected by this is repaired by retrieving its hierarchy again.
