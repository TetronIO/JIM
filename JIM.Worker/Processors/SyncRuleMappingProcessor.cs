        ...
        foreach (var source in syncRuleMapping.Sources.OrderBy(q => q.Order))
        {
            ...
            if (source.ConnectedSystemAttribute != null)
            {
                ...
                foreach (var csoAttributeName in connectedSystemObjectType.Attributes.Select(a => a.Name))
                {
                    ...
                    var csoAttributeValues = connectedSystemObject.GetAttributeValues(csoAttributeName);
                    if (csoAttributeValues.Count > 0)
                    {
                        // work out what data type this attribute is and get the matching imported object attribute
                        var csoAttribute = connectedSystemObjectType.Attributes.Single(a => a.Name.Equals(csoAttributeName, StringComparison.CurrentCultureIgnoreCase));
                        foreach (var csoAttributeValue in csoAttributeValues)
                        {
                            ...
                            switch (csoAttribute.Type)
                            {
                                case AttributeDataType.Text:
                                    ...
                                    var newStringValues = csoAttributeValue.StringValues.Where(sv => !connectedSystemObject.AttributeValues.Any(av => av.Attribute.Name == csoAttributeName && av.StringValue != null && av.StringValue.Equals(sv)));
                    ...
                    else
                    {
                        ...
                        var mvoAttributeValuesToDelete = connectedSystemObject.MetaverseObject.AttributeValues.Where(q => q.Attribute.Name == csoAttributeName);
                ...
                if (source.ConnectedSystemAttribute.AttributePlurality == AttributePlurality.SingleValued)
                {
                    ...
                    // injection point for refactoring...
                    ...
                    if (csoAttributeValue != null)
                        SetMetaverseObjectAttributeValue(connectedSystemObject.MetaverseObject, syncRuleMapping.TargetMetaverseAttribute, csoAttributeValue);
                    else
                ...
                else
                {
                    ...
                    // interjection point for refactoring...
