...
- ~~**Whether the service account actually holds password-reset rights**, via `allowedAttributesEffective`.~~ **Withdrawn during implementation: that mechanism answers a different question and inverts on exactly the deployments JIM recommends.** See "What Phase 2 found" above for the specification citations. The check ships reporting an unknown, and names the containers where the permission needs to hold so an administrator can check it at the directory. A sound implementation is possible (client-side evaluation of the target's `nTSecurityDescriptor`) and is a decision in its own right rather than part of this phase.
