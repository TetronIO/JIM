...
#   .devcontainer/db.local.yml       — overrides for the standalone database (jim-db)
#
...
# .devcontainer/db.local.yml — overrides jim.database for standalone db (jim-db)
DB_LOCAL="$REPO_ROOT/.devcontainer/db.local.yml"
...
services:
  jim.database:
    ...
    command: ${DB_COMMAND}
COMPOSE_EOF
print_success ".devcontainer/db.local.yml"
