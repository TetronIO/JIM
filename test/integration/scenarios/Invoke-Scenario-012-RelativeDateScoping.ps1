...
.DESCRIPTION
    ...
    Scenario 010. The relative-date resolver's arithmetic (unit rounding, Ago/FromNow) is
...
Write-TestSection "Scenario 012: Relative-Date Inbound Scoping"
...
try {
    ...
    Write-Host "Running Scenario 012 setup..." -ForegroundColor Gray
    & "$PSScriptRoot/../Setup-Scenario-012.ps1" -JIMUrl $JIMUrl -ApiKey $ApiKey -Template $Template
...
if ($testResults.Success) {
    Write-Host ""
    Write-Host "OK All Scenario 012 tests passed!" -ForegroundColor Green
    exit 0
}
else {
    Write-Host ""
