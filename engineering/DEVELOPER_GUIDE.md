...
- **In a GitHub Codespace**: uses the built-in `gh-gpgsign` helper, which signs via the GitHub API. No local key management required, but you must first enable **GPG verification** for your account: visit github.com/settings/codespaces, enable *GPG verification*, and allow this repository (or all repositories). Without it the API refuses to sign with `Current user GPG signing disabled`. Because the capability is minted into the codespace token at startup, enabling it requires a Codespace restart (Stop then Start, or rebuild) to take effect. The setup script probes signing during container creation and prints these recovery steps if the capability is missing.
