...
<#
.SYNOPSIS
    Setup for Scenario 023: Unique Value Generation

.DESCRIPTION
    Configures JIM to generate the identifiers an HR feed would not carry in a real deployment
    (Unique Value Generation, #242, release 1). The provisioning substrate is Scenario 001's,
    ...
    release-1 capability Account Name alone does not cover:
      ...
      - A generated export mapping on the LDAP target's "preferredLanguage" attribute (Random token,
        ...
        Scenario 001 mapping, so it is genuinely spare on both directory types this scenario supports.
...
.PARAMETER Template
    Data scale template, passed through to Setup-Scenario-001.ps1. Micro is the default: this scenario
...
.EXAMPLE
    ./Setup-Scenario-023.ps1 -ApiKey "jim_..." -Template Micro

.EXAMPLE
    ./Setup-Scenario-023.ps1 -ApiKey "jim_..." -DirectoryConfig (Get-DirectoryConfig -DirectoryType SambaAD -Instance Primary)
#>
...
try {
    ...
    # pattern Setup-Scenario-001.ps1 itself uses for the Training attributes.
    ...
    if (-not $importRule) {
        throw "Setup failed: import Synchronisation Rule '$importRuleName' not found; Setup-Scenario-001.ps1 should have created it."
    }
    ...
    if (-not $preferredLanguageAttr) {
        throw "Setup failed: LDAP attribute 'preferredLanguage' not found in the schema for '$ldapSystemName' " +
              ...
              "pick a different spare attribute here and in Invoke-Scenario-023-UniqueValueGeneration.ps1's " +
    ...
    if (-not $exportRule) {
        throw "Setup failed: export Synchronisation Rule '$exportRuleName' not found; Setup-Scenario-001.ps1 should have created it."
    }
    ...
    if (-not $accountNameMapping) {
        throw "Setup failed: no Account Name mapping on '$importRuleName'; Setup-Scenario-001.ps1 -GenerateAccountName should have created it."
    }
...
# Summary
Write-TestSection "Scenario 023 Setup Complete"
...
# Return Scenario 001's configuration plus the ids this scenario's assertions need.
