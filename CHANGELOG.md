...
- 🔄 Search and filter boxes across the admin portal now filter as you type (debounced, so a keystroke does not fire a query), instead of only updating after you click away; each also gains a clear (x) button. Multi-criteria forms behind an explicit Search button and the log filter behind Refresh are unchanged. (#864)
