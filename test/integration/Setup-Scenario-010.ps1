...
<#
.SYNOPSIS
    Configure JIM for Scenario 010: Sync Rule Scoping Behaviour
...
.DESCRIPTION
    ...
    The Invoke-Scenario-010 script reconfigures the rules' scope actions between sub-tests
...
.EXAMPLE
    ./Setup-Scenario-010.ps1 -JIMUrl "http://localhost:5200" -ApiKey "jim_abc123..."

.EXAMPLE
    ./Setup-Scenario-010.ps1 -ApiKey "jim_..." -DirectoryConfig (Get-DirectoryConfig -DirectoryType OpenLDAP)
#>
...
Write-TestSection "Scenario 010 Setup: Sync Rule Scoping Behaviour"
...
$hrSettings = @{}
if ($filePathSetting) { $hrSettings[$filePathSetting.id] = @{ stringValue = "/connector-files/test-data/scenario-010-hr-users.csv" } }
...
# EnforceState is disabled here because Scenario 010 is testing scoping transitions, not drift
...
$hrCsvFilePath = "/connector-files/test-data/scenario-010-hr-users.csv"
...
Write-TestSection "Scenario 010 Setup Complete"
