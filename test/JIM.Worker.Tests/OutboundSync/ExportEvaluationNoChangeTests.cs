...
public class ExportEvaluationNoChangeTests
{
    ...
    /// <summary>
    /// When a pending export has all-null value fields (e.g., clearing an attribute), it should
    /// NOT be treated as no-net-change if the target CSO currently has a non-null value.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// Tests that when attributes are recalled (removed from MVO), CreateAttributeValueChanges
    /// produces no changes because the MVO no longer has values for those attributes. The recalled
    /// values are passed via removedAttributes but we intentionally do NOT generate null-clearing
    /// exports because target systems may reject null values for mandatory attributes. Proper
    /// recall export handling requires attribute priority (Issue #91).
    /// </summary>
    [Test]
    public void CreateAttributeValueChanges_RecalledSingleValuedAttributes_ProducesNoChangesAsync()
    {
        ...
        // Create the removed attribute values (these are snapshots taken before recall)
        ...
        // Set up CSO attribute cache with target CSO's current values
        ...
        // Assert: no changes produced because MVO has no attribute values (all recalled)
        // and null-clearing exports are intentionally NOT generated (known limitation - Issue #91).
        // The target retains its existing values until attribute priority is implemented.
        Assert.That(changes, Is.Empty,
            "Expected no attribute changes — recalled attributes should not produce null-clearing exports (Issue #91)");
    }

    /// <summary>
    /// Tests the full export evaluation flow for recalled attributes. Since null-clearing
    /// exports are intentionally NOT generated (Issue #91 — attribute priority required),
    /// the export evaluation should produce NO pending exports for recalled attributes.
    /// The target retains its existing values until attribute priority is implemented.
    /// </summary>
    [Test]
    public async Task EvaluateExportRules_RecalledAttributes_ProducesNoPendingExportAsync()
    {
        ...
        // Create the removed attribute values
        var removedDisplayName = new MetaverseObjectAttributeValue
        {
            Id = Guid.NewGuid(),
            MetaverseObject = mvo,
        ...
        // Set up the existing target CSO (Normal status = Update operation)
        var existingCso = new ConnectedSystemObject
        {
            Id = Guid.NewGuid(),
            ConnectedSystemId = targetSystem.Id,
        ...
        // Add CSO attribute values
        var csoDisplayNameValue = new ConnectedSystemObjectAttributeValue
        {
            Id = Guid.NewGuid(),
            AttributeId = targetDisplayNameAttr.Id,
            StringValue = "Joe Bloggs",
        ...
        var csoEmployeeIdValue = new ConnectedSystemObjectAttributeValue
        {
            Id = Guid.NewGuid(),
            AttributeId = targetEmployeeIdAttr.Id,
            StringValue = "EMP001",
        ...
        existingCso.AttributeValues.Add(csoDisplayNameValue);
        existingCso.AttributeValues.Add(csoEmployeeIdValue);
        ...
        MockDbSetConnectedSystemObjects = ConnectedSystemObjectsData.BuildMockDbSet();
        MockDbSetSyncRules = SyncRulesData.BuildMockDbSet();
        ConnectedSystemObjectAttributeValuesData.AddRange(existingCso.AttributeValues);
        MockDbSetConnectedSystemObjectAttributeValues = ConnectedSystemObjectAttributeValuesData.BuildMockDbSet();
        ...
        // Build the export evaluation cache (as the sync processor does at sync start)
        var cache = await Jim.ExportEvaluation.BuildExportEvaluationCacheAsync(sourceSystem.Id);

        // Act: call EvaluateExportRulesWithNoNetChangeDetectionAsync (the production entry point)
        ...
        // Assert: no pending exports created because null-clearing exports are intentionally
        // NOT generated. The MVO has no attribute values (all recalled), so CreateAttributeValueChanges
        // produces 0 changes, and CreateOrUpdatePendingExportWithNoNetChangeAsync skips creating
        // a pending export when there are no attribute changes.
        Assert.That(result.PendingExports, Is.Empty,
            "Expected no PendingExports — recalled attributes should not produce null-clearing exports (Issue #91)");
    }
