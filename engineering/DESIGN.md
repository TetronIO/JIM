...
- **Ownership across the JIM boundary is shown by repeating a name, not by drawing a container.** A Connector Space belongs to a Connected System, but the space sits inside the JIM boundary and the system sits outside it, so any box enclosing both must straddle the JIM container and would imply the external system is partly inside JIM. Instead give the owned box a `.jimdg-owner-name` eyebrow carrying the owner's name verbatim (see `sync-pipeline.svg`). Use a concrete illustrative name rather than a role word: "HR System" repeated twice reads as the same system, where "Source" repeated twice still reads as generic labelling. Say so in the caption when names are illustrative.
- **Edge labels sit on their edge, and the edge is cut away behind them.** Centre the label on the line it names (`.jimdg-anchor-middle`), put the edges, arrowheads and packets inside a group carrying `mask="url(#<prefix>-labelcut)"`, and let that mask punch a small rounded rectangle out of them behind each label. The line then stops cleanly either side of the words and a packet passing underneath is occluded at the same edge. Labels live in a trailing `…-labels` group, outside the mask. `scripts/Update-DiagramLabelCuts.ps1` generates the cut rectangles from the rendered text metrics and verifies them; never hand-write one.
- **Cut, do not paint over.** The knockout stroke `.jimdg-hub-title` uses works for boundary titles because they sit on a surface whose fill colour is known, so there is an opaque colour to borrow. Edge labels mostly sit on the bare page, where there is not: the docs background is knowable but the README exports in `.github/diagrams/` render on whichever GitHub theme the reader has chosen. A mask cut is transparent, so it is correct on every background. A knockout is also wrong at label size for a second reason: it follows glyph outlines, so the edge stays visible in the space between words, and widening it to close that gap bulges the halo around the letters and slices packets into crescents. The `#ffffff` / `#000000` inside a `<mask>` are luminance, not colour, and are the one place a literal is correct: swapping them for `--jimdg-*` tokens would make the cut theme-dependent.
- **An edge too short to carry its label keeps the label beside it, at least 8 units clear.** A cut wider than the edge erases the whole edge, so the label only moves onto the line when a visible stub of at least ~28 units survives (the arrowhead alone is 10). This is the fallback for step labels and captions too, which name a stage rather than hug an edge.
- **A label crossing a node or boundary outline must be moved, never cut.** Masking a `<rect>` removes its fill along with its stroke, punching a hole in the surface. `-Check` reports these separately for that reason.
- **Mirrored geometry keeps mirrored labels.** When two edges are reflections of each other about the diagram's centre line, place their labels at reflected coordinates too (`x` and `1160-x`, same `y`); asymmetry in a symmetric composition reads as an error.
- **Boundary titles are painted last so edges pass behind them.** `.jimdg-hub-title` carries a knockout stroke in the boundary fill colour (`paint-order: stroke fill`), which only clears the edges beneath it if the title comes after them in document order (SVG paints in document order). Keep the title in a trailing `…-boundary-title` group, not inside the boundary group with its `<rect>`, and keep it the final element when adding to the file.
