...
<#
.SYNOPSIS
    Populate OpenLDAP with the password policy fixture for Scenario 022: OpenLDAP Password Policy

.DESCRIPTION
    Turns the Yellowstone suffix (dc=yellowstone,dc=local) of the OpenLDAP container into a directory
    that ENFORCES a password policy, so that Scenario 022 can prove JIM reads that policy and generates
    ...
    - uid=s22probe, an ordinary inetOrgPerson with a known password, placed OUTSIDE ou=People so the
      Scenario 001 substrate's import never sees it. The scenario changes this account's own password
...
.PARAMETER ProvisionerBindDN
    The account JIM binds as. Setup-Scenario-022.ps1 takes the same value.

.PARAMETER ProvisionerPassword
    The provisioner's password. Setup-Scenario-022.ps1 takes the same value.
...
.EXAMPLE
    ./Populate-OpenLDAP-Scenario-022.ps1
#>
...
Write-TestSection "Scenario 022: Populating OpenLDAP (Yellowstone) with the password policy fixture"
...
function Invoke-Scenario22Ldap {
    ...
    .SYNOPSIS
        ...
        temp-file + docker-exec pattern of Populate-OpenLDAP-Scenario-019.ps1.
    #>
    param(
...
cn: default
description: Scenario 022 default password policy (minimum length 12, history 5, maximum age 90 days)
...
cn: $provisionerCn
description: The account JIM binds as in Scenario 022; not the rootdn, so the password policy applies to what it writes
userPassword: $ProvisionerPassword

dn: $ProbeBindDN
objectClass: inetOrgPerson
uid: $probeUid
cn: Scenario 022 Probe
sn: Probe
givenName: Scenario
description: Changes its own password in Scenario 022's negative control; outside ou=People so the Scenario 001 substrate never imports it
...
pwdCheckQuality: 2
"@
Write-Host "  OK $policyDN carries Scenario 022's values (pwdMinLength 12, pwdInHistory 5, pwdMaxAge 7776000, pwdCheckQuality 2)" -ForegroundColor Green
...
Write-TestSection "Scenario 022 OpenLDAP Population Complete"
...
Write-Host ""
Write-Host "Scenario 022 OpenLDAP population complete" -ForegroundColor Green
