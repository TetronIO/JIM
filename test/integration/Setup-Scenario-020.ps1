...
<#
.SYNOPSIS
    Setup for Scenario 020: Password Synchronisation

.DESCRIPTION
    Builds the substrate Scenario 020 asserts against: accounts provisioned into the chosen directory, enabled, and
    holding a password the scenario knows, with Password Synchronisation configured on the directory but switched
    OFF. Works against Samba AD or OpenLDAP; see Setup-Scenario-017.ps1 for which directory-specific behaviours are
    gated and which are caveats.

    Two pieces, and the second is the whole point of this script:

      1. Provisioned accounts with a known password. That is exactly what Setup-Scenario-017.ps1 builds, so this
         composes it rather than rebuilding it (Scenario 017 composes Setup-Scenario-001.ps1 in the same way). The
         one thing it asks for differently is the expiry behaviour: Scenario 017 wants must-change-at-next-sign-in
         because that is what it asserts, whereas Scenario 020 needs accounts whose Initial Password signs in
         ...
         is also why Setup-Scenario-020 asks for NeverExpires rather than RequireChangeAtNextSignIn regardless of
         directory: the latter is the one Active Directory-only behaviour Setup-Scenario-017.ps1 actually gates on
...
.EXAMPLE
    ./Setup-Scenario-020.ps1 -ApiKey "jim_..." -Template Micro
#>
...
Write-TestSection "Scenario 020 Setup: Password Synchronisation"

# ─────────────────────────────────────────────────────────────────────────────────────────────
# Step 1: Provisioned accounts holding a known, usable password
# ─────────────────────────────────────────────────────────────────────────────────────────────
Write-TestStep "Step 1" "Running Setup-Scenario-017.ps1 for provisioned accounts with a known Initial Password"
...
try {
    ...
    Set-JIMConnectedSystemPasswordSynchronisation `
        ...
        -ChangeReason "Scenario 020: staged before the change window, as requirement 4 describes" | Out-Null
...
Write-TestSection "Scenario 020 Setup Complete"
