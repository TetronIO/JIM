...
- ✨ A running Schedule now shows its whole shape in the Operations queue. The header above its tasks draws the Schedule as a rail, one marker per step in the order they run, with the step names underneath and the step it has reached named beside them ("Step 2 of 5") in place of the task counts that had to be added up. The rail shows every step, not just those still queued: a task is deleted the moment it finishes, so steps already done are read from their Activities instead of vanishing as the run proceeds, and a step that failed stays visible, including when the group is collapsed to a single row. A step running several tasks at once is drawn as one divided marker, a wedge per task carrying that task's own outcome, so one of two parallel imports failing while the other is still running reads as exactly that; the wedges are ordered by outcome rather than by task, so a failure always starts at the top of the marker and stays visible however wide the Schedule fans out. (#1162)
