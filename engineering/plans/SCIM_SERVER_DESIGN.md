...
>
> 1. **JIM-to-JIM SCIM round-trip is an explicit compatibility goal.** JIM's SCIM client connector (#545) pointed at this server must achieve paginated full import, `meta.lastModified`-filtered delta import, and export with a confirming import. Index-based pagination, `meta.lastModified` filtering and ETag support therefore move into required scope; the MVP/Post-MVP splits below were written in isolation and no longer govern those items.
>
>    **Corrected against what shipped (#1177, August 2026):** the criterion originally said "ETag-conditional change detection". The client establishes change detection from a `meta.lastModified` filter instead; a conditional GET per resource costs one request per object, which is worse than the filtered list a Delta Import already issues. ETag support stays in required scope here, but for `If-Match` on writes, where it guards a lost update, rather than `If-None-Match` on reads. **This round trip is the one success criterion from the client connector's plan still outstanding, and it is this issue's to satisfy:** #545 is closed, so nothing else is waiting to prove it.
...
> 3. **Shared protocol library `JIM.Scim`, which now exists.** `src/JIM.Scim/` shipped with the client connector (#1177), referencing only `JIM.Models` as designed. It already holds the SCIM resource DTOs (`Resources/`), serialisation (`Serialisation/`), the PATCH operation model (`Messages/`), the discovery and capability model (`Discovery/`), schema URN constants (`ScimUrns.cs`), endpoint paths (`ScimEndpoints.cs`), and the SCIM-to-`AttributeDataType` mapping and flattening convention (`Schema/ScimAttributeMapper.cs`). This server consumes it from `JIM.Web`; do not re-model SCIM types here.
> 4. **One flattening convention, owned by `JIM.Scim`:** canonical-type flattening (`emails.work`, `emails.primary` from the `primary=true` entry), implemented in `Schema/ScimAttributeMapper.cs` and already proven against a live service provider by the client connector. The first-entry-wins mapping in the Schema Mapping section below is superseded.
