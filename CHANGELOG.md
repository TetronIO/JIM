...
- 🔒 Attributes that hold credential material (`unicodePwd`, `userPassword`, `dBCSPwd`, `ntPwdHistory`, `lmPwdHistory`, `supplementalCredentials`, `unixUserPassword` and `msDS-ManagedPassword`) can no longer be imported into JIM, selected for management, or named as the source or target of an Attribute Flow, whether from the portal, the REST API or PowerShell. A schema refresh reports any it found and blocked, and one that an existing deployment had already selected is deselected and locked rather than deleted, so Synchronisation Rules referencing it stay intact. Passwords are synchronised through JIM's dedicated password channel, which writes to the Connected System without ever reading the value back into the Metaverse.
