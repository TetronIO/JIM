...
- ✨ Custom Metaverse Object Types can now be created programmatically: a new `POST /api/v1/metaverse/object-types` endpoint and `New-JIMMetaverseObjectType` cmdlet let administrators add new object types (with optional attribute associations, icon, and deletion-rule configuration) without going through the admin UI. Useful for automated provisioning of bespoke identity types and for integration test scenarios that need many sandbox types created and torn down quickly.
- ✨ The scoping criterion API and `New-JIMScopingCriterion` cmdlet now accept `LongValue` (for `LongNumber` attribute comparisons) and `CaseSensitive` (for text comparisons), bringing the public surface fully in line with the underlying scoping evaluator. The corresponding DTO returns `LongValue` on read so scoping rules with long-integer thresholds round-trip cleanly.
...
- ✨ Factory reset: a new `POST /api/v1/system/reset` endpoint and `Reset-JIMSystem` cmdlet wipe all customer data and configuration from the database in a single transaction, preserving the schema, EF Core migration history, the rows seeded at first launch (built-in metaverse attributes and object types, built-in roles, built-in connector definitions, built-in example data sets and templates, built-in predefined searches), the singleton service settings record, and infrastructure API keys. The operation is refused with HTTP 409 if any activity is currently in progress to protect synchronisation integrity. Connector files on the host mount are NOT wiped and must be cleared out-of-band if a clean filesystem is also required.
- 🧪 Integration test Scenario 10 now covers the full sync rule scoping transition matrix end-to-end: inbound enter/in-scope-update/exit transitions (both Disconnect and RemainJoined actions), outbound enter scope, outbound exit scope under both Disconnect and Delete actions, the cross-system inline cascade (Delete PendingExport queued during inbound sync, before any Export run), and round-trip persistence of scoping criteria operators across nested AND groups (#656). Each cascade sub-test runs against a freshly reset JIM instance via the new `Reset-JIMSystem` cmdlet so the assertions are not polluted by intermediate state from the inbound block.
- 🧪 Integration test Scenario 11 (Scoping Criteria Evaluation Matrix) exercises every applicable (operator x value-type x group-structure) combination of scoping criteria evaluation end-to-end via the synchronisation pipeline. Three coverage tiers — Quick (~12 cells, < 90s, PR feedback), Default (~41 cells, < 5 min, regression), and Exhaustive (~152 cells, < 10 min, pre-release / post-evaluator-refactor) — pick a representative subset of the full matrix; selection is parameter- or menu-driven. A round-trip persistence sub-test verifies that each value carrier (`StringValue`, `IntValue`, `LongValue`, `DateTimeValue`, `BoolValue`, `GuidValue`) survives create / read / update / delete unchanged, and API-level negative-cell probes record currently-undefended invalid combinations (e.g. `Contains` on a `Boolean` attribute) as informational gaps. Each cell is materialised as a dedicated CSO and Metaverse object type so per-cell projections can be asserted independently.

### Fixed

- 🐛 Retrieving or refreshing a Connected System's schema in the admin UI now persists the discovered object types and attributes, so the object type and attribute selection interface renders straight afterwards. Previously the refresh reported success and listed what changed, but the schema was silently not saved (the detached `ConnectedSystem` write path marked only the root entity, the same class of gap previously fixed for partitions and setting values), so when the page reloaded it read back zero object types and the selection UI never appeared. A single, newly-discovered object type is now auto-selected so administrators land directly on attribute selection. Removal of object types/attributes that have disappeared at the source is deliberately out of scope for this fix (it requires reference-aware deletion to avoid destroying referenced sync rules) and is left as a follow-up (#782).
- 🐛 The outbound deprovisioning cascade (`EvaluateOutOfScopeExportsAsync` → `HandleOutboundDeprovisioningAsync`) no longer throws a `duplicate key value violates unique constraint "IX_PendingExports_ConnectedSystemObjectId_Unique"` when the target CSO still has an Exported-status PendingExport attached from a prior export. Both Delete-PE-creation paths (the cascade and `EvaluateMvoDeletionAsync`) now route through a single `EnsureDeletePendingExportAsync` helper that handles existing-PE collisions consistently: an existing Delete PE is reused, any other change type is deleted and replaced. This was a latent bug whenever a sync fired the cascade between an export and its confirming import (overlapping schedules, late or failed imports).
