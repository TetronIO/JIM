...
- **System Context, Containers, and the Worker component view** are hand-authored inline SVGs under `docs/assets/diagrams/`, themed by the site stylesheet. Authoring conventions live in `docs/CLAUDE.md` (Concept Diagrams); the visual language (tokens, geometry, motion) is defined in `engineering/DESIGN.md` > Diagrams.
- **The remaining component views** (Application Layer, Web Application, Connectors, Scheduler) are Mermaid diagrams inline in `docs/developer/architecture.md`, so they can evolve alongside the code.
