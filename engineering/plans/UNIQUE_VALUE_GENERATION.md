...
- `TryRemediateAsync`, `IsAnchoredAsync`, `CommitAsync`, `EnterNeedsDecisionAsync`, `AuthoriseRenameAsync`, `RetryAsync`, `ReleaseNeedsDecisionForMappingAsync`, `ListNeedsDecisionAsync` (release 4); `GetSequenceStateAsync`, `GetAssignmentsForObjectAsync`, `StartAgainAsync` (recall through the #1537 recall path, delete assignments, purge the attribute's retired values, reset the counter, set the #892 review flag; one Activity) (release 1). No per-identity regenerate action: deferred to #614 with Set Value.
