...
3. Neither hop of the last seam is free, and both MUST get an explicit causal edge.
   - **Queueing to executing.** This requirement originally read "already free: `ActivityRunProfileExecutionItem.PendingExportId` links them today and MUST be reused, not duplicated by a new edge". That was **wrong on the data**, and was corrected during implementation. `PendingExportId` is populated only on a `PendingExport`-type item (a provisioning export with no Connected System Object yet); every ordinary `Exported` item stores null, so there is nothing to walk back along and an export item had no cause of any kind. Nor can the link be derived after the fact: the Pending Export row is deleted the moment the export succeeds. The synchronisation therefore records its own Run Profile Execution Item on the Pending Export as it stages it (`PendingExport.QueuedByRunProfileExecutionItemId`), and the export run turns that into a `PendingExportQueueingCausedExportExecution` edge as it writes the executing item.
   - **Executing to confirming import.** `SyncImportTaskProcessor.ReconcilePendingExportsAsync` correlates confirming-import outcomes to the originating export only by `ConnectedSystemObjectId`, and a Connected System Object can cycle through export and import repeatedly, so an id-only join can pick the wrong cycle.

   The anti-duplication principle behind the original wording still stands and is unchanged: an edge must never restate a relationship an existing foreign key already expresses. This hop simply turned out not to be such a case.
...
5. The RPEI Causality panel (#1087's surface) MUST gain an upward "Caused by" affordance that walks recorded causal edges from the viewed RPEI back through as many hops as are retained. It sits **below the view canvas** and is **expanded by default**. Below rather than above keeps the record's own outcome the first thing on the page; Phase 2's "Consequences" affordance therefore goes below it again, and never above the canvas. Sitting outside the canvas means it renders identically in the Flow, Timeline and Graph views, and leaves the Flow view's measured SVG connector geometry untouched.
6. **The upward walk is over cohorts, not a linear chain.** At each level, every edge pointing at the current effect is grouped by its **attribution tuple**: edge type, reason, Connected System, and Synchronisation Rule. One group renders as one expandable statement carrying its member count ("10 Identities deleted"), never one line per cause. A cohort of one is the degenerate case and renders as a plain hop. Where a level yields two or more cohorts, the chain MUST fork into named branches rather than flatten (seven Connected System Objects leaving a Synchronisation Rule's scope and three source records deleted are two root causes converging on one effect, and that convergence is the signal an administrator most needs). Fan-in and fan-out share this treatment; see Phase 2, requirement 2.
...
- **Reference wording comes from the schema, never from the Metaverse Object Type.** Any causality text describing a reference relationship (reference recall above all) MUST name the relationship with the **attribute's** name and the object with the Metaverse Object Type's `PluralName`, both read at render time: "Removed Tina Adams from Static Members on 4 Groups". It MUST NOT branch on the Metaverse Object Type to choose vocabulary. JIM's own built-in Group carries three reference attributes (`Static Members`, `Owners`, `Managed By`), so an "if the Metaverse Object Type is Group, say members" rule is wrong on two of the three, and wrong for every customer-defined Metaverse Object Type by construction. Deriving from the attribute gives the built-in Group a first-class read and gives a bespoke Metaverse Object Type ("Removed Tina Adams from Contributors on 3 Projects") the identical one, through a single code path with no conditional. Where a cohort spans more than one attribute or Metaverse Object Type, fall back to counting objects: "References removed from 4 objects across 2 Metaverse Object Types". The same rule governs **counts**: an affordance standing for several objects counts them in the Metaverse Object Type's own noun ("4 Groups"), never as "4 attributes", which describes attribute rows and misreads a fan-out as a single object's attribute update. The panel already has the precedent: `CausalityEvent.AttributeRowsCaption` returns "Target identified by" for `DeprovisionQueued` because counting its rows as attributes "read as an attribute update rather than an account being removed"; extend that mechanism rather than adding a parallel one. If a softer phrasing ("removed as a member of Project-Pulse") is wanted later, its durable home is an optional display noun on `MetaverseAttribute` beside the existing `AttributeRenderingHint`, so it stays data an administrator can set on their own Metaverse Object Types rather than a hardcoded branch.
...
- **[Synchronisation Rule Causality Tracking](../../plans/doing/SYNC_RULE_CAUSALITY_TRACKING.md) (#399, Doing)**: an adjacent but orthogonal axis, per-attribute Synchronisation Rule attribution within a single change, rather than cross-object causal chains. Not a blocking dependency.
...
1. ~~**Edge granularity.**~~ **Resolved (2026-08-04): store at the outcome, render at the record.** The effect side carries a non-null `EffectRpeiId` **and** a nullable `EffectSyncOutcomeId`. The original framing (that outcome granularity "materially increases the number of edges written") does not hold: edge count is driven by cause-effect pairs, and at every enumerated seam it is the cause side that multiplies. Ten Metaverse Object deletions staging removals into one Group's single Pending Export write ten edges either way; naming the outcome adds a column to those ten rows, not an eleventh row. The decision therefore turned on correctness, not volume: cohort grouping (Functional Requirements, Phase 1, requirement 6) is computed **per effect**, so an edge that does not name its outcome cannot be grouped correctly on any RPEI carrying more than one. Rendering stays at the record: one panel-level "Caused by" affordance is the entry point, and an individual outcome gains its own affordance only where its cause differs from the item's dominant one.
...
- **Pre-existing wording to correct.** `ActivityRunProfileExecutionItemDetail.razor` tells the administrator that JIM staged "membership-removal Pending Exports for objects that referenced it" (two occurrences), and the reference-recall path in `ExportEvaluationServer` / `ISyncServer` carries the same word through its XML documentation and log messages. This predates the PRD and is not caused by it, but it is exactly the defect the schema-derived wording requirement above exists to prevent, so correct it when that requirement lands.
