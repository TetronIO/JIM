...
29. Generation happens for an object being synchronised when three conditions hold, and the form states them: no higher-priority contribution supplies a value (FR 6), which is generation's only precedence gate now that connector-space adoption is removed (FR 30) and the Attribute Flow priority model is the sole arbiter of whether a target's own value is kept over a generated one; every attribute the base expression reads has a value, where "reads" means every `cs["..."]` or `mv["..."]` accessor named anywhere in the expression regardless of conditional branches (the resolver is static), unless the mapping's Missing Input Behaviour is "evaluate anyway"; and the mapping is connected for the object. The Missing Input Behaviour default for a generated mapping is "contribute no value" (wait), not "evaluate anyway" as for ordinary expressions, because a value built from a missing input would be kept for ever. The form lists the inputs it reads. A generated mapping added to an existing population generates (or adopts, FR 30) for every object lacking a value on the next full synchronisation; the preview shows that count before saving. A committed value never changes with its inputs; changing one is deferred to #614 (see Non-Goals).
30. **Adopt before generate.** Connector-space adoption is removed in both modes: a participating target already holding a value is no longer, by itself, a reason to skip generation. The priority model is the only precedence that remains. To keep an existing target account's value (a brownfield join, where an Object Matching Rule joins HR's projected Metaverse Object to an account a directory already holds), the administrator adds an import Attribute Flow from that target at a higher priority than the generated flow; because generation only contributes when every higher-priority contribution is silent (FR 6, FR 29), the target's own value wins under ordinary priority resolution and generation never runs for that object. Once that flow is in place and the target's value has flowed in and been exported back at least once, it becomes the higher-priority contributor for every object the target holds, not only the ones that pre-existed the flow: generation then supplies the initial value for new starters only, and from that point the target owns the value, so a rename made in the target flows into the Metaverse; the superseded generated value assignment is removed by the existing page-flush reconciliation. This is the ordinary shape for a directory that owns account names once they exist; an administrator who wants JIM to keep owning the value should not add that flow, and should instead ensure the target holds no conflicting values before the first export. In import mode only, one adoption case remains, sourced from the Metaverse Object itself, not from any Connected System: when the generated flow becomes the winning contributor and the Metaverse Object already holds a value for the attribute (typically left behind by a higher-priority contributor that has since withdrawn), JIM adopts that value as the Committed assignment instead of generating a new one, subject to the uniqueness check; a value that collides with another live assignment is an error, never a silently regenerated one. In export mode there is no adoption: with no assignment, JIM generates and exports like any export Attribute Flow; to set a value only on accounts JIM itself creates, without touching existing ones, use Initial Export Only (#223), which never touches existing accounts and does not consume a value for them. JIM never renames a live account by ordinary generation; only Collision Remediation with the anchoring rule, or an administrator, changes an accepted value. Run order matters for the brownfield pattern above to work safely, because a target's higher-priority contribution has to reach that target's synchronisation, and cancel a staged generated-value Pending Export, before that target's export runs: see [Initialising JIM](../../../docs/concepts/synchronisation-pipeline.md#initialising-jim).

**Start again**

33. A generated Attribute Flow offers **Start again** (portal, REST and PowerShell): in one action it forgets the attribute's retired values and returns the counter to the flow's start value, so a rebuilt solution reuses the names and number range it burnt. It touches no existing value: an object that holds a value keeps it and its assignment, and only objects without one are generated for on the next synchronisation. Start again does not recall values, because recall (#1537) stages removal exports against live accounts, which is not what resetting a sequence is for: only the retired values register and the counter move, and an object's own held value and assignment (FR 30) are left exactly as they are. After the reset the gates skip any number an object still holds. The confirmation states the counts and consequences, requires the attribute name to be typed, and the action is recorded as an Activity. It has no preconditions. This is the only way a counter moves backwards.
...
### Scenario 14: Keeping an existing account's value

**Given** a brownfield Metaverse Object whose Corporate AD account `jsmith` is joined by employee id after HR projected the Metaverse Object, and the administrator has added an import Attribute Flow from Corporate AD's Account Name at a higher priority than the generated Account Name flow
**When** the generated Account Name flow evaluates
**Then** the import flow's `jsmith` wins under ordinary priority resolution before generation is ever considered (FR 6, FR 29); no value is generated and no rename is exported. Once this flow is in place, it becomes the higher-priority contributor for every Corporate AD account, not only `jsmith`'s: generation supplies the initial value for new starters only, and a rename made in Corporate AD from then on flows into the Metaverse.

### Scenario 14b: Adopting a value the Metaverse Object already holds

**Given** a Metaverse Object whose Account Name was previously supplied by a higher-priority contributor that has since withdrawn, leaving the Metaverse Object holding `jsmith` with no current contributor
**When** the generated Account Name flow becomes the winning contributor
**Then** JIM adopts `jsmith` from the Metaverse Object as the Committed assignment and does not propose `john.smith`; no rename is exported.
...
A new scenario, `Invoke-Scenario-023-UniqueValueGeneration.ps1` (numbered after the current highest), exercises every positive and negative path and every configuration permutation against the standard Samba AD and OpenLDAP targets plus a CSV target that can neither probe nor classify. Setup, run and teardown follow the runner's conventions (`Run-IntegrationTests.ps1`; scenario scripts are never invoked directly). It must cover, at minimum:
...
| Existing accounts (brownfield) | A higher-priority import Attribute Flow from the target keeps a brownfield join's existing value under ordinary priority resolution, generation never running for it, and thereafter supersedes generation for every object the target holds; a value left behind by a withdrawn higher-priority contributor is adopted from the Metaverse Object itself; no rename exported in either case |
...
Because JIM could not generate identifiers before this feature, the shared HR feed (`Generate-TestCSV.ps1`) carries IT-owned attributes as source data: `samAccountName`, `email` and `userPrincipalName`. Scenario 001 (HR to Identity Directory) imports `samAccountName` to Account Name from HR and exports it to the directory, which is the unrepresentative shape this feature exists to remove; Scenarios 002, 008, 010, 012, 013, 015, 017 and 018 reference the same columns. As part of this feature, every existing scenario is audited against one criterion: **an IT-owned attribute that flows from a source feed into the Metaverse is converted to generation; an attribute used as a join key or external identifier (the cross-domain scenarios) is kept.** Scenario 001 is the canonical conversion. The generator gains a switch so converted scenarios receive feeds without those columns while unconverted ones are unchanged. Converted scenarios must still pass with identical downstream expectations, which is itself a regression test of the feature under realistic load.
...
|---|---|---|
| 1 | Generation with all three tokens, counters, adopt before generate, case-insensitive and cross-run uniqueness, assignment lifecycle, configuration surfaces, Scenario 022's generation cases, docs. Target-side collisions are ordinary export errors naming the value and system. Scenario 001 converts Account Name only. | Nothing new |
| 2 | Metaverse-Derived Attribute Flows (own PRD) and the retired values register; Scenario 001 conversion completed with Email and UPN derived. | Release 1 |
...
15. **Adopt before generate**, sourced from the Metaverse Object's own held value in import mode; there is no connector-space adoption in either mode, and no adoption at all in export mode. Ordinary generation never renames a live account.
...
- [ ] A brownfield join keeps its existing value through a higher-priority import Attribute Flow, not connector-space adoption; adopt before generate holds only for a value the Metaverse Object itself already holds from a withdrawn higher-priority contributor (FR 30, Scenarios 14 and 14b).
