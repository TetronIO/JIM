...
<#
.SYNOPSIS
    Seed a deterministic HR schema into a Scenario 016 database container

.DESCRIPTION
    Builds the schema the JIM SQL Connector matrix (Scenario 016) runs against, in one database server,
...
.EXAMPLE
    ./New-Scenario-016-TestDatabase.ps1 -Provider SqlServer
    ./New-Scenario-016-TestDatabase.ps1 -Provider Oracle -RowCount 500000
#>
...
Write-TestSection "Scenario 016 Seed: $($config.DisplayName)"
...
if (-not $Force) {
    ...
    if ($existingHash -eq $seedHash) {
        ...
        $resetResult = Invoke-ContainerSqlScript -Config $config -ScriptText $resetScript -ScriptName "jim-scenario-016-reset-$($Provider.ToLowerInvariant()).sql"
...
$seedStart = Get-Date
$result = Invoke-ContainerSqlScript -Config $config -ScriptText $scriptText -ScriptName "jim-scenario-016-seed-$($Provider.ToLowerInvariant()).sql"
