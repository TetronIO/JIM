...
**Surfaces:** the Service Settings page's "Preview features" card (portal), `GET`/`PUT api/v1/features` (REST), and `Get-JIMFeature`/`Enable-JIMFeature`/`Disable-JIMFeature` (PowerShell). The generic Service Settings list, `api/v1/service-settings`, and `Get-JIMServiceSetting`/`Set-JIMServiceSetting`/`Reset-JIMServiceSetting` all exclude and refuse the `FeatureFlags` category: a flag changes only through the surfaces above, which enforce the tier rules the generic settings endpoints know nothing about.
