...
- [x] **UI shell:** `ConfigurationChangePreviewPanel.razor` with progress, staged arrival, summary landing view, drill-down grid, cancel, staleness and sampled labels, cap prompt. **Delivered bar the cap prompt** (see below). Four decisions worth recording:
  ...
  - **Deferred deliberately: the informed-choice cap prompt.** It is mostly *not* UI work. Today `MaximumDeltasPerGroup` is an unconditional constant, so a "keep the full data set" choice would change nothing; honouring it needs the choice threaded from the request through `PreviewSummariser`, plus the second service setting (100,000 estimated rows, Class C) and its classification entry. Shipping a prompt whose recommended-against option silently still capped would be worse than shipping no prompt. It lands as its own slice, before the pilot adapter, which is also the first thing able to produce an estimate large enough to trigger it.
