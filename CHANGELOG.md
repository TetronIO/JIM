...
- 🖥️ Flow view's Source column now shows that its events belong to the record above them, holding the record and what happened to it in one bordered group rather than as loose adjacent blocks; this is the same grouping the Downstream column already uses per Connected System. The connector onward to the Identity column now leaves that whole group, where before it left the record card and so appeared to route past any Source event sitting between the two columns. (#1087)
