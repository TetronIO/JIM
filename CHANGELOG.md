...
- 🔄 Deleting an identity now deprovisions downstream accounts according to each export Synchronisation Rule's Deprovisioning Action, rather than only deleting accounts JIM originally created. Set the action to Delete and every account the rule covers is removed from the target system when its identity is deleted or leaves scope, whether JIM provisioned it or matched a pre-existing account; leave the default Disconnect and accounts are left in place, untouched. The action is now configurable in the Synchronisation Rule editor and at creation time via the API and PowerShell, and a new Downstream Deprovisioning panel on the Metaverse Object Type page shows and edits the action for every export rule targeting the type. Existing rules keep the Disconnect default, so opt in per rule where deletion is wanted. (#655)
...
- 🔄 The LDAP Connector's default Modify Batch Size has been raised from 100 to 1000 values per request, cutting the number of round trips needed to export very large group memberships by an order of magnitude. Some directory servers' per-modification cost grows with a group's current size, so fewer, larger batches dramatically reduce total export time for groups with tens of thousands of members and up. Existing Connected Systems keep their stored value (the setting is captured per system at creation), so raise Modify Batch Size in the Connected System's Export settings to benefit; newly created Connected Systems use the new default. Constrained directories can still lower the setting (minimum 10). The connector documentation now also recommends OpenLDAP's `sortvals` tuning directive for deployments provisioning large groups.
- 🔄 Executing an Example Data Template through the REST API now queues the generation to the JIM worker service and returns immediately with the tracking Activity's ID, exactly like the portal and Run Profile execution, instead of running the whole generation inside the HTTP request with no Activity recorded (where a failure was invisible outside the service log). Follow the returned `activityId` via `GET /activities/{id}` or the lightweight `GET /activities/{id}/progress` endpoint. This is a pre-v1.0 breaking change for API clients that relied on the previous call blocking until generation had finished: the `202 Accepted` response now means queued, and the response body now carries `activityId`, `taskId` and `message`. (#1112)
- 🔄 Object Matching Rule sources no longer accept a Metaverse attribute as the source value. The shape could never function (export matching always needs a Connected System attribute to compare accounts on) and the standard rule shape, a Connected System attribute matched against the rule's Target Metaverse Attribute, now works for both import and export matching. The REST API rejects sources without a Connected System attribute, and the PowerShell matching rule cmdlets no longer offer -SourceMetaverseAttributeId. (#1053)

### Fixed

- 🐛 Accounts queued for deletion when an identity is deleted are now reported on the Activity of the run that queued them, instead of appearing only in service logs. A synchronisation run that deletes identities (or a background Metaverse Object Housekeeping batch that deletes them once their grace period expires) queues a delete for every account covered by an export Synchronisation Rule whose Deprovisioning Action is Delete. Each of those now appears on the Causality Tree as a consequence of the deletion that caused it, nested beneath the MVO Deleted outcome and naming the Connected System the account is being removed from, so a leaver's execution item reads as the whole story: disconnected, Connected System Object deleted, identity deleted, and one Pending Export per downstream account being deprovisioned. They are counted in the Activity's Pending Exports total too. Deprovisioning is the most consequential thing a run can stage, and it was previously invisible on the Activity. (#1044)
- 🐛 A Pending Export execution item now shows its Pending Export's details. The panel was only rendered for export-related errors, so the informational items recorded when a run stages a membership removal or a deprovisioning delete showed nothing about the export they were reporting. The panel also now shows the Pending Export's change type alongside its status, and describes a queued deletion as such rather than as staged attribute changes. Pending Export outcomes on a Causality Tree no longer render a broken link to a non-existent identity. (#1044)
- 🐛 Executing an Example Data Template through the REST API no longer crashes with an index-out-of-range error when generating pattern-based attribute values (for example Group names); Example Data Set values are now loaded reliably regardless of which service executes the template, and a template referencing a set that genuinely has no values now fails with a clear message naming the set. (#1112)
- 🐛 Example Data generation no longer crashes intermittently while generating objects. The parallel generator shared a single random number generator across all worker threads, and that type is not safe for concurrent use; under load its internal state could be corrupted and return an out-of-range value that aborted generation partway (and could skew generated values even when it did not crash). It now uses a thread-safe shared generator.
- 🐛 The Example Data generation progress bar now advances about once a second during the object-generation phase instead of appearing frozen and then jumping. The CPU-bound parallel generation was consuming every worker thread and starving the progress reporter; generation now leaves one core free, so progress is reported throughout the phase.
- 🐛 The Operations queue progress bar for Example Data generation now advances smoothly from 0% to 100% across the whole job. Previously it swept to 100% during the in-memory generation phase and then sat frozen at 100% for the entire database-persistence phase (frequently the longest), with only the status text moving; the change-history phase reported nothing at all. Generation, change-history and persistence are now reported as one continuous progress sweep, so the bar reflects real progress throughout, including a rolling estimated time remaining while persisting.
- 🐛 The File Connector now writes Binary attribute values to export files as Base64 text instead of silently writing empty cells, both for changed values and for unchanged values carried through a full-file rewrite. The connector documentation now describes how each data type, including references and binary values, is rendered in an export cell. (#1046)
- 🐛 Import matching now works for Object Matching Rules on Long Number and Decimal attributes, such as numeric badge or employee identifiers; these previously never joined an incoming account to its existing identity, so synchronisation projected a duplicate instead. Matching rules on attribute types that cannot be matched (for example Boolean or DateTime) are now reported as a warning in the service log instead of silently doing nothing, mirroring the export-side matching behaviour. (#1046)
- 🐛 Connected System Objects now retain their partition assignment. The import process assigned the Run Profile's partition to objects (and backfilled it on matched existing objects) specifically so partition-scoped deletion detection could scope correctly, but the high-volume write paths silently discarded the value, leaving objects invisible to their partition's obsoletion sweep; such objects could never be flagged as deleted. (#1046)
- 🐛 Deletion audit records now retain Long Number, Decimal and reference values. When an identity holding these attribute types was deleted with change tracking enabled, its deletion change history recorded blank values for them. Error stack traces are now also retained when an error is attached to an already-recorded execution item during a synchronisation run. (#1046) Values beyond the 32-bit integer range (for example directory Large Integer attributes such as account expiry or last password set timestamps) were previously truncated in the audit record, showing a wrong number; the stored attribute value itself was never affected. Decimal change values recorded through the high-volume synchronisation write path are now also persisted; they were previously dropped from the audit record on that path. (#871)
- 🐛 The REST API now returns Long Number, Decimal and Binary attribute values on Connected System Objects, and Binary attribute values on Metaverse Objects and Pending Export changes; these previously returned null. Binary values are returned as Base64 text. When listing Metaverse Objects with requested attributes, every attribute type now surfaces its actual value in its natural JSON type; previously every non-Text attribute returned null. (#1046)
- 🐛 Long Number attributes now flow correctly everywhere the other data types already did. Previously: a direct inbound Attribute Flow to a Long Number Metaverse attribute failed the object's synchronisation with an unhandled error; an expression result flowing to a Long Number attribute was silently dropped; an expression result flowing to a Number attribute from a value beyond the 32-bit range was silently truncated to a corrupt value (it is now rejected with a logged warning instead); export evaluation could wrongly skip a Long Number change as "no net change" when the values actually differed, and wrongly skip exports whose only value was a Long Number, Guid or Boolean when the target system held no value; and Long Number values were invisible to expression contexts in export evaluation and Drift Correction, evaluating as absent. Values beyond the 32-bit integer range round-trip losslessly in all these paths now. (#1046)
...
- 🐛 An object that left the scope of an export Synchronisation Rule and returned to scope before the deprovision executed no longer has its live target account deleted by the stale deprovision. The Pending Delete is now cancelled when the object is evaluated back in scope, even when the returning change touches only the scoping attribute and flows to no exported attribute; a scope-out staged in the same synchronisation page keeps its Delete.
...
- ⚡ Export runs no longer load every Pending Export to find deferred references; the check is now a single indexed query. Retrying references deferred from a previous export cycle previously loaded and hydrated the Connected System's whole Pending Export set, including every attribute value change, before filtering client-side for the (usually zero) unresolved rows; at 525,000 Pending Exports this cost around 11 minutes per run even when there was nothing to resolve. (#1102)
...
- ⚡ Exported attribute values are now applied to the Connected System Object as soon as the export succeeds, instead of waiting for the confirming import to re-materialise them from the target system. This collapses the confirming import's writes for successfully exported objects (measured: 524,997 successfully exported Connected System Objects previously re-materialised 9.8 million attribute values on the following confirming Full Import) and re-arms Full Synchronisation's unchanged-object fast path sooner, since an object whose export left it fully matching no longer looks changed on the next pass. Applies to calls-based connectors (LDAP and similar); file-based connector exports are unaffected and keep re-materialising as before.
- ⚡ Full Import now skips loading and comparing objects whose content has not changed since the previous import. Each object gets a small stored content hash after a successful import; on the next run, an unchanged object's freshly computed hash matches the stored one and its attribute values are never loaded from the database or diffed, making Full Import cost proportional to the number of changed objects rather than the size of the whole connector space. The hash is always computed from the incoming data, never from what is already stored, so the optimisation only ever skips work it can prove is safe to skip; any doubt falls back to the full comparison. Transparent and automatic; see the new Verification Mode toggle above to validate it. (#1082)
