...
- 🖥️ A Run Profile execution item's detail page is now split over tabs. **Overview** carries what the run did to the item and any error it hit; **Causality** carries why it happened and what it caused, being much the largest of the three (three switchable views, an attribute drawer and the upward Caused by chain, which on a cascade pushed the execution summary off the top of the screen); and a Pending Export the item created or failed on moves to **Pending Export**, a separate subject with its own status, retry count and staged changes. The last two appear only where there is something to put in them. The page had accumulated several unrelated subjects in one scroll, which made the answer harder to find than it needed to be.
