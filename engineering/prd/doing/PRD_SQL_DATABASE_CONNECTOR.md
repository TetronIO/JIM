...
- **No support for column types with no reliable, straightforward mapping**, decided 2026-08-05 and tracked as [#1243](https://github.com/TetronIO/JIM/issues/1243): XML (`xml`, `XMLTYPE`), `sql_variant`, `hierarchyid`, `geography`/`geometry`, `INTERVAL`, `time`, `ROWID` and `BFILE`. Schema discovery skips such a column with a warning naming the Object Type, column and SQL type, and fails loudly only where the configuration depends on it. A Text mapping for XML was considered and rejected: both servers normalise XML on storage, so a value written is not the value read back, and where JIM is authoritative the confirming import would report a change every cycle and re-export forever. Supporting it properly means a structured attribute data type with semantic equality, which is a feature in its own right rather than a type-mapping row. The connector documentation must state plainly which column types are not supported and why.
...
| DECIMAL/NUMERIC/MONEY/NUMBER | Decimal | Type delivered by prerequisite #1046. Read through the driver's own decimal accessor rather than as whichever CLR type it inferred for the column; see requirement 8a |
...
8a. **Exact-numeric columns must be read as decimals, and a value beyond a decimal's range must fail comprehensibly rather than be shortened.** Two findings against the live containers (2026-08-08), both about that one mapping:

- **The driver picks the CLR type from the column's declaration, not from the value.** ODP.NET (Oracle.ManagedDataAccess.Core 23.26.300 against Oracle Database Free 23ai) materialises a `NUMBER` by its declared precision and scale: scale 0 gives Int16, Int32, Int64 or Decimal by precision, while a column with a scale gives **Single** up to precision 7, **Double** from 8 to 15, and Decimal from 16 up. A `NUMBER(5,2)` therefore arrived as a Single and a `NUMBER(10,2)` as a Double, so a Decimal-mapped column had already been through binary floating point before the Connector saw it, which is exactly the guarantee `AttributeDataType.Decimal` exists to keep. **No wrong value was demonstrated:** ODP.NET's precision bands line up exactly with what `Convert.ToDecimal` preserves from each floating-point type (7 significant digits from a Single, 15 from a Double), checked exhaustively over every 7-significant-digit value and by sampling millions of values in the Double band, and no expression in an administrator-supplied `SELECT` was found to be described with a narrow precision while carrying a wider value. That alignment is an undocumented property of one driver version rather than anything JIM is promised, so the Connector asks the driver for the column as a decimal (`DbDataReader.GetDecimal`) instead of relying on it. The accessor is ADO.NET's own, so no dialect knowledge is involved and the provider seam is unchanged. The approximate binary types are the exception: both drivers refuse `GetDecimal` for them, because they are genuinely not decimals, so those fall back to the driver's own value; that is the caveat the FLOAT/REAL row above already records.
- **A number wider than a CLR decimal is unsupported, and is refused rather than rounded.** `AttributeDataType.Decimal` is a CLR `decimal`, which holds 28 to 29 significant digits. Oracle's `NUMBER` holds 38 and Microsoft SQL Server's `decimal`/`numeric` 38, so a column declared wider than a decimal can hold values JIM cannot represent. Observed against both live containers, each driver refuses such a value inside the reader itself, before any Connector code runs, and neither message names the column or the row (ODP.NET raises `InvalidCastException: Specified cast is not valid.`, Microsoft.Data.SqlClient an `OverflowException: Conversion overflows.`). The Connector turns that into an error naming the Object Type, the column and the limit, and stating what an administrator can change: narrow the column, stop selecting the attribute, or expose the source through a view that casts it to 28 significant digits or fewer. It **fails the run** rather than reporting one object's error, because the driver refuses the value while the row is still being materialised, so there is no import object to attach an error to yet, and the column may itself be the anchor, in which case the row cannot be identified either. The value is **never rounded or truncated to fit**; a number silently shortened is precisely the corruption the Decimal type exists to prevent. This is a genuine product limitation, and the connector documentation must state it.
...
15. Creates against database-generated keys (identity/sequence columns) must return the generated value as the object's external ID in `ConnectedSystemExportResult`, following the LDAP and File Connector precedents.
    - **Natural primary keys** (decided 2026-08-07): where the key is a business identifier rather than a generated one, JIM has to author it, so schema discovery marks a table-backed object type's anchor columns `WritableOnCreate` and a Synchronisation Rule may target them. Provisioning into a table with a natural primary key (single-column or composite) is therefore supported. A view- or SELECT-backed object type's anchor stays `ReadOnly`, because neither source is writable at all, as does the attribute JIM composes for a multi-column anchor, which is a projection rather than a column.
    - **JIM never rewrites a key on update.** The value is written with the `INSERT` and never again: `SyncRuleMapping.FlowsOnUpdateExport` keeps a `WritableOnCreate` attribute out of every Update Pending Export and out of Drift Correction, and the connector refuses an Update naming an anchor column as a defence in depth, because an `UPDATE` that rewrites a primary key succeeds silently and leaves the Connected System Object anchored to a key no row holds.
    - **A business identifier that genuinely changes is an Object Matching concern, not one the connector absorbs.** Where an employee number is reissued, the anchor change presents to JIM as a delete plus an add, and an Object Matching Rule on a stable attribute is what rejoins the new Connected System Object to its Metaverse Object. Re-identifying an existing object is an administrator's decision, not something a synchronisation run should do quietly.
...
| Type-mapping round-trip | Each mapped SQL type imports and exports losslessly, including zoneless DateTime interpretation and exact-numeric Decimal round-trip. **The date and time round trip must be exercised against a Connected System whose Database Time Zone is not UTC.** At the UTC default every zone conversion is the identity, so a zone-inversion bug (import and export applying the offset the same way round rather than inverting each other) passes a UTC-configured test unnoticed; only a non-UTC zone makes the two directions distinguishable |
...
| JIM.Application | Registration with the centralised connector dispatch delivered by prerequisite #875; seeding of the new Connector Definition |
...
| Doc | Change |
|------|--------|
| `docs/connectors/jim-sql-connector.md` | New connector guide: configuration per provider, delta mode guidance (change-log table recommended; watermark limitations), type mapping including an explicit list of the column types JIM does not support and why (see the Non-Goals entry and [#1243](https://github.com/TetronIO/JIM/issues/1243)), security/least-privilege guidance, Oracle driver licence note. Must state the numeric precision limit from requirement 8a: a Decimal attribute holds 28 to 29 significant digits, so an Oracle `NUMBER` or a Microsoft SQL Server `decimal`/`numeric` declared wider than that fails the import rather than rounding, and what an administrator does about it. Must also state that JIM is open to supporting a wider range of database servers (PostgreSQL, MySQL/MariaDB and others) when there is demand, inviting feedback via the [Ideas category of GitHub Discussions](https://github.com/TetronIO/JIM/discussions/categories/ideas) |
...
**Sequencing decision (2026-07-15): the following three issues are hard prerequisites and will be implemented before the SQL connector**, so the connector is built on the final foundations rather than reworked after them:

- **#875 (centralise connector dispatch)**: the connector registers with the centralised dispatch this delivers, rather than adding a third copy of every hand-coded name-based branch.
- **#637 (connector sub-phase progress)**: the connector narrates its long-running operations from day one ("Executing query", "Reading rows", "Writing rows") via the callback pattern this delivers.
- **#1046 (Decimal attribute data type, child issue of #170)**: the connector maps exact-numeric columns to Decimal natively from day one, avoiding a Text-to-Decimal mapping migration on live Connected Systems.
