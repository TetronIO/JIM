...
| `Scenario18-WritebackToSource` | Whether a value JIM derives is written back into the Connected System it came from. Two JIM File Connector systems carry identically shaped outbound rules over the same Metaverse Objects, evaluated in one run, differing only in whether the target is the run's source. **Expected to FAIL until [#1284](https://github.com/TetronIO/JIM/issues/1284) is resolved**, and excluded from `-Scenario All` for that reason; `-Template` is ignored | file (no directory container) |
