...
### OpenLDAP: Two Bind Identities

`openldap-primary` is bound as two different identities depending on who is asking, and the distinction matters: it is what makes the lab prove JIM works under the permissions a customer would actually grant it, rather than as an unrestricted rootDN.
...
- **Samba AD has no equivalent delegation yet**: `JimBindDN`/`JimBindPassword` there currently equal the domain Administrator's `BindDN`/`BindPassword`, so JIM still binds as Administrator against Samba AD. Delegating that bind to a least-privilege account is tracked as follow-up work ([#1716](https://github.com/TetronIO/JIM/issues/1716)), not done here.
