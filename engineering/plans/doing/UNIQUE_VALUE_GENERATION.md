...
- **Last Updated:** 2026-09-25 (Phase 4 delivered: Scenario 023, developer guide and causality reference; three further defects Scenario 023 found fixed); 2026-09-25 (decision 5 revised: connector-space adoption removed in both modes, replaced by the priority model plus Metaverse-Object-only adoption in import mode; found by Scenario 023's integration run, decided by the product owner; engine change and docs land in Phase 4); 2026-09-24 (Phase 3 work package C: PowerShell surface for generated mappings); 2026-09-24 (Phase 3 work package A: server-side generation validation, application methods and REST surfaces); 2026-09-23 (decision 6 revised: inline per-object resolution with run-scoped caches); 2026-09-19 (adversarial review applied; four-release delivery; uniqueness tokens, counters and the retired values register added)

## Overview

JIM cannot generate the identifiers an IDAM team owns (account name, employee number, email, UPN); the demo data hides that by putting them in the HR feed. This plan delivers, in four releases: a "Generated Value" source type on import and export Attribute Flows whose value is a base expression plus a **uniqueness token** (a number only if the value is taken, a forward-only **sequence** with optional fixed width, or a **random** token); a unique value service with four availability gates; a JIM-owned sticky **generated value assignment** per object and attribute; a **retired values register** so values are never re-issued; probing of the target directory; and **Collision Remediation**, which revises JIM's own assignment when a target rejects a value as in use, unless the value is anchored by another target, in which case an administrator is given a decision with three exits. Everything surfaces through the existing causality model.
...
- Demo credibility and real-deployment fit: HR feeds stop carrying IT-owned attributes (Scenario 001 becomes the canonical example).
...
- **Integration harness.** Scenario scripts under `test/integration/scenarios/`, highest is 22 (Scenario 022, OpenLDAP password policy, landed after this plan was written). `Generate-TestCSV.ps1` emits `first.last<Index>` for every user's `samAccountName` with no switches to omit columns; `Invoke-Scenario-001` asserts on that exact value in six places and keys CSV edits on the `samAccountName` column. The OpenLDAP image has no `unique` overlay; there is no under-privileged bind precedent.
...
5. **Adopt before generate, sourced from the Metaverse Object only, import mode only.** *Revised 2026-09-25 (Scenario 023, decision by the product owner):* the original design (below) is superseded. Before generating, the service now checks only whether the **Metaverse Object itself** already holds a value for the attribute (typically left behind by a higher-priority contributor that has since withdrawn); if so, the assignment is created Committed with that value and nothing is generated. Export mode has no adoption at all: with no assignment, generation runs and the value is exported like any export Attribute Flow. Connector-space adoption is removed: it is no longer, by itself, a reason to skip generation, and the Attribute Flow priority model is the only precedence. The brownfield-join case the review originally cited is now handled by an administrator adding an import Attribute Flow from the target at higher priority than the generated flow, which supersedes generation under ordinary priority resolution rather than through a bespoke adoption check (PRD decision 15, revised). *Original design (superseded, kept for context):* the service checked whether the object already held a value for the attribute that a participating target had accepted (import mode: any joined non-excluded Connected System Object holding it for the attribute an export mapping flowed it to; export mode: the Connected System Object holding it). Scenario 023 found this read the object's joined Connected System Objects even where no import Attribute Flow from that system existed, a precedence rule outside the Attribute Flow priority model, and that it only worked when the join already existed before generation ran, so the common brownfield case (HR projects a person, an Object Matching Rule then joins an existing directory account in the same synchronisation) exported the generated value over the existing account's value.
...
18. **Configuration surfaces come before the connector work.** Integration scenarios configure JIM through PowerShell and REST, and Scenario 001's conversion needs only the local gates.
...
Rules that keep deletion safe: an object's own current value is always free for it (every gate excludes the requesting object); adopt before generate, for a value the Metaverse Object itself already holds, in import mode (decision 5, revised 2026-09-25); missing knowledge never permits a rename (decision 10). The counter is unaffected by any of this (decision 3).
...
- `ResolveAsync(requests, options)`: adopt-existing check against the Metaverse Object's own held value (import mode only; decision 5, revised 2026-09-25), sticky short-circuit, then candidates through the gates; exhaustion and width failures named per object; dry-run for previews.
...
3. PowerShell: `New-JIMSyncRuleMapping` gains `ImportGenerated` and `ExportGenerated` parameter sets (`-Generate`, `-TokenKind`, `-SuffixStyle`, `-SuffixStart`, `-SequenceStart`, `-SequenceIncrement`, `-FixedWidth`, `-OnWidthExceeded`, `-RandomFormat`, `-RandomLength`, `-Separator`, `-AttemptLimit`, `-NeverReuse`); `Set-JIMSyncRuleMapping` the same as nullable settings; `Get-JIMGeneratedValue` (per Metaverse Object), `Get-JIMGeneratedValueSequence`, `Restart-JIMGeneratedValues` (Start again, `-Confirm` by default); Pester in `src/JIM.PowerShell/Tests/`.
4. `ConnectedSystemServer` mapping create and settings-update validation: target type versus token (Number targets accept only sequence or random digits, no base expression, no padding; the reason is returned to every surface); `NeverReuse` forced on for sequence; a start value raised above the counter moves it and is reported in the save response so the portal can confirm; a lower one is accepted and has no effect.
...
1. `Invoke-Scenario-023-UniqueValueGeneration.ps1` (setup, run, teardown through `Run-IntegrationTests.ps1`) covering the PRD's Modes, Strategy, Tokens, Never reuse (sequence only in this release), Existing accounts (a higher-priority import Attribute Flow keeping a brownfield join's value under ordinary priority resolution, and a value adopted from the Metaverse Object itself after a higher-priority contributor withdraws), Start again, Failure, Stability and Surface parity rows, plus the local-gate rows of Gates; the mixed-capability case uses the SQL connector with a unique constraint, because a CSV target cannot reject a duplicate. Running this scenario against the release-1 engine is what surfaced the connector-space adoption gap; see the dated note below.
2. `Generate-TestCSV.ps1` gains `-OmitItOwnedAttributes` touching its three emission sites, folded into the cache key in `Get-OrGenerate-TestCSV.ps1` / `Test-CsvCache.ps1`. `Setup-Scenario-001.ps1` replaces the `samAccountName → Account Name` import mapping with a generated one; `Invoke-Scenario-001` is rewritten to derive expected account names from `Lower(first).Lower(last)` with JIM's suffix rule and to key CSV edits on `employeeId` (six assertion sites and the CSV-edit sites listed in Current state). Email and UPN stay as they are until release 2.
...
**Delivered (2026-09-25), and where it differs from the three items above.**
- Item 1: Scenario 023 is in, green on OpenLDAP and Samba AD. It does not cover a target-side collision (the mixed-capability SQL case and a Samba AD `sAMAccountName` clash alike): in release 1 that is an ordinary export error, existing export behaviour rather than generation, and the harness cannot yet host a deliberate export failure (the LDAP Connector logs the rejection at Error level and the runner's end-of-run log scan fails on any Error line). Both move to release 4, where Collision Remediation reworks that path; decided by the product owner 2026-09-25.
- Item 2: `-OmitItOwnedAttributes` and an opt-in `Setup-Scenario-001.ps1 -GenerateAccountName` are in; Scenario 023 uses both. Scenario 001 itself is not converted: its setup is shared by Scenarios 004 to 007, 017, 020, 021 and 022, and the canonical scenario should not depend on an In development feature flag. It converts when the flag is removed, tracked by [#1803](https://github.com/TetronIO/JIM/issues/1803).
- Item 3: the developer guide section (`engineering/DEVELOPER_GUIDE.md` 3d) and the causality reference (`engineering/CAUSALITY_REFERENCE.md`) are in. The `CHANGELOG.md` entries are withheld until the flag is removed (#1803).
- Also found by Scenario 023 and fixed in this phase: a generated export change merged into a drift-staged Pending Export was left unresolved and failed the page at the integrity guard; a stale assignment was reasserted after a higher-priority Attribute Flow that had taken the attribute over was withdrawn, renaming the account back (an import-mode assignment that no longer matches the object's value is now treated as stale); and a feature-disabled refusal was logged as an unhandled Error.

**2026-09-25 (decision by the product owner, from Scenario 023's integration run).** Connector-space adoption is removed in both modes: it read the object's joined Connected System Objects even where no import Attribute Flow from that system existed, a precedence rule outside the Attribute Flow priority model, and it only worked when the join already existed before generation ran, so the common brownfield case exported the generated value over an existing account's value. Decision 5 above and the review finding 2 table entry are revised accordingly. The engine change, adopting only from the Metaverse Object's own held value in import mode and never adopting in export mode, lands in this phase's PR together with Scenario 023's "Existing accounts" cases above and the docs it drives (the public Generated values section, the new Initialising JIM guidance, the developer guide and the causality reference). See PRD FR 30 and the PRD header's Updated line for the full rationale.
...
2. Scenario 001 completed: Email and UPN derived from Account Name; `Invoke-Scenario-001` expectations for email and UPN derived the same way. Scenario 022 gains the Never reuse rows for name-based and random tokens and the derived-values rows.
...
0. Exclusions (deferred from Phase 3, 2026-09-24): the participating-systems panel on the generated mapping form, the `exclusions` list on the mapping DTOs, `-ExcludeConnectedSystemId` on the mapping cmdlets, and validation that each excluded system participates.
...
5. Harness: an ACL-restricted bind account in the Samba image (`samba-tool dsacl`) and the OpenLDAP `unique` overlay in the OpenLDAP image, so the under-privileged bind and OpenLDAP mail uniqueness cases are producible. Scenario 022 gains the probe rows.
...
6. Tests: attribution table; remediation revise-and-flag; re-staging with recomputed dependants; anchored; unknown after clear; authorised rename; switch off; unattributable; `Parked` ignored by bulk retry; edge written at queueing; parallel batch safety. Runtime: Samba AD with a conflicting account created after the probe. Scenario 023 gains the target-side collision cases deferred from release 1 (Phase 4): a Samba AD `sAMAccountName` clash, remediation off recording an ordinary export error, and the mixed-capability contrast. First the harness must accept an intended export failure: a way for a scenario to declare an expected Error line that both the live watcher (`Start-JimErrorWatcher`) and the end-of-run `Assert-NoWorkerErrors` ignore, or a decision that a target's rejection of one object is logged below Error.
...
4. Scenario 022 gains the Collision Remediation and Anchoring rows; connector pages and capability matrix; `CHANGELOG.md` `✨` entry for Collision Remediation; PRD and plan move to `done/` when the issue closes.
...
| 2 | Ordinary generation renames live accounts | Decision 5; Phase 2's connector-space adoption turned out not to close this for the common brownfield case (Scenario 023, 2026-09-25); re-addressed by the priority model plus Metaverse-Object-only adoption, landing in Phase 4 |
...
| 11 | Scenario 001 conversion larger than stated | Phase 4 |
| 12 | Probe claims overstated | Decision 14; Phase 7 |
| 13 | Scenario 022 cases the harness cannot produce | Phases 4, 7 |
...
- Scenario 001 converted (Account Name in release 1; Email and UPN in release 2) and green; Scenario 022 green for each release's rows; the full suite green.
