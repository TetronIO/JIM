...
- ⚡ The tail of a large export no longer crawls through groups with very large membership change sets. Post-export bookkeeping ran change detection once per changed value, which is quadratic in the number of changes; a batch containing a 100,000-member group spent over ten minutes in it while the progress display showed seconds remaining. The bookkeeping now detaches the batch in a single pass. This also fixes an export batch failure ("another instance with the same key value is already being tracked") that could abort a batch after the target system write had already succeeded.
