...
- **An object changed several times between two imports was imported several times.** The changelog records every write, and the first lab run of Scenario 8 failed with a duplicate external id on the Source Delta Import. The changelog source now fetches an object once per read, on its first record, as the accesslog source already did; deletions are never skipped.
- **The harness built its own `ldapmodify` and `ldapdelete` commands from the Connected System port.** Samba AD and OpenLDAP never showed it because their two ports coincide; on 389 (LDAPS 3636 for JIM, plain 3389 inside the container) the commands hung. Every such site now uses the in-container port and scheme, and a Pester test scans the scenario scripts for the pattern.
- **A global password policy is baked into the fixture** (`passwordCheckSyntax on`, `passwordMinLength 7`), mirroring the OpenLDAP lab's `pwdMinLength 7`, so the password scenarios have something to refuse and JIM's 389 policy discovery reads a real policy. The fixture also raises `passwordMinTokenLength` from 389's default of 3 to 5, because syntax checking refuses any password sharing a three-character run with the entry's own name or mail, and the scenarios' static `Chalkstream-7-Vault!` was refused for `charlie.mathews10` ("Cha" from `givenName: Charlie`) while its neighbours were accepted; the build now checks that the static password passes for an entry named Charlie and that `Charlie-7-Vault!` is still refused.
