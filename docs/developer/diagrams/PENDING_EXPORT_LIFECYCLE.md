...
- **Optimistic export apply** (#1079)<br /> On export success, `ExportExecutionServer.ProcessBatchSuccessAsync` projects the exported attribute changes onto the CSO's attribute values (add-if-absent, replace-on-Update, delete-if-present, delete-all) at export time rather than waiting for the confirming import to re-materialise them. This means the confirming import's "Compare each import attribute against existing CSO values" step (see [Full Import Flow](FULL_IMPORT_FLOW.md)) usually finds nothing to do, avoiding millions of redundant attribute value inserts at scale. Two rules keep this safe:
  ...
  Reference-typed attribute changes (for example a group's member list) usually arrive at export already resolved to a Distinguished Name string, without the in-run transient hint that reference resolution stamps for deferred exports. Confirming that Distinguished Name against a Connected System Object Id therefore needs a database lookup, and a first version of this fallback queried the database once per batch: at Scale500k25kGroups (2026-07-21) this cost 10-15 seconds per call over an unindexed scan, 500+ calls across one export run. The fallback is now a single whole-Connected-System lookup (secondary external Id value to Connected System Object Id, case-insensitive), built once per export run the first time any batch needs it and reused - including across parallel batches - for the rest of the run.
