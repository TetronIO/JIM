...
<#
.SYNOPSIS
    Configure JIM for Scenario 019: Auxiliary Classes
...
.DESCRIPTION
    ...
    auxiliary class testing (#492), following Scenario 014's two-suffix shape: Source
    (Yellowstone) imports and projects, Target (Glitterband) exports.

    Creates:
    - "Scenario 019 Source" Connected System, base DN = dc=yellowstone,dc=local
    - "Scenario 019 Target" Connected System, base DN = dc=glitterband,dc=local
    ...
    - A Target Export Sync Rule with NO mappings yet: the badge attribute mappings can only
      ...
      Type and refreshed its schema, so Invoke-Scenario-019-AuxiliaryClasses.ps1 adds them there
    ...
    -DirectoryType request for Scenario 019 before this script is ever invoked.
...
.PARAMETER DirectoryConfig
    ...
    which OpenLDAP instance is passed in, mirroring Setup-Scenario-014.ps1.

.EXAMPLE
    ./Setup-Scenario-019.ps1 -ApiKey "jim_abc123..."
#>
...
if ($DirectoryConfig.DirectoryType -ne "OpenLDAP") {
    throw "Scenario 019 (Auxiliary Classes) is OpenLDAP only. Run-IntegrationTests.ps1 should have rejected this combination before Setup-Scenario-019.ps1 was invoked."
}

# Re-derive Source (Yellowstone) and Target (Glitterband) configuration independently of
# whichever single OpenLDAP instance was passed in, mirroring Setup-Scenario-014.ps1.
...
Write-TestSection "Scenario 019 Setup: Auxiliary Classes"
...
foreach ($badgeAttrName in @("Badge Number", "Badge Colour")) {
    ...
    New-JIMMetaverseAttribute -Name $badgeAttrName -Type Text -AttributePlurality SingleValued `
        -ObjectTypeIds @($mvUserType.id) -ChangeReason "Scenario 019 auxiliary class testing (#492)" -Confirm:$false | Out-Null
...
# Summary
# ============================================================================
Write-TestSection "Scenario 019 Setup Complete"
