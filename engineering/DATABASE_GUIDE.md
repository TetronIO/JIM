...
**Every service waits for the server first.** Before its first database call, each of JIM.Worker, JIM.Web and JIM.Scheduler calls `JimApplication.WaitForDatabaseAsync` (`DatabaseStartupWait`), which retries `IRepository.TryConnectAsync` for up to five minutes and logs each attempt, so no service depends on its supervisor starting PostgreSQL first. `TryConnectAsync` retries only what waiting can fix (`NpgsqlException.IsTransient`) and throws anything else, such as rejected credentials, at once. A server with no JIM database yet counts as connected, because the migration above creates the database.
