...
**Outbound ✅ implemented (commit `aa75402e`) + integration-verified (Scenario 13).** Driver: **fold into the shared sync base** (both Full and Delta), honoured on the next sync of a system. `ProcessScopeReviewPendingMetaverseObjectsAsync` runs after the CSO page loop: drains `ScopeReviewPending` MVOs in batches, gives each an RPEI (linked via `_mvoIdToRpei`), feeds them to the existing export path via `_pendingExportEvaluations` with **empty `changedAttributes`** (confirmed correct against `CreateAttributeValueChanges`: falls back to the MVO's current values for a Create/provision, Update-with-no-change yields no Pending Export, out-of-scope deprovision is scope-driven), reuses the per-page flush sequence (`EvaluatePendingExportsAsync` → `FlushPendingExportOperationsAsync` → `ResolvePendingExportReferenceSnapshotsAsync` → `FlushRpeisAsync` → `ClearChangeTracker`), then clears the flag. AsNoTracking MVOs are safe: provisioning CSOs reference the MVO by FK scalar only and persist via raw SQL. Whichever system's sync runs first handles its own export rules; a still-mismatched MVO is re-flagged by the next reconciler sweep (eventually consistent; single-system case has no ping-pong). Repo methods `GetMetaverseObjectIdsWithScopeReviewPendingAsync` / `ClearMetaverseObjectScopeReviewPendingAsync` (exposed via `ISyncRepository`), MVO loader now includes `Type`, and a partial index `IX_MetaverseObjects_ScopeReviewPending` keeps the flagged-MVO scan O(transitions).
  - **Reference attributes: covered (previous "known limitation" closed).** Flagged MVOs still load without the `ReferenceValue` navigation, and deliberately so: including it would join a full Metaverse Object row per reference (a large group would drag its entire membership), and the FK scalar is all export needs. Export evaluation is FK-scalar-safe on both flow types: direct flows already fell back to `ReferenceValueId` in `CreateAttributeValueChanges`, and expression flows now do too (`BuildAttributeDictionary` previously read only the navigation, so an expression over a reference attribute silently evaluated to null on the reconciler path; fixed red-first). Unit-proven for the reconciler shape (`ReconcilerReferenceExportTests`: single-valued and multi-valued direct flows, expression flow, and the tracked/navigation shape pinned against regression) and integration-proven by Scenario 13's Manager reference flow (the reconciler-provisioned joiner's exported row carries the referenced Metaverse Object's ID via an `mv["Manager"]` expression mapping; the expression form is used because direct Reference-to-Text mappings are rejected by type validation and a CSV column cannot be typed as Reference).

**Verification status:** Phase 4a + inbound 4c are integration-verified by Scenario 12 T4 (real stack, clean reset + rebuild): the reconciler schedule runs to Completed and a subsequent sync disconnects a now-out-of-scope object with no data change. Outbound 4c is integration-verified by Scenario 13 (real stack, clean reset + rebuild): an export rule scoped on a Metaverse relative date holds a joiner's downstream provisioning, a plain sync after the boundary provisions nothing (the hot path misses the static-data transition), and after the reconciler flags the Metaverse Object the next sync provisions it downstream, all with no Metaverse Object data change. The control object is untouched throughout, and the joiner's exported row carries its Manager reference (the control's Metaverse Object ID via an `mv["Manager"]` expression mapping; the reference-attribute proof for the reconciler-loaded, navigation-free Metaverse Object shape).

### Phase 5: API / UI (built-in protection) ✅
- Enable/disable and interval/pattern change on the built-in Schedule are allowed; rename, delete and step changes are blocked. Delete and step mutations are guarded authoritatively in the application layer (`SchedulerServer.DeleteScheduleAsync` throws on `BuiltIn`; `CreateScheduleStepAsync`/`UpdateScheduleStepAsync`/`DeleteScheduleStepAsync` throw when the parent schedule is `BuiltIn`), covering every caller including the portal (which calls the application layer directly). Rename is guarded at the API controller (the sole rename write-path, where the pre-mutation name is in hand): `UpdateAsync`/`DeleteAsync` return 400 for a built-in rename/delete, and `UpdateAsync` returns 400 for a built-in step add/remove/replace (structural diff on step Ids) while skipping step reconciliation entirely for built-in schedules (so field-level edits are ignored too). The portal mirrors all of it: the schedule editor makes a built-in schedule's name read-only and its Steps tab read-only (add/edit/reorder/delete hidden, explanatory alerts), the list replaces its delete action with a lock, and `SaveAsync` skips step reconciliation for built-in schedules so the application-layer step guard is never tripped on a legitimate re-time. Unit-tested (`SchedulerServerBuiltInGuardTests` for the application-layer delete + step throws; `SchedulesControllerTests` for the rename/delete/step 400s and the allowed re-time/enable case).
