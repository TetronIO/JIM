...
- 🐛 Executing the bundled "Users & Groups" example data template no longer hangs the worker for minutes on a single CPU core, nor crashes it on memory-constrained hosts. The persistence path previously pushed the entire generated dataset (10,500 MVOs plus attribute values and change-history records for the bundled template) through EF Core's change tracker in a single `SaveChanges` call, producing tens of thousands of serialised parameterised INSERTs and a large working-set spike that could starve the worker's healthcheck and trigger a Docker restart. Persistence has been rewritten to use PostgreSQL `COPY` binary import (mirroring the proven pattern used on the synchronisation hot path), sliced into batches that honour the configured Sync Page Size, so memory stays bounded and write throughput is dominated by disk/WAL rather than driver chatter.
...
[Unreleased]: https://github.com/TetronIO/JIM/compare/v0.10.1...HEAD
