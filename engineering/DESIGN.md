...
- **Edge labels keep 12 user units of clearance from every edge, measured box-to-line, not anchor-to-line.** An SVG `<text>` `x`/`y` is an anchor point *on the baseline*, but what collides with an edge is the rendered box: at the 12.5px label size that box reaches roughly 11.6 units above the baseline and 3.4 below. On a horizontal or vertical edge the gap is the same at every height, so placing by eye works; on a **diagonal** edge the line travels sideways across the label's height, so an anchor that looks clear at the baseline is crossed by the line at cap height. Offset diagonal labels **along the edge's normal**, and check the box *corner* nearest the line (for an edge through `(x1,y1)` with unit direction `(ux,uy)`, a corner `(cx,cy)` is `|(cx-x1)*uy - (cy-y1)*ux|` away). Where a label cannot reach 12 without colliding with a neighbouring edge, slide it along its own edge to where the fan opens out rather than shrinking the gap.
- **Mirrored geometry keeps mirrored labels.** When two edges are reflections of each other about the diagram's centre line, place their labels at reflected coordinates too (`x` and `1160-x`, same `y`, opposite `text-anchor`); asymmetry in a symmetric composition reads as an error.
