...
**A `JimApplication` Is One Unit of Work; Never Decide From One That Outlives It:**
- On the Worker's `TrackAll` context, a query that returns an entity already in the identity map returns the tracked instance **as it first stood**; EF Core does not refresh a tracked entity's values from a later query. That is correct and desired inside one task (a sync run applies the rules it read at its start for the whole run), and wrong the moment a context outlives the unit of work: housekeeping evaluated every batch on the main loop's `JimApplication`, so an export Synchronisation Rule switched from Delete to Disconnect still had its directory objects deleted until the Worker restarted (PR #1689, found by Scenario 004).
- Create the instance for the unit of work (`using var jim = _jimFactory.Create()`), decide and write on it, dispose it. The main loop's own instance is for dequeueing, heartbeats and cancellation only: every decision it makes is a SQL predicate or an `ExecuteUpdate`, never a field read off a tracked copy, and the idle tick clears its tracker (`Worker.PerformIdleTickAsync`) so it does not retain every task it has ever run.
- Do **not** answer this with `AsNoTracking()` on the read paths. The Worker tracks by default because sync loads overlapping graphs and relies on identity fixup; untracked duplicates of shared entities (attributes, object types) collide on write, which is the failure `TrackAll` was adopted to end. Fix the lifetime, not the tracking.
...
|-------------|-------------------------|----------------------------------------------------------------------------------------|-------------------------------------|--------------------------------------------------------------------------|
| **Summary** | `GetXxxSummaryAsync`    | Minimal scalar projection (a DTO of a handful of fields). No entity materialisation.   | `MetaverseObjectRecallSummary`      | High-scale filtering and reconciliation (100K+ objects).                 |
...
**Released migrations are frozen, and the freeze is machine-enforced.** `src/JIM.PostgresData/Migrations/released-migrations.lock` lists every migration that has shipped in a release (id + content hashes), appended by `scripts/Update-ReleasedMigrationsManifest.ps1` during `/release`. A listed migration has been applied to customer databases: `ReleasedMigrationImmutabilityTests` (JIM.Worker.Tests) fails the build if one is renamed, regenerated, edited or deleted, or if a new migration's timestamp sorts before the newest listed id. Everything in the next paragraph (regenerating after `main` moves) is therefore lawful ONLY for migrations not yet in the manifest; once listed, the only way forward is a NEW migration that corrects the schema additively.
