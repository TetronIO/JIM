...
- [ ] (Out of attribute-priority scope; deferred to a holistic **Guardrails** capability) Mass-change protection for `NullIsValue`-driven clears is one special case of a broader concern (mass clears, updates, deletes, exports). Do **not** build a one-off fence inside attribute priority; tracked as its own GitHub issue
