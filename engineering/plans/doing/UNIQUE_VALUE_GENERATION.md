...
3. PowerShell: `New-JIMSyncRuleMapping` gains `ImportGenerated` and `ExportGenerated` parameter sets (`-Generate`, `-TokenKind`, `-SuffixStyle`, `-SuffixStart`, `-SequenceStart`, `-SequenceIncrement`, `-FixedWidth`, `-OnWidthExceeded`, `-RandomFormat`, `-RandomLength`, `-Separator`, `-AttemptLimit`, `-NeverReuse`); `Set-JIMSyncRuleMapping` the same as optional settings; `Get-JIMGeneratedValue -MetaverseObjectId` (pipeline-friendly), `Get-JIMGeneratedValueSequence`, `Restart-JIMGeneratedValues` (Start again, `SupportsShouldProcess`/`ConfirmImpact='High'`, confirms by default); Pester in `src/JIM.PowerShell/Tests/`. **Delivered without `-ExcludeConnectedSystemId`** (deviating from this bullet's original text): exclusions are a release 4 surface and are deliberately absent from every configuration surface in release 1, matching the REST DTOs (`SyncRuleMappingGenerationDto`'s own doc comment) and the portal form.
