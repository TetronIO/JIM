...
// **************************************************************************************
// Tetron Identity Manager - Sync Service
//
// Needs to:
...
using Serilog;
InitialiseLogging();
Log.Information("TIM.Service - Hello World!");

try
{
    while (true)
    {
        Log.Information("TIM.Service - Doing some work!");
        Thread.Sleep(2000);
    }
}
catch (Exception ex)
...
static void InitialiseLogging()
{
    ...
    loggerConfiguration.WriteTo.File(Path.Combine(loggingPath, "tim.service..log"), rollingInterval: RollingInterval.Day);
