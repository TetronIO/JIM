...
<#
.SYNOPSIS
    Test Scenario 019: Auxiliary Classes (merge, import, export class convergence, discovery)
...
.DESCRIPTION
    ...
    Topology (configured by Setup-Scenario-019.ps1, seeded by Populate-OpenLDAP-Scenario-019.ps1),
    following Scenario 014's two-suffix shape: "Scenario 019 Source" (dc=yellowstone,dc=local)
    imports and projects; "Scenario 019 Target" (dc=glitterband,dc=local) exports. Six users
...
.PARAMETER Template
    ...
    user set (see Populate-OpenLDAP-Scenario-019.ps1) and ignores the template.
...
.PARAMETER SkipPopulate
    Skip re-seeding OpenLDAP. Scenario 019 is excluded from OpenLDAP snapshot handling in
...
.EXAMPLE
    ./Invoke-Scenario-019-AuxiliaryClasses.ps1 -Step All -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-019-AuxiliaryClasses.ps1 -Step Import -ApiKey $env:JIM_API_KEY
#>
...
if ($DirectoryConfig.DirectoryType -ne "OpenLDAP") {
    throw "Scenario 019 (Auxiliary Classes) is OpenLDAP only. Run-IntegrationTests.ps1 should have rejected this combination before this script was invoked."
}

$sourceSystemName = "Scenario 019 Source"
$targetSystemName = "Scenario 019 Target"

# Re-derive Source (Yellowstone) and Target (Glitterband) directory configuration independently
# of whichever single OpenLDAP instance was passed in, mirroring Setup-Scenario-019.ps1: the
...
function Invoke-Scenario19LdapAdd {
    <#
    .SYNOPSIS
        Runs an LDIF payload through ldapadd against a Scenario 019 OpenLDAP suffix, following
        the established temp-file + docker-exec pattern (Populate-OpenLDAP-Scenario-019.ps1).
    #>
    param(
...
function Get-Scenario19LdapAttributeValues {
    <#
    .SYNOPSIS
        Reads EVERY value of an attribute straight from a Scenario 019 OpenLDAP suffix, bypassing
        JIM.
...
Write-TestSection "Scenario 019: Auxiliary Classes"
...
try {
    ...
    if (-not $SkipPopulate) {
        ...
        & "$PSScriptRoot/../Populate-OpenLDAP-Scenario-019.ps1"
    ...
    Write-Host "Running Scenario 019 setup..." -ForegroundColor Gray
    & "$PSScriptRoot/../Setup-Scenario-019.ps1" -JIMUrl $JIMUrl -ApiKey $ApiKey -Template $Template -DirectoryConfig $DirectoryConfig
    Write-Host "  OK JIM configured for Scenario 019" -ForegroundColor Green

    # Re-import module for a live connection after Setup-Scenario-019.ps1 ran in a separate invocation.
    ...
    if (-not $sourceSystem -or -not $targetSystem) {
        throw "Connected Systems not found. Ensure Setup-Scenario-019.ps1 completed successfully."
    }
    ...
    if (-not $sourceFullImport -or -not $sourceFullSync -or -not $targetFullImport -or -not $targetFullSync -or -not $targetExport) {
        throw "Required Run Profiles not found. Ensure Setup-Scenario-019.ps1 completed successfully."
    }
    ...
    if (-not $sourceImportRule -or -not $targetExportRule) {
        throw "Required Sync Rules not found. Ensure Setup-Scenario-019.ps1 completed successfully."
    }
    ...
    if (-not $mvBadgeNumber -or -not $mvBadgeColour) {
        throw "Badge Metaverse attributes not found. Ensure Setup-Scenario-019.ps1 completed successfully."
    }
    ...
    if ($lastStepIndex -ge 3) {
        ...
        # cross-system priority fallback Scenario 014 proves). The carriers keep winning with the
        ...
        # same recall shape Scenario 014's WithdrawalReElection proves) and the refused Pending
...
if ($testResults.Success) {
    Write-Host ""
    Write-Host "OK All Scenario 019 tests passed!" -ForegroundColor Green
    exit 0
}
else {
    Write-Host ""
    Write-Host "FAIL Some Scenario 019 tests failed" -ForegroundColor Red
    exit 1
}
