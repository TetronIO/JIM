...
<#
.SYNOPSIS
    Test Scenario 015: SCIM 2.0 Client Connector
...
.EXAMPLE
    ./Invoke-Scenario-015-ScimConnector.ps1 -Step All -ApiKey "jim_..."
#>
...
Write-TestSection "Scenario 015: SCIM 2.0 Client Connector"
...
function Write-HrCsv {
    ...
    $localCsvPath = Join-Path ([System.IO.Path]::GetTempPath()) "scenario-015-hr-users.csv"
    Set-Content -Path $localCsvPath -Value ($csvLines -join "`n") -NoNewline
    Write-FileToConnectorVolume -SourcePath $localCsvPath -DestinationPath "/connector-files/test-data/scenario-015-hr-users.csv"
}
...
try {
    if ($Step -eq "All") {
        ...
        & "$PSScriptRoot/../Setup-Scenario-015.ps1" `
    ...
    if (-not $hrSystem) { throw "Connected System '$hrSystemName' not found. Run Setup-Scenario-015.ps1 first." }
    if (-not $scimSystem) { throw "Connected System '$scimSystemName' not found. Run Setup-Scenario-015.ps1 first." }
    ...
    Write-TestSection "Scenario 015 Complete"
    ...
    Write-Failure "Scenario 015 failed: $($_.Exception.Message)"
