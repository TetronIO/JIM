...
<#
.SYNOPSIS
    Test Scenario 010: Sync Rule Scoping Behaviour
...
.DESCRIPTION
    ...
    (Reset-JIMSystem followed by Setup-Scenario-010) so the assertions are not polluted
    ...
    Coverage that lives in other scenarios (referenced, not duplicated here):
      - WhenAuthoritativeSourceDisconnected leaver cascade -> Scenario 001, Scenario 004
      - MVO deletion rule trigger from CSO disconnect      -> Scenario 004
...
.EXAMPLE
    ./Invoke-Scenario-010-SyncRuleScoping.ps1 -Step All -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-010-SyncRuleScoping.ps1 -Step InboundExitDisconnect -ApiKey "jim_..."
#>
...
if ($Template -in $longTailTemplates) {
    throw "Template '$Template' is only valid for Scenario 008. Use 'Nano' for Scenario 010."
}
...
# for whichever lab is in use; Samba AD keeps its "(AD)" label. Setup-Scenario-010 and the scenario build
...
Write-TestSection "Scenario 010: Sync Rule Scoping Behaviour"
...
function Reset-JIMForCascadeTest {
    ...
    # runner only trusts the directory's CA once, at environment setup. Setup-Scenario-010 validates the
    ...
    Write-Host "  Re-running Setup-Scenario-010..." -ForegroundColor Gray
    & "$PSScriptRoot/../Setup-Scenario-010.ps1" `
        -JIMUrl $JIMUrl `
        -ApiKey $ApiKey `
        -Template $Template `
    ...
    # Setup-Scenario-010 removes and re-imports the module, so we need to do the same
    ...
    # Finance -> Sales) actually flow to the MVO. With the default Setup-Scenario-010 config
...
try {
    ...
    # Run Setup-Scenario-010 to configure JIM (idempotent: cleans up first)
    Write-Host "Running Scenario 010 setup..." -ForegroundColor Gray
    & "$PSScriptRoot/../Setup-Scenario-010.ps1" `
        -JIMUrl $JIMUrl `
        -ApiKey $ApiKey `
        -Template $Template `
...
if ($testResults.Success) {
    Write-Host ""
    Write-Host "OK All Scenario 010 tests passed!" -ForegroundColor Green
    exit 0
}
else {
    Write-Host ""
