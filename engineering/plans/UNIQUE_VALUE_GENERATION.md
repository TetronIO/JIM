...
1. Portal (`SyncRuleAttributeFlowTab.razor`, inline dialog): a third "Source Type" item, "JIM generates it", with the form in the Generated Value Options mockups: target, base expression (existing editor and `ExpressionTester`), uniqueness token radio group with the sub-controls per kind (style, start, separator; start, increment, fixed width with room remaining, overflow behaviour; random format and length with value-space hint), "Never reuse a value" switch (on), attempt limit (advanced), live preview, sequence state panel with "Set next number" confirmation; `Generated` row chip in both layouts with a `GetMappingTypeChipColour` case. Identity page (`MvoDetailsTable.razor` on `View.razor`): `Generated by JIM` chip, attribute menu with "View history" and "Generate a new value…" with the confirmation as mocked.
