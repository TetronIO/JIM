...
Clearing a Connected System's connector space removes every CSO (and, unless you choose to keep it, their change history) without deleting the Connected System itself; its configuration, schema, and Synchronisation Rules survive. Clearing itself is a hard delete: no Metaverse Object Deletion Rule or grace period runs at the moment of the clear, so the removal of the CSOs is immediate. The portal's Connector Space page, `POST /connected-systems/{id}/clear`, and [`Clear-JIMConnectedSystem`](../powershell/connected-systems.md#clear-jimconnectedsystem) all queue the same tracked background task.

Because a clear skips obsoletion, any Metaverse attribute value the cleared system contributed keeps its provenance and keeps exporting downstream even though nothing joins it to the system any more, and the identities themselves receive no Deletion Rule decision at all. For the common case, a Full Import that brings every object straight back, this is exactly what you want: the join re-forms and nothing changes. It only matters when an object never returns, for example a source population that shrank during the clear, or a system you are decommissioning without deleting: without something to notice the gap, those values and identities would be stranded indefinitely.

JIM closes that gap itself. Clearing records, inside the same transaction, which Metaverse Objects were joined at that moment, and arms the **stranded-value sweep**, but it only runs once you have genuinely rebuilt the Connector Space: the sweep executes at the first Full Synchronisation of this Connected System after a Full Import of it has completed successfully, later than the clear. A Full Synchronisation run before that leaves the sweep armed and does nothing beyond ordinary synchronisation, stating so on its Activity, because treating an empty or half-rebuilt Connector Space as a genuine departure would recall values that simply have not been re-imported yet.

Once the gate is open, the sweep first checks how many of the recorded objects have not rejoined. If that share is above the **Post-clear reconciliation: maximum missing share** Service Setting (`Sync.PostClearReconciliation.MaxMissingPercent`, default 10%), the sweep refuses outright: nothing is recalled, marked, or deleted, the arming stays in place, and the Activity states the counts and the remedy (re-import the rest, or raise the setting). This is what protects you from a broken re-import, such as a filter or base DN change that quietly returns a fraction of the population, being mistaken for a mass departure.

Below the threshold, the sweep proceeds in two steps. First, it finds Metaverse attribute values this system contributed with no CSO of this system still joined, and recalls them exactly as an obsoletion would have: a value with a surviving contributor is re-elected to it, and a value with none is cleared. Where a Metaverse Object's only remaining connections carry no import source for its type (for example, only a provisioned target account remains), its values are preserved as last known state rather than recalled, matching the [deletion behaviour](metaverse.md#deletion-behaviour) preservation rule. Second, every recorded object that still has not rejoined is evaluated against its type's [Deletion Rule](metaverse.md#deletion-behaviour), exactly as if the cleared system had disconnected it during an ordinary synchronisation: a Manual rule does nothing, a rule with a grace period marks the object (and cancels the marking if it rejoins within the window), and a no-grace rule deletes it immediately along with its exports. Nothing here changes the rule itself; only the evaluation timing does. The sweep's outcome, whatever it is, is always stated on the Full Synchronisation run's Activity: it ran and found nothing, it ran and recalled values or applied Deletion Rules, it refused on a shortfall, or it stayed armed because the gate was still closed.

Independently of any one clear, every armed sweep (and Connected System deletion) also looks across the whole Metaverse for objects that hold no connector at all and whose type's Deletion Rule does not depend on which system left; historical strays from a clear that predates this feature, or from several disconnections over time, are given their rule too. The Pending Deletions page attributes these as "No connector remains" rather than naming a system, since none triggered the decision.

!!! tip "Watch the Connected System page while a sweep is armed"
    A notice on the Connected System page tells you whether a sweep is waiting for a Full Import, or ready for the next Full Synchronisation once one has completed. A Full Import that completes with object-level errors does not count towards readiness: an object that failed to import was never staged, and the sweep must not treat it as departed.
...
**Disable the Synchronisation Rules first.** An Object Type cannot be deselected while an enabled Synchronisation Rule is still bound to it, and a Synchronisation Rule cannot be enabled against an Object Type that is not selected. Saving either is refused with a message naming the rules. The two refusals keep a rule from ever running against a type JIM no longer imports; an outbound rule would otherwise go on provisioning into it, and would provision the obsoleted objects all over again. So taking a type out of management is two deliberate steps: disable (or delete) its Synchronisation Rules, then deselect it.

!!! note "Deselected before this changed"
    Earlier versions of JIM left a deselected Object Type's objects joined and contributing the values they last imported, and let its Synchronisation Rules go on running. After upgrading, a deselected Object Type with **no** enabled Synchronisation Rule bound to it is taken out of management on its next Full Import, which obsoletes and disconnects its objects as described above; preview or set deletion limits first if that is not what you intend, or select it again. A deselected Object Type that still has an enabled Synchronisation Rule bound to it is left as it was, and every Full Import completes with a warning naming the type and the rules to disable.
...
A destructive diff pauses on a three-way decision (an additions-only diff keeps the plain **Apply Schema Changes** action, since additions cannot affect anything):
...
- **Apply &amp; Remove** is the full commitment, for a genuinely decommissioned object type or attribute. Its plan dialog leads with the consequence, because this deletes configuration and identity data: the named Synchronisation Rules and mappings are **deleted** (under child Activities of the refresh), and the data goes with them through a background worker task you can follow from the Operations tab. Connected System Objects of a removed object type are marked Obsolete and deprovision through the standard pipeline on the next synchronisation run, with disconnection, attribute recall, grace periods and [Metaverse Deletion Rules](metaverse.md#deletion-behaviour) all applying as configured; stored values of a removed attribute are deleted. The dialog counts both before you confirm, per object type and per attribute, and the task's Activity records every object it obsoleted.

Watch the preview's **discovery warnings** before applying: a Connected System identity without permission to read the full schema produces a partial read, which can make object types or attributes appear removed when they are not. Other warnings appear here too, about what the account can do beyond reading the schema; on Active Directory, for example, that the account is not allowed to list the domain's Deleted Objects container, or that JIM could not confirm it can, in which case a Delta Import would refuse to run or would complete with a warning (see the [LDAP Connector](../connectors/jim-ldap-connector.md#service-account-permissions)). Re-running the preview after granting the access confirms the fix.

The same flow is available to automation: `Import-JIMConnectedSystemSchema -Preview` in [PowerShell](../powershell/connected-systems.md#import-jimconnectedsystemschema) returns the preview result (its `HasRemovalsOrDefinitionChanges` property flags the changes that matter, its `Dependents` property names the invalidated configuration and its `RemovalImpact` property counts what a removal would take), and committing with `-DisableDependents` or `-RemoveDependents` takes the matching posture. The REST API offers `POST connected-systems/{id}/import-schema/preview` beside the committing `import-schema` endpoint, whose body accepts `disableDependents` or `removeDependents`; see the [REST API reference](../../api/reference/).
...
- **Policies that apply to only some objects.** Every directory that publishes a policy also has a way to give some objects a different one: Active Directory's Fine-Grained Password Policies, an OpenLDAP entry's own `pwdPolicySubentry`, and 389 Directory Server's subtree and per-object policies. Reading them normally requires privileges JIM's service account should not hold, so JIM detects whether any exist rather than enumerating them, and reports one of three answers: none exist, some exist, or it could not tell. "Could not tell" is shown as its own state rather than being treated as "none", because an empty result from a directory is exactly what a caller with no rights over them receives; a search that finds nothing is always could not tell. Where the panel says policies exist or that it could not tell, treat the figures shown as a minimum.
- **Further checks.** A directory can enforce rules beyond the figures it publishes. Where it tells JIM that such checks are configured (an OpenLDAP check module, or 389 Directory Server's dictionary and character checks), the panel says beneath the figures that the directory applies further checks JIM cannot see, so a password satisfying everything above can still be refused. A custom Active Directory password filter is exposed over no protocol at all, so there the panel cannot even say that.
...
**When the panel shows no rules, it says why.** The four reasons are distinct because each wants a different response from you:

| The panel says | What it means | What to do |
|----------------|---------------|------------|
| Nothing has been read yet | JIM has not read this system's policy. | **Refresh Schema** on the Schema tab reads it. |
| This directory publishes no password policy that JIM can read | A generic LDAP directory, or an OpenLDAP without the `ppolicy` overlay. Whatever rules it enforces live in configuration an ordinary connection cannot see, and there is no cross-vendor standard for exposing them. | Nothing to grant or refresh. If the directory enforces rules, configure the generator by hand. |
| JIM could not read this directory's password policy: the account it connects as cannot read the server configuration that holds it | 389 Directory Server keeps its policy on `cn=config`, and OpenLDAP names its default policy in the overlay's configuration; the service account may not read them. | Grant the read described under [Service Account Permissions](../connectors/jim-ldap-connector.md#service-account-permissions), then refresh the schema. |
| This directory's password policy mechanism is loaded but no policy is configured, so no rules apply | OpenLDAP loads the `ppolicy` overlay but no `pwdPolicy` entry exists. | Nothing, unless you expected a policy: then configure one in the directory and refresh the schema. |

JIM reports that it found nothing rather than implying the system has no rules, whichever of these applies.
