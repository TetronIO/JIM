...
- 🐛 The built-in Words Example Data Set no longer ships `Jalapeã±O`, an encoding-damaged spelling of `Jalapeño` that generated example objects could pick up as a value. Built-in Example Data Sets are not editable, so an instance already holding the damaged word keeps it; newly installed instances get the corrected one. (#1287)
...
- 🐛 A Connector introduced in a new release now reaches instances upgraded to it, not only newly installed ones. Built-in Connectors were only ever registered while an instance was setting itself up for the first time, so an upgraded instance silently lacked any Connector added since it was installed; they are now reconciled on every start, and an upgrade to this release registers whatever was missing (the JIM SQL Connector, for most instances). (#1287)
