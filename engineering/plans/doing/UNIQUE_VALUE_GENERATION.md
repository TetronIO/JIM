...
- Item 1: Scenario 23 is in, green on OpenLDAP (Samba AD verification pending). The mixed-capability SQL case is not: in release 1 a target-side collision is an ordinary export error whatever the connector, so it is proved once against Samba AD's own `sAMAccountName` uniqueness (Test 10); the capability contrast belongs to release 4, where classification exists.
