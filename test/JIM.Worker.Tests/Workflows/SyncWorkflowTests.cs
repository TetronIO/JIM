...
public class SyncWorkflowTests : WorkflowTestBase
{
    ...
    public async Task ConsecutiveDeltaSyncs_EachProcessOnlyNewModificationsAsync()
    {
        ...
        var fullSyncActivity = await CreateActivityAsync(
            ...
            ConnectedSystemRunType.FullSynchronisation);
        var cts1 = new CancellationTokenSource();
        await new SyncFullSyncTaskProcessor(new SyncEngine(), new SyncServer(Jim), SyncRepo, connectedSystem, fullSyncProfile, fullSyncActivity, cts1)
        ...
        var deltaSyncActivity1 = await CreateActivityAsync(
            ...
            ConnectedSystemRunType.DeltaSynchronisation);
        var cts2 = new CancellationTokenSource();
        await new SyncDeltaSyncTaskProcessor(new SyncEngine(), new SyncServer(Jim), SyncRepo, connectedSystem, deltaSyncProfile, deltaSyncActivity1, cts2)
        ...
        await new SyncDeltaSyncTaskProcessor(new SyncEngine(), new SyncServer(Jim), SyncRepo, connectedSystem, deltaSyncProfile, deltaSyncActivity2, cts3)
            .PerformDeltaSyncAsync();
