...
An execution item's detail page is split over tabs. **Overview** carries what the run did to the item: the execution summary, the record it processed, its attribute changes and any error. **Causality** carries why it happened and what it caused, which is much the largest of the three; see below. Where the item created or failed on a Pending Export, that gets its own **Pending Export** tab, because a queued export is a separate subject with its own status, retry count and staged attribute changes. The last two appear only where there is something to put in them, and an item with only an Overview hides the tab bar entirely rather than offering a choice of one.
