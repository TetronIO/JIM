...
- **"Call to obsolete method `Multiple`" on `Assert.Multiple(() => { ... })` is a false positive; do not act on it.** In NUnit 4.6.1 only the delegate-typed overloads carry `[Obsolete]` (`Multiple(TestDelegate)`, `Multiple(AsyncTestDelegate)`, `MultipleAsync(AsyncTestDelegate)`). A parameterless void lambda is convertible to both `TestDelegate` and `Action`, and C# overload resolution picks `Multiple(Action)`, which is **not** obsolete; CodeQL resolves it to the `TestDelegate` overload instead. The compiler is the arbiter here: `dotnet build <test project> -warnaserror` is clean, and a genuine obsolete call would raise CS0618 and fail it, which this repository's zero-warning rule would never let through. `Assert.EnterMultipleScope()` does exist and is a reasonable API, but adopting it piecemeal would leave a handful of the ~87 `Assert.Multiple` call sites across ~26 files in a different style; if we want it, it is a deliberate suite-wide migration in its own PR. Reply to the thread with this rationale and resolve it (precedent: PR #1151, two threads).
