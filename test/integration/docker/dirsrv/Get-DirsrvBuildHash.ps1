...
function Test-DirsrvSnapshotCurrent {
    ...
    .DESCRIPTION
        ...
        populate scripts have not changed. A missing label counts as a mismatch.
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    if (-not (Test-DirsrvLabelEquals -Actual $snapshotHash -Expected $ExpectedSnapshotHash)) { return $false }

    $baseHash = docker image inspect $ImageTag --format '{{index .Config.Labels "jim.dirsrv.base-hash"}}' 2>&1
    if ($LASTEXITCODE -ne 0) { return $false }
    return (Test-DirsrvLabelEquals -Actual $baseHash -Expected $ExpectedBaseHash)
}
