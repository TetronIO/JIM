...
<#
.SYNOPSIS
    Scenario 018: does JIM write a derived value back into the Connected System it came from? (#1284)
...
try {
    Write-TestSection "Scenario 018: Writeback into the source Connected System (#1284)"
    ...
    $hrCsv = Join-Path $stage "scenario-018-hr.csv"
    @(
    ...
    $controlCsv = Join-Path $stage "scenario-018-control.csv"
    "employeeId,writeback" | Set-Content -Path $controlCsv -Encoding UTF8

    Write-FileToConnectorVolume -SourcePath $hrCsv      -DestinationPath "/connector-files/test-data/scenario-018-hr.csv"
    Write-FileToConnectorVolume -SourcePath $controlCsv -DestinationPath "/connector-files/test-data/scenario-018-control.csv"
    Write-Host "  OK Seeded 3 people into HR, and a header-only Control file" -ForegroundColor Green

    Write-Host "Running Scenario 018 setup..." -ForegroundColor Gray
    $context = & "$PSScriptRoot/../Setup-Scenario-018.ps1" -JIMUrl $JIMUrl -ApiKey $ApiKey -Template $Template
    if (-not $context) { throw "Setup-Scenario-018.ps1 returned no configuration." }
    ...
    function Invoke-S18RunProfile {
        ...
        Assert-ActivitySuccess -ActivityId $result.activityId -Name "Scenario 018 $Name ($Label)"
        return $result
    }
    ...
    if ($projected -ne 3) {
        ...
        throw "Scenario 018 cannot continue: only $projected of 3 people projected."
    }
    ...
    if ($controlPending -lt 1) {
        ...
        throw "Scenario 018's control assertion failed; the comparison is not meaningful."
    }
    ...
    Write-TestSection "Scenario 018 Summary"
    ...
    Write-Failure "Scenario 018 failed: $($_.Exception.Message)"
