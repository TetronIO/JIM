...
public class ExampleDataTemplateAttribute
{
    #region accessors
    ...
    /// </summary>
    public List<ExampleDataTemplateAttributeWeightedValue>? WeightedStringValues { get; set; }
    ...
    /// </summary>
    public ExampleDataTemplateAttributeDependency? AttributeDependency { get; set; }
    #endregion
    ...
    public void Validate()
    {
        ...
        if (ConnectedSystemObjectTypeAttribute == null && MetaverseAttribute == null)
            throw new ExampleDataTemplateAttributeException("ConnectedSystemAttribute and MetaverseAttribute are null");

        // needs to be within a 1-100 range
        if (PopulatedValuesPercentage < 1 || PopulatedValuesPercentage > 100)
            throw new ExampleDataTemplateAttributeException("PopulatedValuesPercentage is less than 1 or greater than 100");
        ...
        else if (MetaverseAttribute != null)
        {
            ...
            throw new ExampleDataTemplateAttributeException("Either a MetaverseAttribute OR a ConnectedSystemAttribute reference is required. None was present.");


        if (attributeDataType != AttributeDataType.Boolean && (BoolTrueDistribution != null || BoolShouldBeRandom != null))
            throw new ExampleDataTemplateAttributeException("Not Bool and BoolTrueDistribution is not null or BoolShouldBeRandom is not null");

        if (attributeDataType != AttributeDataType.DateTime && IsUsingDates())
            throw new ExampleDataTemplateAttributeException("Not DateTime and MinDate is not null or MaxDate is not null");

        if (attributeDataType != AttributeDataType.Text)
        {
            // pattern can only be used with string attributes
            if (usingPattern)
                throw new ExampleDataTemplateAttributeException("Not string but using pattern");
        ...
        if (attributeDataType != AttributeDataType.Reference)
        {
            if (ReferenceMetaverseObjectTypes is { Count: > 0 })
                throw new ExampleDataTemplateAttributeException("ReferenceMetaverseObjectTypes can only be used with reference attribute data types");

            if (ManagerDepthPercentage.HasValue)
                throw new ExampleDataTemplateAttributeException("ManagerDepthPercentage can only be used with reference attribute data types");
        }            

        if (attributeDataType != AttributeDataType.Reference && usingMvaRefMinMaxAttributes)
            throw new ExampleDataTemplateAttributeException("MvaRefMinAssignments or MvaRefMaxAssignments can only be used with reference attribute data types");
        ...
        {
            if (IsUsingNumbers())
                throw new ExampleDataTemplateAttributeException("Bool but using number properties. This is not supported");

            if (usingExampleData)
                throw new ExampleDataTemplateAttributeException("Bool but using number example data. This is not supported");

            if (usingPattern)
                throw new ExampleDataTemplateAttributeException("Bool but using a pattern. This is not supported");
        }

        if (IsUsingNumbers())
        {
            if (MinNumber.HasValue && MaxNumber.HasValue && MinNumber.Value >= MaxNumber.Value)
                throw new ExampleDataTemplateAttributeException("Number and min number is equal or greater than max number");

            if (SequentialNumbers == true && RandomNumbers == true)
                throw new ExampleDataTemplateAttributeException("Number and sequential numbers and random numbers");
        }
        ...
        if (attributeDataType == AttributeDataType.DateTime)
        {
            ...
            {
                if (MinDate >= MaxDate)
                    throw new ExampleDataTemplateAttributeException("DateTime and min date is equal or greater than max date");

                if (MaxDate <= MinDate)
                    throw new ExampleDataTemplateAttributeException("DateTime and max date is less than or equal to min date");
            }

            if (usingPattern || usingExampleData || IsUsingNumbers())
                throw new ExampleDataTemplateAttributeException("DateTime and non-DateTime properties used. This is not supported");
        }
        ...
        if (attributeDataType == AttributeDataType.Reference)
        {
            if (attributeName != Constants.BuiltInAttributes.Manager && (ReferenceMetaverseObjectTypes == null || ReferenceMetaverseObjectTypes.Count == 0))
                throw new ExampleDataTemplateAttributeException($"ReferenceMetaverseObjectTypes not populated. Attribute: {attributeName}");

            if (ManagerDepthPercentage.HasValue)
            {
                if (PopulatedValuesPercentage.HasValue)
                    throw new ExampleDataTemplateAttributeException("ManagerDepthPercentage cannot be used with PopulatedValuesPercentage. Ensure it's set to null");

                if (ManagerDepthPercentage is < 1 or > 99)
                    throw new ExampleDataTemplateAttributeException("ManagerDepthPercentage must be between 1-99(%)");
            }
            ...
            if (attributePlurality != AttributePlurality.MultiValued)
                throw new ExampleDataTemplateAttributeException("MvaRefMinAssignments and MvaRefMaxAssignments can only be used on multi-valued attributes.");

            // min must be equal or greater than zero
            if (MvaRefMinAssignments is < 0)
                throw new ExampleDataTemplateAttributeException("MvaRefMinAssignments must be equal or more than 0");
            ...
            if (MvaRefMinAssignments.HasValue && MvaRefMaxAssignments.HasValue && MvaRefMinAssignments.Value >= MvaRefMaxAssignments.Value)
                throw new ExampleDataTemplateAttributeException("MvaRefMinAssignments must be less than MvaRefMaxAssignments");
        }
    }
    #endregion
}