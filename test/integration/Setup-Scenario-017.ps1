...
<#
.SYNOPSIS
    Setup for Scenario 017: Initial Password Provisioning

.DESCRIPTION
    Configures JIM to set an Initial Password on every account its outbound Synchronisation Rule
    provisions, so that Scenario 017 (Samba AD only) and Scenario 020 (either directory) can prove the
    account holder can actually use it.

    The provisioning substrate is Scenario 001's: HR CSV source, directory target, an Export
    Synchronisation Rule that provisions Users. This script composes Setup-Scenario-001.ps1 rather than
    rebuilding it, the same way Scenario 006 does, and then turns on the one thing Scenario 001 does not
...
.PARAMETER ExpiryBehaviour
    ...
    RequireChangeAtNextSignIn, which is what Scenario 017 asserts against and what an administrator would choose
    for a new starter. Scenario 020 overrides it, because it needs accounts whose Initial Password signs in
...
.EXAMPLE
    ./Setup-Scenario-017.ps1 -ApiKey "jim_..." -Template Micro
#>
...
# gated here. This is what lets Setup-Scenario-020.ps1 compose this script against OpenLDAP.
...
Write-TestSection "Scenario 017 Setup: Initial Password Provisioning"

# Step 1: Build the provisioning substrate (HR CSV -> Metaverse -> Samba AD)
Write-TestStep "Step 1" "Running Setup-Scenario-001.ps1 for the provisioning substrate"
...
try {
    ...
    #
    # Two observed behaviours make Setup-Scenario-001's single hierarchy import unreliable against Samba AD, and
    ...
    #      succeeds. Reproduced on every clean run of this scenario. Setup-Scenario-001 catches the failure,
    ...
    #      Setup-Scenario-001 selects the Partition *after* importing, so even a successful first import returns
    #      the Partition alone.
    #
    ...
    #
    # Setup-Scenario-001 exports userAccountControl through an expression, to exercise integer attribute flow.
...
# Summary
Write-TestSection "Scenario 017 Setup Complete"
...
# Return Scenario 001's configuration, plus what the assertions need to reach the directory as the
