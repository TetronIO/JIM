...
- **Last Updated:** 2026-09-19 (adversarial review applied; four-release delivery; uniqueness tokens, counters and the retired values register added)
...
3. **Counters are attribute-scoped and forward-only.** `GeneratedValueSequence` is keyed on the target attribute (Metaverse or Connected System attribute), never deleted automatically, seeded from the highest existing numeric value on first use, reserved in blocks per page with an atomic `UPDATE ... RETURNING`, and moved only upward by "Set next number" (confirmed, audited, portal and PowerShell). Gaps from unused block tails are documented as normal.
...
6. **The engine stays synchronous; generation resolves as a deferred pass per page, wired into everything that follows.** The engine records a `GenerationRequest` (object, mapping, evaluated base value) where a generated mapping is the winning contributor, and stages a placeholder pending addition so incumbent detection (`FindEffectiveIncumbentSyncRuleId`) sees the attribute as taken. Before `PersistPendingMetaverseObjectsAsync`, the worker resolves all of a page's requests in one batch, applies the values with provenance, and appends each attribute to the object's changed-attribute entry so export evaluation, drift detection and change history see it. Sync Preview resolves in dry-run mode (local gates only, nothing persisted).
...
| `LastMovedAt`, `LastMovedByName` | DateTime?, string? | Audit of "Set next number" |
...
| "Generate a new value" (identity action) | Assignment deleted, value retired, object review-flagged; next synchronisation assigns afresh |
...
- `TryRemediateAsync`, `IsAnchoredAsync`, `CommitAsync`, `EnterNeedsDecisionAsync`, `AuthoriseRenameAsync`, `RetryAsync`, `ReleaseNeedsDecisionForMappingAsync`, `ListNeedsDecisionAsync` (release 4); `RegenerateAsync` (identity action), `SetNextSequenceNumberAsync`, `GetAssignmentsForObjectAsync` (release 1).
...
4. Review-flag integration: "Generate a new value" sets the #892 flag; the flag's processing also resolves generation for the object (no derived pass yet).
5. Sync Preview: `SyncRuleAttributeFlowPreviewAdapter` and `SyncRuleAttributeFlowProposalMaterialiser` learn the source type; dry-run resolution renders the candidate and the gate consulted.
...
8. File the example-data migration follow-up issue.

#### Phase 3: Configuration and identity surfaces (portal, REST, PowerShell)

1. Portal (`SyncRuleAttributeFlowTab.razor`, inline dialog): a third "Source Type" item, "JIM generates it", with the form in the Generated Value Options mockups: target, base expression (existing editor and `ExpressionTester`), uniqueness token radio group with the sub-controls per kind (style, start, separator; start, increment, a "Pad to a fixed width" switch revealing width with room remaining and the overflow behaviour; random format and length with value-space hint), "Never reuse a value" switch (on), attempt limit (advanced), live preview, sequence state panel with "Set next number" confirmation; `Generated` row chip in both layouts with a `GetMappingTypeChipColour` case. Identity page (`MvoDetailsTable.razor` on `View.razor`): `Generated by JIM` chip, attribute menu with "View history" and "Generate a new value…" with the confirmation as mocked.
2. REST: `SyncRuleMappingDto`, `CreateSyncRuleMappingRequest`, `UpdateSyncRuleMappingRequest` gain a `generation` object; `SourceType` renders `GeneratedMapping`; direction gating follows the `InitialExportOnly` block; `SyncRuleMappingSettingsUpdate` carries the settings through PATCH. New: `GET metaverse-objects/{id}/generated-values`, `POST metaverse-objects/{id}/generated-values/{attributeId}/regenerate` (`202 Accepted`, effect on the next synchronisation), `GET`/`PUT sync-rules/{id}/mappings/{mappingId}/sequence` (read state; set next number, forward-only). OpenAPI updated; API tests beside `SyncRuleMappingUpdateApiTests.cs`.
3. PowerShell: `New-JIMSyncRuleMapping` gains `ImportGenerated` and `ExportGenerated` parameter sets (`-Generate`, `-TokenKind`, `-SuffixStyle`, `-SuffixStart`, `-SequenceStart`, `-SequenceIncrement`, `-FixedWidth`, `-OnWidthExceeded`, `-RandomFormat`, `-RandomLength`, `-Separator`, `-AttemptLimit`, `-NeverReuse`, `-ExcludeConnectedSystemId`); `Set-JIMSyncRuleMapping` the same as nullable settings; `Get-JIMGeneratedValue` (per identity), `Reset-JIMGeneratedValue` (regenerate), `Get-JIMGeneratedValueSequence`, `Set-JIMGeneratedValueSequence`; Pester in `src/JIM.PowerShell/Tests/`.
4. `ConnectedSystemServer` mapping create and settings-update validation: target type versus token; exclusions must be participating systems; `NeverReuse` forced on for sequence; "Set next number" refuses a lower number.
5. Docs: `docs/powershell/synchronisation-rules.md`, the API reference, and the concept section in `docs/configuration/synchronisation-rules.md` beside "Initial Export Only (outbound)": generated values, tokens, counters, never-reuse (recommended on; the rehire exception), "Generate a new value", what a target-side collision looks like in this release.

#### Phase 4: Integration and release

1. `Invoke-Scenario22-UniqueValueGeneration.ps1` (setup, run, teardown through `Run-IntegrationTests.ps1`) covering the PRD's Modes, Strategy, Tokens, Never reuse (sequence only in this release), Adopt before generate, Failure, Stability and Surface parity rows, plus the local-gate rows of Gates; the mixed-capability case uses the SQL connector with a unique constraint, because a CSV target cannot reject a duplicate.
...
1. `RetiredGeneratedValue` table and migration (`LOWER` unique index), `RetiredGate`, retirement on every assignment deletion path (cascade paths write the retirement in the same transaction; the flush reconciliation and "Generate a new value" write it directly), `GeneratedValueRetired` history event on the identity's timeline, "Never reuse a value" honoured (release 1 persists the switch and forces it for sequences; this phase gives it effect for the other tokens).
...
1. Portal: Needs Decision tab on Operations modelled on `OperationsPasswordsTab.razor` (filter by Connected System and Synchronisation Rule, summary tile, per-row and bulk "Try again"); "Authorise rename on next export" confirmation naming every system that will change and stating the new value is decided at the next export (mock 06, reworded); `GeneratedValueAttentionIndicator` following `InitialPasswordAttentionIndicator` on `SyncRuleList.razor`, `ConnectedSystemList.razor` and the identity; `Corrected` chip and the attribute-scoped causality Timeline (mock 04); Activity Timeline events and exits (mock 05); `OutcomeDisplayMap`, `CausalityModelBuilder`, `CausalityCauseWording`, `CausalitySummaryBuilder`; bUnit tests.
...
- One generation service, one assignment model, one review-flag pickup and one dependency graph that internally managed identities, workflows and a future Set Value reuse.
