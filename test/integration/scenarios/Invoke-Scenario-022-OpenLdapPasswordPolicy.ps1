...
<#
.SYNOPSIS
    Test Scenario 022: OpenLDAP Password Policy (discovery, enforcement, generated passwords satisfy it)
...
.DESCRIPTION
    ...
    The chain, in order, and why each link is needed:
      ...
      4. Scenario 001's import, synchronisation and export at Micro provision ten accounts as the
...
.PARAMETER SkipPopulate
    Skips Populate-OpenLDAP-Scenario-022.ps1. The negative control still runs; without the fixture it fails.
...
.EXAMPLE
    ./Invoke-Scenario-022-OpenLdapPasswordPolicy.ps1 -ApiKey "jim_..." -Template Micro
#>
...
<#
    ...
    With the stack up (openldap-primary healthy) and Populate-OpenLDAP-Scenario-022.ps1 run once:
    ...
    before it can go further:
      ...
      - trust that certificate in JIM with Add-JIMCertificate -CertificateData (Setup-Scenario-015.ps1's
        pattern: upload the bytes, never skip validation);
      - hand Setup-Scenario-022.ps1 a DirectoryConfig with UseSSL $true, Port 1636, LdapSearchScheme
...
if ($DirectoryConfig.DirectoryType -ne "OpenLDAP") {
    throw "Scenario 022 requires OpenLDAP. The fixture is a ppolicy overlay on the Yellowstone suffix, and " +
...
# The fixture. Populate-OpenLDAP-Scenario-022.ps1 and Setup-Scenario-022.ps1 carry the same defaults; they
...
Write-TestSection "Scenario 022: OpenLDAP Password Policy"
...
if (-not $SkipPopulate) {
    ...
    & "$PSScriptRoot/../Populate-OpenLDAP-Scenario-022.ps1" `
...
if (-not $enforced) {
    ...
    throw "ENFORCEMENT NOT PROVEN: the ppolicy overlay did not refuse a 5-character password from " +
          ...
          "Check that Populate-OpenLDAP-Scenario-022.ps1 ran, that the overlay's olcPPolicyDefault names " +
...
$config = & "$PSScriptRoot/../Setup-Scenario-022.ps1" `
...
if (-not $config) {
    throw "Failed to set up Scenario 022 configuration"
}
...
try {
    ...
    if ($lastStepIndex -ge $stepOrder.IndexOf("Discovery")) {
        ...
        # Setup-Scenario-001's schema import is what triggers discovery, and it ran as the provisioner.
        ...
        Add-TestResult -Name "furtherChecksApply is false (no check module named on the overlay or the policy)" `
            ...
            -Detail "furtherChecksApply was '$(Get-PolicyProperty -Policy $policy -Name 'furtherChecksApply')'. True with pwdCheckQuality 2 and no module means the reader fell back to the coarse rule: check the provisioner's read on cn=config (Populate-OpenLDAP-Scenario-022.ps1)."
    }
    ...
    if ($lastStepIndex -ge $stepOrder.IndexOf("Provision")) {
        ...
        # persisted baseline for a delta to compare against (Scenario 017's reasoning).
...
Write-TestSection "Scenario 022 Summary"
