...
- ✨ A Run Profile execution now shows its whole journey on the Activity, not just what it is doing this second: every step of the run, with a tick and how long it took for the ones that are done, the step running now highlighted with the Connector's own steps and message beneath it, and the steps still to come greyed out. A step the run did not need (deletion detection on a Delta Import, opening a connection for a file-based import) is labelled "not needed" rather than left looking outstanding, and a failed run marks the step it failed in. The steps stay with the Activity, so a run that finished days ago still answers where its four hours went. (#454)
