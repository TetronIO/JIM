...
- 🖥️ Tooltips no longer stretch across the page. A long explanation grew to one very wide line, because nothing limited how far it could run; it now wraps into a readable block. A value with an explanation behind it also says so, carrying a faint dotted underline that firms up as you hover it, so the tooltip is discoverable rather than found by accident.
