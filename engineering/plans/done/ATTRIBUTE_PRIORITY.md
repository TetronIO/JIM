...
- **Status:** Done (all five phases delivered. One test block, the grace period and next-contributor fallback matrix, is split out to [#1307](https://github.com/TetronIO/JIM/issues/1307): the engine behaviour it covers has landed, so what remains is a scenario extension rather than attribute-priority work. Surface 1c was dropped, with the reasoning recorded in Phase 3)
- **Issue:** [#1199](https://github.com/TetronIO/JIM/issues/1199) completed the outstanding surfaces, test matrices and docs; [#91](https://github.com/TetronIO/JIM/issues/91) was the original tracker (closed 2026-07-06 with Phases 3-5 still open, which is why #1199 exists)
...
- [x] Make the drift detection contributor check priority-aware: legitimate only when the contribution wins resolution (refines the has-import-rule check in `DriftDetectionService.EvaluateDrift`, gated on multi-contributor attributes via an optional priority context; a losing contributor's diverged value is corrected as drift). Keyed on the winning rule's system (`ContributedBySystemId`). The formerly-open "rule-granular refinement (scope evaluation) for the multiple-objects-of-one-system-on-one-MVO case" is **closed as unnecessary**: that topology is unsupported by design and unreachable in practice. `EstablishJoinAsync` (`SyncTaskProcessorBase`) enforces at most one CSO per (Connected System, MVO), refusing a second same-system join with a `CouldNotJoinDueToExistingJoin` RPEI; export evaluation's CSO cache is keyed on `(MvoId, ConnectedSystemId)` and would hard-fail if the invariant were ever violated; and no manual/admin join path exists. System-keyed provenance is therefore exact for all supported topologies. (A partial unique index on `(ConnectedSystemId, MetaverseObjectId)` where the Metaverse Object FK is non-null now backs the invariant at the database level.)
- [x] Next-contributor fallback on recall: landed (`ReElectSurvivingContributorsAsync` re-elects a still-joined lower-priority survivor through the attribute-flow gate when a winning contributor's CSO is obsoleted, instead of blanking the attribute). The interim grace-period recall freeze is replaced by per-attribute behaviour (re-elect where a survivor exists, even under grace; freeze only single-source attributes with no survivor). **Reference-attribute re-election landed**: the re-flow no longer skips reference attributes (every object a survivor references already exists and is joined at recall time, so references resolve in the single recall pass; the survivor reload includes `ReferenceValue`, whose scalar `MetaverseObjectId` FK resolves the referent). Also fixed alongside: attribute-flow diffs now compare contributions against the MVO's *effective* values (excluding rows already pending removal, `GetEffectiveAttributeValues` in `SyncEngine.AttributeFlow.cs`), so a survivor holding a value *identical* to the recalled one retains it with its own provenance instead of the value being silently cleared. **In-place withdrawal re-election landed** too: when a still-joined winner stops supplying a value without asserting null, the worker detects the genuinely cleared attributes after the CSO's inbound flow (`GetClearedAttributeIds`) and runs the same re-election in the same run, so the survivor takes over immediately rather than on its own next synchronisation; a "Null is a value" withdrawal still asserts null (the marker row means the attribute is never treated as cleared)
...
- [x] Emit "asserted null" and "no contributor" as `SyncOutcome` node types in the RPEI outcome graph (#363) for observability (first release). **AssertedNull landed** (`ActivityRunProfileExecutionItemSyncOutcomeType.AssertedNull`, emitted in Detailed tracking as a child of the inbound flow outcome with an asserted-attribute count; UI colour/label/icon added). **NoContributor landed**: defined as "an attribute value was cleared this run with no replacement contributed" (a value removed, nothing re-added, no asserted-null marker, and no other value remaining, so a multi-valued attribute shrinking is not a clear; an attribute that was already blank reports nothing, resolving the noise concern, and the tally is computed worker-side from the pending change lists, so no engine→worker plumbing was needed). Emitted in Detailed tracking at both clear sites: inbound attribute flow (a contributor withdraws with no fallback) and attribute recall on obsoletion (no surviving contributor; re-elected and grace-frozen attributes are not counted). UI colour/label/icon added ("MVO No Contributor")
...
- [x] **Surface 1 (mapping editor):** show read-only precedence context (current contributor order and where this mapping lands) with a link to Surface 2; set "Null is a value" here; new mappings persist at the safe-addition default; no reordering on this surface (initial-ordering-at-creation deferred) (#1305)
- [x] **Surface 3 (discovery):** new Schema "Data Flow" list page showing all inbound and outbound flows, with a direction filter plus filters (connected system, CS object type, MVO object type, CS attribute, MVO attribute, free-text); pure discovery (no inline management), rows linking to the relevant sync rule mapping (and Surface 2 for import precedence); priority / "Null is a value" columns on import rows only, export rows show the export mapping and its `EnforceState`; relocatable to a future Policy area. Shipped as `DataFlowTab.razor` (a third tab on the Schema page) over a `DataFlowHeader` read model, with the same filters exposed as `GET api/v1/synchronisation/data-flows` and `Get-JIMDataFlow` in the same change. Two deviations from the sketch above, both deliberate: the direction filter is labelled **Inbound/Outbound** rather than Import/Export, because that is the vocabulary every other JIM surface uses for rule direction and the enum names are not customer-facing; and contributor counts are taken across the whole configuration rather than the filtered set, so filtering to one Connected System cannot make a shared attribute look like a sole contributor. The CS attribute and MVO attribute filters cannot match a flow whose relevant side is an expression, because an expression's attribute references live in its text and are not modelled; free-text search covers those, and the limitation is stated on the page, in the API documentation and in the cmdlet help rather than left to be discovered
- [x] Add "Advanced Options" section to the mapping editor with the "Null is a value" checkbox (#1305)
...
- [~] **Dropped (Aug 2026): priority indicator column on the sync rule list view (Surface 1c).** A **Priority** column on a list of Synchronisation Rules reads as the *rule's* priority, which is not a concept JIM has: priority belongs to a mapping's position in one Metaverse Attribute's contributor order, and a rule contributing to twelve attributes has twelve unrelated positions. The sketch above papered over this by putting a count in the column ("3 attrs"), which is a different fact wearing the column's name. Surface 3 answers the question the indicator was reaching for directly and across all rules at once, via the **Multiple contributors** filter, so the signpost has nothing left to point at. Dropped rather than deferred, with no follow-up issue: there is no demand signal for it. If it ever returns on real demand it must be headed "Attribute Priority" rather than the bare word, and report the rule's standing rather than its participation, since a count of the attributes a rule shares appears on nearly every import rule once a deployment matures and so distinguishes nothing (see #1199 for the same note)
...
- [x] Integration tests for fine-grained authority (worked example 2). Executing these surfaced and fixed the defect that made the whole mechanism inoperative: object scope was evaluated per rule only for join and projection, while Attribute Flow applied **every** enabled import rule for the system, so a rule the object was explicitly scoped out of contributed anyway (`SyncTaskProcessorBase.ProcessMetaverseObjectChangesAsync` used `activeSyncRules` where it needed `inScopeImportRules`). Fixing it exposed a second defect underneath: two contributing rules evaluated in the same pass (only possible on one Connected System, i.e. this very topology) both wrote, because the attribute-flow writers diff against persisted values and cannot see a pending addition, leaving a single-valued attribute with two values. Both are covered by `ScopedImportRuleAttributeFlowTests` (unit) and Scenario 14 (real PostgreSQL). A third, separable gap was found and deliberately not fixed here: deleting a contributing Synchronisation Rule nulls its values' `ContributedBySyncRuleId` (FK ON DELETE SET NULL) and a later synchronisation contributing the identical value does not re-stamp it, because the writers diff by value, so the value survives with no contributing rule.
  - [x] Scoped exception rule (priority 1) wins for in-scope objects; direct changes in the lower-priority system are corrected via EnforceState export (`ScopedExceptionAuthority`: a second, scoped import rule on Primary takes Description at priority 1 for Dave, and the same Export (Secondary) run corrects Dave's diverged entry while leaving out-of-scope Frank's alone, since Secondary won for him)
  - [x] Non-exception objects: higher-priority system's rule wins; losing system's direct changes corrected back (`EnforceStateCorrectsLoser`: Frank's direct Secondary edit never reaches the Metaverse, is still diverged in the directory after synchronisation, and is corrected by the Enforce State export; verified red by turning Enforce State off, which leaves the divergence in place exactly as the design says it should)
...
- [ ] **Split out to #1307 (Aug 2026).** Grace period / attribute recall and next-contributor fallback (replacing the interim recall-freeze; the motivating failure was a disconnected source nulling inputs to an expression-built attribute such as an LDAP DN, producing an invalid value like `CN=,OU=,...` and a failed export). The engine work these cells cover has landed (see Phase 2c above: re-election under grace, in-place withdrawal and reference attributes); what remains is the end-to-end matrix on real PostgreSQL with a grace period configured, which is a scenario extension rather than attribute-priority work, so it is tracked separately rather than holding this plan open:
...
- [x] Add user documentation for the attribute priority surfaces: the object type detail page management home and the Attribute Flow editor in `docs/concepts/attribute-priority.md`, and the Data Flow page as `docs/configuration/data-flow.md` (a how-to under Configuration rather than a concept, since it describes operating a surface), cross-linked from the Attribute Priority concept page as the way to find contested attributes in the first place
