...
<#
.SYNOPSIS
    Test Scenario 005: Object Matching Rules
...
.EXAMPLE
    ./Invoke-Scenario-005-MatchingRules.ps1 -Step All -Template Micro -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-005-MatchingRules.ps1 -Step Projection -Template Micro -ApiKey $env:JIM_API_KEY
#>
...
Write-TestSection "Scenario 005: Object Matching Rules"
...
try {
    ...
    # Use dedicated minimal CSV for Scenario 005 (no baseline users)
    # This ensures tests are self-contained and don't cause mass export failures
    # Seed a full baseline set of CSVs into the volume first. Setup-Scenario-001.ps1
    ...
    # Overlay Scenario 005's tailored HR CSV (1 baseline user for schema discovery)
    Write-Host "Applying Scenario 005 HR overlay..." -ForegroundColor Gray
    Copy-Item -Path "$scenarioDataPath/scenario-005-hr-users.csv" -Destination "$testDataPath/hr-users.csv" -Force
    ...
    # Setup scenario configuration (reuse Scenario 001 setup)
    ...
    # Scenario 005 uses hrId as external ID (not employeeId) to test:
    ...
    Write-Host "Reconfiguring CSV external ID for Scenario 005..." -ForegroundColor Gray
    ...
    Write-Host "✓ JIM configured for Scenario 005" -ForegroundColor Green
    ...
    if ($Step -eq "SamePageJoinConflict" -or $Step -eq "All") {
        ...
        Copy-Item -Path "$scenarioDataPath/scenario-005-hr-users.csv" -Destination $csvPath -Force
    ...
    # see Scenario 001's Reconnection step and Scenario 004's AuthoritativeRejoinCancellation step). #1612 suspects
    ...
    # landing on the same MVO in the same page. Scenario 004 cannot express this at all: its CSV uses employeeId
    ...
    if ($Step -eq "SamePageRejoinCancellation" -or $Step -eq "All") {
        ...
        # later steps (Case Sensitivity, ExportMatchJoin) rely on Setup-Scenario-001's baseline configuration.
        ...
        try {
            ...
            $srpDepartment = "Information Technology"  # one of the OUs Scenario 005 pre-creates for Samba AD
            ...
            # -AttributeName/-AttributeValue is the exact-match attribute filter Scenario 001 also uses).
            ...
            # them off the MetaverseObjects table directly (same pattern as Scenario 004's #119 trigger-mode
            ...
            # so later steps (Case Sensitivity, ExportMatchJoin) see Setup-Scenario-001's baseline configuration.
    ...
    # Setup-Scenario-001.ps1 configures (employeeID for Samba AD / employeeNumber for OpenLDAP -> Employee ID).
    #
    ...
    # for Samba AD; uid=<Account Name>,ou=People for OpenLDAP - see Setup-Scenario-001.ps1's distinguishedName
...
"@
        ...
        # Setup-Scenario-001.ps1) means removing the CSV row only schedules the deletion: nothing deprovisions the
    ...
    Write-Host ""
    Write-Host "✗ Scenario 005 failed: $_" -ForegroundColor Red
