...
- ✨ New built-in Metaverse Attributes close the gap to SCIM 2.0's core and enterprise User schema, so SCIM resources map cleanly onto JIM's canonical vocabulary: the multi-valued Emails, Account Enabled (the natural boolean target for SCIM's active flag), Nickname, Preferred Language, Locale, Time Zone, Middle Name, Honorific Prefix, and Honorific Suffix. Existing deployments gain the new attributes automatically at service startup. (#1104)
- ✨ Metaverse Attributes now carry Standard Mappings: advisory metadata recording each attribute's counterpart in the SCIM 2.0 and LDAP/AD vocabularies, with notes where the correspondence has nuance. Built-in attributes come pre-seeded and maintained by JIM (view them from the Schema area's Attributes tab), and you can record your own mappings on custom attributes from the attribute editor, with every change audited in the attribute's configuration change history. Also returned by the REST API's attribute detail endpoint. The mappings will power Attribute Flow editor hints, connector wizard default-flow suggestions, and schema documentation; they are never consulted by the synchronisation engine, so your Attribute Flow configuration remains the single source of mapping truth. (#1104)
