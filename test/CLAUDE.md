...
The lab binds JIM's Connected Systems as a delegated service account (`cn=svc-jim,ou=Services,<suffix>`), never the rootDN, under an explicit `olcAccess` set and a ppolicy password policy; the directory administrator (rootDN) is kept for populating data, out-of-band assertions and the healthcheck. The five LDIF files behind this live in `test/integration/docker/openldap/acl/` and are the single source for both the lab and the customer-facing recipe in `docs/connectors/jim-ldap-connector.md`. Samba AD works the same way: JIM binds as `CN=svc-jim,OU=Services,<domain DN>`, a member of `CN=JIM Connectors,OU=Services,<domain DN>` which carries the delegation, while `samba-tool` population, assertions and the healthcheck stay on the domain Administrator; the access control entries live in `test/integration/docker/samba-ad-prebuilt/delegation/jim-ad-delegation.acl` and are likewise published verbatim in the connector documentation. The full explanation for both directories, including which identity does what and why, lives in "Two Bind Identities" in `engineering/INTEGRATION_TESTING.md`; it is not repeated here.

**OpenLDAP MDB map sizes (accesslog and main databases):**

OpenLDAP's MDB databases have a fixed maximum map size (`olcDbMaxSize`). A full **main** database rejects writes with `MDB_MAP_FULL` errors. A full **accesslog** database is far worse: slapo-accesslog **silently stops recording changes** (main-database writes still succeed), so delta imports find zero modifications and sync changes are lost; the only evidence is `MDB_MAP_FULL` lines in slapd's own log.
...
**PostgreSQL scales per template too:**

`docker-compose.override.yml` parameterises the database's tuning with `JIM_DB_*` environment variables (shared_buffers, max_wal_size, checkpoint_timeout, wal_compression, work_mem, shm_size and friends), defaulting to the low-footprint dev profile (256MB shared_buffers, 1GB max_wal_size) when unset. `Run-IntegrationTests.ps1` assigns a profile per Scale template before compose up (e.g. Scale500k: 4GB shared_buffers, 16GB max_wal_size, 15min checkpoint_timeout, lz4 wal_compression) and clears the variables for smaller templates. Rationale: on the dev defaults, the Scale500k25kGroups run triggered WAL-pressure checkpoints every ~12 seconds (PostgreSQL logged "checkpoints are occurring too frequently" 1,488 times over the run) and each checkpoint's fsync storm stalled bulk COPY streams for up to 78s; frequent checkpoints also amplify WAL volume itself via full-page writes. Diagnostics: the database writes jsonlog to `/var/log/jim/database/` inside the container, and the override preloads `pg_stat_statements` (run `CREATE EXTENSION IF NOT EXISTS pg_stat_statements;` once to expose the view).
