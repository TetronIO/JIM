...
List Contents and Read Property are what the tombstone search needs, and JIM only ever reads tombstones. Read Permissions lets the account read the container's own permissions, which is how JIM can confirm the grant is in place. Skip this and nothing fails: the search succeeds, returns nothing, and every deletion is silently missed by every Delta Import.
