...
The canonical release history lives in [`CHANGELOG.md`](../CHANGELOG.md), which has structured per-version notes (Added / Changed / Fixed / Performance / Security) for every release, with comparison links between versions.
...
> **Note:** The PowerShell module manifest (`src/JIM.PowerShell/JIM.psd1`) must be updated manually when changing versions. The `ModuleVersion` field in the manifest is not automatically synchronised with the VERSION file. This is a known gap - future work could automate this via a pre-release script or GitHub Action.

### Updating the Version

1. Edit the `VERSION` file with the new version number
2. Update the PowerShell manifest (`src/JIM.PowerShell/JIM.psd1`) - change `ModuleVersion`
...
# src/JIM.PowerShell/JIM.psd1: ModuleVersion = '0.3.0'
...
The release bundle (`jim-release-X.Y.Z.tar.gz`), built by `scripts/Build-ReleaseBundle.ps1`, contains:

```
jim-release-X.Y.Z/
+-- setup.sh                  # The installer (deploy/setup.sh); run inside the bundle, it installs from it
+-- VERSION                   # The version the bundle installs; the installer reads it
+-- docker-images/
...
|   +-- README.md
|   +-- CHANGELOG.md
|   +-- INSTALL.md            # Installation instructions, generated by the build script
+-- checksums.sha256          # SHA256 checksums for verification
+-- README.txt                # Quick start guide
```

`docker-compose.override.yml` is deliberately absent: it holds development-only settings, and Docker Compose applies it to any command run without `-f` in the same folder.

### Deploying in an Air-Gapped Environment

The customer procedure has one home: [Deployment Guide > Air-Gapped Deployment](../docs/administration/deployment.md#air-gapped-deployment), with the offline copy in the bundle's `docs/INSTALL.md`. In short, the installer runs inside the extracted bundle (`sudo ./setup.sh`), loads the images, installs in `/opt/jim` and starts JIM with `--pull never`, so nothing is downloaded. Change the procedure there and in `Build-ReleaseBundle.ps1`, not here.
