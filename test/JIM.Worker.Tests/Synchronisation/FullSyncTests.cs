...
public class FullSyncTests
{
    ...
    public async Task PendingExportDeletedWhenCsoStateMatchesTestAsync()
    {
        ...
        var syncFullSyncTaskProcessor = new SyncFullSyncTaskProcessor(Jim, new SyncRepositoryAdapter(Jim),connectedSystem, runProfile, activity, new CancellationTokenSource());
    ...
    public async Task CsoProjection_JoinStateExplicitlyPersistedAsync()
    {
        ...
        var activity = ActivitiesData.First();
        var runProfile = ConnectedSystemRunProfilesData.Single(q => q.ConnectedSystemId == connectedSystem!.Id && q.RunType == ConnectedSystemRunType.FullSynchronisation);
        var syncProcessor = new SyncFullSyncTaskProcessor(Jim, new SyncRepositoryAdapter(Jim),connectedSystem!, runProfile, activity, new CancellationTokenSource());
    ...
    public async Task CsoObsoleteWithRecall_CreatesPendingExportsOnTargetSystem_Async()
    {
        ...
        var syncProcessor = new SyncFullSyncTaskProcessor(Jim, new SyncRepositoryAdapter(Jim),sourceSystem, runProfile, activity, new CancellationTokenSource());
        await syncProcessor.PerformFullSyncAsync();
    ...
    public async Task FullSync_WhenCsoInPendingProvisioning_UpdatesExistingCsoNotCreatesNewAsync()
    {
        ...
        var syncFullSyncTaskProcessor = new SyncFullSyncTaskProcessor(
            Jim, new SyncRepositoryAdapter(Jim), connectedSystem, runProfile, activity, new CancellationTokenSource());
        await syncFullSyncTaskProcessor.PerformFullSyncAsync();
