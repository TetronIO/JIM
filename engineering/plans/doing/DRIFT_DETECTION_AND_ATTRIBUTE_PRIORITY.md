...
**Captured directions (input to the IA review; not yet decided, Jun 2026):**

- **MVO attribute detail page (new surface).** There is currently no per-attribute detail page; the MVO object *type* page (`/admin/schema/object-types/{id}`) lists attributes only as table rows. A dedicated attribute detail page is a strong candidate **drill-down home** for the per-attribute priority widget, and could double as an **analysis** surface: value distribution, and "share of winning values" per contributing system, cheaply aggregated from `MetaverseObjectAttributeValue.ContributedBySystemId`. Note that stat reflects which system *won* resolution, not which *could* contribute (losing contributions are not stored). Config (priority; cheap, reads a few `SyncRuleMapping` rows) and analytics (scale aggregation over attribute values) should load independently so the analytics ambition does not gate the config control's responsiveness.
- **Global IAF explorer (candidate centralised surface, possibly its own feature).** A filterable list of all import attribute flows (filters: MVO type, MVO attribute, connected system, CSO type, attribute type, free-text search). Serves browse / audit / troubleshooting ("what flows where") and is the natural host for bulk operations. Because priority is only meaningful within one MVO attribute's contributor set, the explorer is read/filter/analyse; the reorder gesture only applies once narrowed to a single object type + attribute. This explorer may warrant being its **own feature** that the priority control plugs into, rather than being absorbed into attribute priority.
- **Bulk-prioritise a connected system (later-phase feature).** "Make connected system X authoritative for everything it contributes" = move X's IAF to priority 1 for every attribute it contributes to. High blast radius (mass authority shift across many attributes and objects), so it is a prime consumer of the configuration-change preview framework (#827) and must offer impact analysis before applying. Deferred to a later phase; would most naturally live in the global IAF explorer above.
...
- [ ] Unify expression-null and direct-absent into a single `ConnectedNoValue` signal feeding the resolver (replace the current unconditional clear at `SyncRuleMappingProcessor.cs:161-170`); surface expression evaluation failures as RPEI errors (typed catch, never treated as null); normalise empty/whitespace scalar expression results to no-value
...
11. **Conditional mappings and null** - DECIDED (Jun 2026): yes. An import expression evaluating to null produces `ConnectedNoValue` (a positive "no value" assertion, not "no opinion"), so it triggers NullIsValue semantics and provides a mechanism for conditionally asserting null. See "Expression null = assert no value" in the decision section for the four implementation notes (unify into `ConnectedNoValue`; failures surface as RPEI errors, never null; normalise empty/whitespace to no-value; document the string-concat malformed-value hazard).
