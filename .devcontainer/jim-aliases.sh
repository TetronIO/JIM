...
# jim.web builds its openapi-gen stage here too, exactly as a release does (it takes seconds),
# so a successful jim-build means the image can be released. Run jim-openapi-generate to
# refresh the static OpenAPI doc on disk when the API surface changes.
_jim_version_suffix() {
...
jim-build() {
  _jim_heal_docker_creds
  _jim_kill_local
  VERSION_SUFFIX="$(_jim_version_suffix)" docker compose $(_jim_compose) up -d --build || return $?
  _jim_keycloak_bridge
}
jim-build-web() {
  _jim_heal_docker_creds
