...
- 🐛 Case-insensitive Object Matching Rules treated `_` and `%` in a value as wildcards, so a value like `j_smith` could match `jxsmith` and join the wrong object, in both directions (an inbound Connected System Object joining to a Metaverse Object, and export matching); it now requires an exact case-insensitive match.
- 🐛 Filtering Metaverse Objects by attribute value (the REST API's `filterAttributeValue`, or `Get-JIMMetaverseObject -AttributeValue`) treated `_` and `%` in the value as wildcards, so a filter could return objects with a different value than the one requested; it now returns only exact case-insensitive matches.
