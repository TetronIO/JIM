...
- 🐛 Connected System settings now enforce either/or requirements at save time instead of failing later. The File Connector requires one of "Object Type Column" or "Object Type"; previously you could save settings without either and only discover the problem when a schema refresh errored out. The settings form now groups the two fields together, shows live feedback until one is supplied, and disables saving until then; the same rule is also enforced server-side for API callers. Connectors can now declare these "at least one of" setting groups in their metadata and JIM validates and renders them generically.
