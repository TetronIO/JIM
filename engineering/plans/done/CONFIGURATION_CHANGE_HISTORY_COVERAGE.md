...
- **Status:** Done
- **Note:** All ten phases complete. Post-close-out (2026-07-06): the Phase 3 seeded-baseline gap for the built-in Metaverse Object Types and Attributes (they were seeded without System Initialisation Create Activities or version-1 baselines, unlike every other batch-seeded type) was closed after review flagged them missing from the seeding children list; `MetaverseServer.RecordSeededMetaverseObjectTypeBaselineAsync` / `RecordSeededMetaverseAttributeBaselineAsync` now record a System-attributed Create + v1 baseline for each after the seed batch (mirroring the Predefined Search / Connector Definition pattern), taking the System Initialisation children from 24 to 112 (2 Object Types + 86 Attributes). Runtime-verified on a fresh seed, and idempotent on restart. Remaining cross-type follow-up left open by decision: batch-seeded configuration objects (now including the Metaverse Object Types/Attributes) lose their version-1 baseline Activities on factory reset (the `Activities` table is truncated but the seed objects are preserved, so re-seeding no-ops); close it by re-baselining preserved seed objects after a reset, or re-running the seeding pipeline (#916).
