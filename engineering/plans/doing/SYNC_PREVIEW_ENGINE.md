...
- [ ] **1a: In-run working set.** Introduce the staged-decision index; convert `EnsureDeletePendingExportAsync`'s per-object read-back (line ~827) to consult it, retaining the batched database lookups for pre-existing Pending Exports. **First half delivered (first extraction PR):** `ExportEvaluationWorkingSet` exists and the set-based deletion path records every verdict in it; the `EnsureDeletePendingExportAsync` consult converts with the out-of-scope family in 1c's remainder, because that is the path that reads it.
