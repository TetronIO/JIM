...
- ✨ Factory reset: a new `POST /api/v1/system/reset` endpoint and `Reset-JIMSystem` cmdlet wipe all customer data and configuration in a single transaction while preserving the schema, seeded built-ins, service settings, and infrastructure API keys. The reset is refused while any activity is in progress, and connector files on the host mount are left untouched.
...
- 🐛 Retrieving or refreshing a Connected System's schema in the admin UI now persists the discovered object types and attributes, so the selection interface appears straight afterwards instead of reading back empty on reload. A single newly-discovered object type is auto-selected so administrators land directly on attribute selection.
- 🐛 Outbound deprovisioning no longer fails with a duplicate-key error when the target object still has a pending export from a prior run; the delete is now reconciled with the existing pending export instead of colliding with it. This could previously surface when a sync ran between an export and its confirming import (overlapping schedules, or a late or failed import).
...
- 🔄 JIM is now distributed under the Tetron Software License Agreement v2.0; see the `LICENSE` file for the current terms.
