...
.DESCRIPTION
    ...
    Coverage tiers, following the Scenario 011 precedent:
...
. "$PSScriptRoot/../utils/Scenario-016-Helpers.ps1"
...
Write-TestSection "Scenario 016: JIM SQL Connector Provider x Capability Matrix"
...
# ─── Result accumulation ───────────────────────────────────────────────────────
#
# Rows accumulate rather than throwing individually, following Scenario 011: one failing capability
...
foreach ($currentProvider in $providers) {
    ...
    foreach ($seedGroup in $seedGroups) {
        ...
        & "$PSScriptRoot/../New-Scenario-016-TestDatabase.ps1" -Provider $currentProvider -RowCount $seedRowCount | Out-Null

        Write-TestStep "Setup" "Configuring JIM against $($config.DisplayName)"
        $context = & "$PSScriptRoot/../Setup-Scenario-016.ps1" `
            -JIMUrl $JIMUrl -ApiKey $ApiKey `
            -Provider $currentProvider -RowCount $seedRowCount

        if (-not $context) {
            throw "Setup-Scenario-016.ps1 returned no configuration for $currentProvider."
        }
...
Write-TestSection "Scenario 016 Summary"
Write-Host "  Coverage tier:  $activeTier" -ForegroundColor Cyan
Write-Host "  Providers:      $($providers -join ', ')" -ForegroundColor Cyan
...
if (-not $overallPass) {
    ...
    throw "Scenario 016 failed: $cellFail cell(s) did not pass ($($failed -join ', '))."
}
