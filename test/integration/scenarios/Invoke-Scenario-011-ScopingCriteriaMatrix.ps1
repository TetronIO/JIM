...
<#
.SYNOPSIS
    Test Scenario 011: Sync Rule Scoping Criteria Evaluation Matrix

.DESCRIPTION
    Exercises the full operator x value-type x group-structure evaluation matrix that
    SyncRuleScopingCriteria exposes, complementing Scenario 010 (which covers the
...
.PARAMETER DirectoryConfig
    Accepted for runner-API consistency. Not used: Scenario 011 has no LDAP step.
#>
...
param(
    ...
    # using directory snapshot images). Scenario 011 has no LDAP step and no AD users
...
Write-TestSection "Scenario 011 Setup: Sync Rule Scoping Criteria Matrix"
...
$rtCsvHostPath = Join-Path $rtTestDataDir 'scenario-011-roundtrip.csv'
...
Write-FilesToConnectorVolume -SourceDir $rtTestDataDir -Files @(
    @{ SourceFile = 'scenario-011-roundtrip.csv'; DestinationPath = '/connector-files/test-data/scenario-011-roundtrip.csv' }
) | Out-Null
...
$rtSettings = @{}
$rtSettings[($csvConnectorFull.settings | Where-Object { $_.name -eq 'File Path' }).id]          = @{ stringValue = '/connector-files/test-data/scenario-011-roundtrip.csv' }
...
$matrixCsvPath = Join-Path $matrixCsvDir 'scenario-011-matrix.csv'
...
Write-FilesToConnectorVolume -SourceDir $matrixCsvDir -Files @(
    @{ SourceFile = 'scenario-011-matrix.csv'; DestinationPath = '/connector-files/test-data/scenario-011-matrix.csv' }
) | Out-Null
...
$matrixSettings = @{}
$matrixSettings[($csvConnectorFull.settings | Where-Object { $_.name -eq 'File Path' }).id]          = @{ stringValue = '/connector-files/test-data/scenario-011-matrix.csv' }
...
$matrixSystem = New-JIMConnectedSystem -Name 'Sc11ScopingMatrixSource' -Description 'Scenario 011 scoping criteria matrix source' `
...
$importProfile = New-JIMRunProfile -ConnectedSystemId $matrixSystem.id -Name 'Sc11 Full Import' `
    -RunType FullImport -FilePath '/connector-files/test-data/scenario-011-matrix.csv' -PassThru
...
Write-TestSection "Scenario 011 Summary"
Write-Host "  Coverage tier:        $activeTier" -ForegroundColor Cyan
...
if (-not $overallPass) {
    throw "Scenario 011 failed: $cellFail cell(s) and $roundTripFail round-trip case(s) did not match expected results."
}
