...
**One correction to the PRD's framing:** requirement 21 references the "callback pattern delivered by prerequisite #637". That mechanism was superseded by #1161 before this plan was written: connectors now declare their steps via `IConnectorPhases.GetPhases()` and narrate them through a non-null `IConnectorProgress` (`EnterPhaseAsync`/`ReportAsync`) parameter on every interaction method. This plan builds against the current contract.
...
- `OpenImportConnection`/`CloseImportConnection`/`OpenExportConnection`/`CloseExportConnection` are synchronous `void`; `ValidateSettingValues` and `GetSchemaAsync` run on Blazor Server circuits. Sync-over-async bridging follows the established `Task.Run(...).GetAwaiter().GetResult()` precedent from `ServerCertificateDiagnosis.LoadTrustedCertificates`.
...
- `GetSchemaAsync`: enumerate tables/views and columns via provider catalogue queries; apply the type mapper; emit one `ConnectorSchemaObjectType` per configured object type with primary-key-derived `RecommendedExternalIdAttribute`; surface related-table value columns as multi-valued attributes; explicit per-column Reference designation from configuration, never inferred.
...
- Progress: `EnterPhaseAsync` per object type/page, `ReportAsync` row-count updates at intervals (File Connector's `ProgressRowInterval` precedent).
...
- New `scenarios/Invoke-Scenario15-SqlConnectorMatrix.ps1` + `Setup-Scenario15.ps1` (filesystem convention self-registers it in the runner menu; add the `*Scenario15*` fallback description). Parameterise by provider via a `Get-DatabaseConfig -Provider SqlServer|Oracle` helper in `utils/Test-Helpers.ps1` (the `Get-DirectoryConfig` analogue), with `-Provider`/`-Quick`/`-FullMatrix` filters following the Scenario 11 precedent: representative subset for the regular gate, full matrix before release.
