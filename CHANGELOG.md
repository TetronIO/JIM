...
- 🖥️ The Causality panel now says when an object no longer exists, instead of quietly not linking it. An Identity or record JIM can prove is gone renders with its name struck through and a **Deleted** marker, and offers **View deletion record** where nothing else on the panel already does, so a deleted object leads somewhere rather than being a dead end. The panel proves it two ways: the run being viewed recorded the deletion, or the Identity was looked up and was not there, which is what catches the common case of an object a *later* run removed. An object JIM merely cannot build a link to is deliberately treated as a different thing and says only that on hover, because its type may simply be unresolvable while the object itself is perfectly alive. That same lookup also repairs the link: an Identity whose type could not be worked out from the item now links correctly, which previously failed on every item whose record had since been deleted. (#1495)
