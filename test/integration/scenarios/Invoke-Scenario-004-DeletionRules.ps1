...
<#
.SYNOPSIS
    Test Scenario 004: MVO Deletion Rules - Comprehensive Coverage
...
.EXAMPLE
    ./Invoke-Scenario-004-DeletionRules.ps1 -Step All -Template Small -ApiKey "jim_..."

.EXAMPLE
    ./Invoke-Scenario-004-DeletionRules.ps1 -Step AuthoritativeImmediate -Template Nano -ApiKey $env:JIM_API_KEY
#>
...
Write-TestSection "Scenario 004: MVO Deletion Rules - Comprehensive Coverage"
...
function Set-ExportRuleDeprovisionAction {
    ...
    Set-JIMSyncRule -Id $rule.id -OutboundDeprovisionAction $Action `
        -ChangeReason "Scenario 004 deprovisioning action test" | Out-Null
...
# now lives in Test-Helpers.ps1, shared with Scenario 005's same-page rejoin cancellation probe (#1612).
# -----------------------------------------------------------------------------------------------------------------
...
try {
    ...
    # setup runs. We then overlay Scenario 004's minimal HR and Training CSVs on top
    ...
    # Overlay Scenario 004's tailored HR and Training CSVs (1 baseline user each)
    Write-Host "Applying Scenario 004 HR and Training overlays..." -ForegroundColor Gray
    Copy-Item -Path "$scenarioDataPath/scenario-004-hr-users.csv" -Destination "$testDataPath/hr-users.csv" -Force
    Copy-Item -Path "$scenarioDataPath/scenario-004-training-records.csv" -Destination "$testDataPath/training-records.csv" -Force
    ...
    if ($Step -eq "AuthoritativeGracePeriod" -or $Step -eq "All") {
        ...
        # display-name search miss (see engineering/notes/SCENARIO_004_TEST4_GRACE_PERIOD_INVESTIGATION.md,
    ...
    # =============================================================================================================
    # Setup-Scenario-001 creates a second target, "Cross-Domain Export", that this scenario deliberately never runs
    ...
    try {
        ...
        # Reset the export rule's Deprovisioning Action to Delete (the Setup-Scenario-001 baseline);
    ...
    $resultsFile = Join-Path $resultsDir "Scenario-004-DeletionRules-$Template-$resultsTimestamp.json"
