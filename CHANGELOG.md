...
- ✨ Predefined Searches now carry the same versioned change history: enabling or disabling a search, and every criteria group or criterion change, records who changed what and when, viewable from a new Changes tab on the search's detail page, with the "Reason for change" prompt on save and `-ChangeReason` on the `Set-JIMPredefinedSearch` and criteria cmdlets, or retrievable via `Get-JIMConfigurationChangeHistory -Type PredefinedSearch`.
