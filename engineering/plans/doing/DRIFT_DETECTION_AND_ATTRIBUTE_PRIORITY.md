...
**Surface 3: Attribute flows discovery page (new Schema list page).** A new list page under Admin > Schema (working title "Attribute Flows"; avoid the internal "IAF" label in UI) for system-wide discovery: every attribute with import flow (single or multi), its MVO object type, and contributing sync rule(s) (truncated with tooltip/expander for long names), filterable across all dimensions (connected system, CS object type, MVO object type, CS attribute, MVO attribute, free-text). The ASCII mock below illustrates this surface. Read/filter/audit first; because reorder is only meaningful within a single MVO attribute's contributor set, **management is not duplicated here in phase 1**: rows link through to Surface 2 for reordering. Open for ideation: whether to add inline management, and whether to include export flows (with a direction filter; priority controls would apply to import rows only).
...
- [ ] Build the shared ordered contributor-list Blazor component (priority, sync rule, connected system, enabled/disabled greyed state, "Null is a value", drag handle) used by Surfaces 1 and 2
...
- [ ] **Surface 3 (discovery):** new Schema "Attribute Flows" list page with filters (connected system, CS object type, MVO object type, CS attribute, MVO attribute, free-text), rows linking to Surface 2; relocatable to a future Policy area
